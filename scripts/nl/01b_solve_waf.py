"""One-off bootstrap: fetch the EUR-Lex ENIG-DOCUMENT HTMLs that
`scripts/nl/01_fetch_pliegos.py` couldn't reach because EUR-Lex sits
behind an AWS WAF that returns HTTP 202 + a JavaScript challenge for
non-browser clients. Headless Chromium runs the WAF challenge JS
automatically.

Run: `.venv/bin/python scripts/nl/01b_solve_waf.py`
After: re-run `scripts/nl/02_extract_pliegos.py` to pick up new HTMLs.
"""
...
def main() -> int:
    ...
    if not MANIFEST_PATH.exists():
        print(f"error: {MANIFEST_PATH} missing — run scripts/nl/01_fetch_pliegos.py first",
              file=sys.stderr)
        return 1
    ...
    for w in wines:
        ...
        if override and override.get("url"):
            ...
            url = to_dutch(base) if base else None
        if not url:
            continue
