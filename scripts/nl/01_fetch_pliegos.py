"""Fetch the EU-OJ "ENIG DOCUMENT" HTML page per NL wine GI.

Pipeline stage 01 (nl). All 22 NL wines have a fetchable EU-OJ URL;
WAF fallback at `scripts/nl/01b_solve_waf.py`.

Manual overrides at `raw/nl/oj-pages/manual_overrides.json` (gitignored).
"""
...
INDEX_PATH = ROOT / "raw" / "nl" / "eambrosia" / "index.json"
OUT_DIR = ROOT / "raw" / "nl" / "oj-pages"
...
def to_dutch(url: str) -> str:
    """Best-effort URL rewrite to the Dutch-language variant."""
    ...
    if re.search(r"/oj/?(?:[#?]|$)", url) and not re.search(r"/oj/[a-z]{3}", url):
        url = re.sub(r"/oj(/?)([#?]|$)", r"/oj/nld\2", url)
    url = re.sub(
    ...
    url = re.sub(
        r"(\d{2}\.)(?:ENG|FRA|DEU|ITA|POR|SPA|NLD|SLV|SLK|CES|HRV|HUN|BUL|RON|ELL|GRE)(?=&|$)",
        r"\1NLD", url,
    )
    url = re.sub(
        r"(:)(?:ENG|FRA|DEU|ITA|POR|SPA|NLD|SLV|SLK|CES|HRV|HUN|BUL|RON|ELL|GRE)(?=&|$)",
        r"\1NLD", url,
    )
    url = re.sub(
        r"/oj/(?:eng|fra|deu|ita|por|spa|nld|slv|slk|ces|hrv|hun|bul|ron|ell|gre)(/?)([#?]|$)",
        r"/oj/nld\1\2", url,
    )
    return url
...
def _is_corrigendum(pub: dict) -> bool:
    text = (pub.get("text") or "").lower()
    return any(k in text for k in ("corrigendum", "rectificatie"))
...
def main() -> int:
    ...
    if not INDEX_PATH.exists():
        print(f"error: {INDEX_PATH} missing — run scripts/nl/00_fetch_data.py first",
              file=sys.stderr)
        return 1
    ...
    for w in tqdm(wines, desc="oj-pages", leave=False):
        ...
        if override and override.get("url"):
            ...
            source_url = to_dutch(base) if base else None
        if not source_url:
            manifest[slug] = {
    ...
    MANIFEST_PATH.write_text(json.dumps({
        ...
        "n_wines": len(wines),
        "counts": {
            "ok": n_ok, "cached": n_cached, "override": n_override,
            "no_publication": n_no_pub,
            "fetch_error_or_not_single_document": n_bad,
        },
        "by_slug": manifest,
    }, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
    print(f"[done] ok={n_ok} cached={n_cached} override={n_override} "
