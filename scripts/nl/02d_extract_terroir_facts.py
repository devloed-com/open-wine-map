"""Extract noteworthy terroir facts from each NL wine's link-to-terroir
section using a bounded LLM layer with dual-source grounding (EU single
document + Dutch Wikipedia).

NL analog of `scripts/sk/02d_extract_terroir_facts.py`. Same per-bullet
provenance, fuzzy-grounding, and cache-invalidation rules; the source
language is Dutch (`nl`) for every record, and the Wikipedia hint comes
from `raw/wikipedia/aocs/nl/<slug>.json` (populated by
`scripts/02b_fetch_aoc_lexicon.py --lang nl --source raw/nl/dokumenten-extracted`).

Providers: anthropic / mistral / ollama / manual (mirrors SK/BE 02d).
"""
...
WIKI_AOCS = ROOT / "raw" / "wikipedia" / "aocs" / "nl"
CACHE_DIR = ROOT / "raw" / "terroir-facts"
MANIFEST = CACHE_DIR / "manifest-nl.json"
...
SUBSECTIONS = [
    {
        ...
        "label": "Natuurlijke factoren (geologie, bodems, klimaat, reliëf)",
        "topics": (
            "geologie, bodemtypes (löss, leem, krijt, mergel, zand, klei, "
            "kalksteen, tuffeau), klimaat (gematigd zeeklimaat, gematigd "
            "maritiem klimaat), reliëf, expositie, beken en rivieren, "
            "wijngaarden, gemeenten"
        ),
        "max_bullets": 5,
    },
    {
        "key": "facteurs_humains",
        "label": "Historische en menselijke factoren (geschiedenis, praktijken)",
        "topics": (
            "geschiedenis van de oorsprongsbenaming, Nederlandse wijnbouw "
            "(opkomst sinds 1970), wijnbouw- en wijnbereidingspraktijken, "
            "Belgisch-Nederlandse hybriden (Solaris, Johanniter, Regent, "
            "Acolon, Pinotin, Cabernet Cortis)"
        ),
        "max_bullets": 2,
    },
    {
        "key": "produit",
        "label": "Kenmerken van het product (sensorisch profiel)",
        "topics": "kleuren, aroma's, structuur, opvoeding, sensorisch profiel van de wijnen",
        "max_bullets": 2,
    },
    {
        "key": "interactions",
        "label": "Causale samenhang (verband terroir / wijn)",
        "topics": (
            "expliciet verband tussen het terroir en het karakter van de "
            "wijn, expressie van de bodem of het klimaat in de wijn"
        ),
        "max_bullets": 1,
    },
]
...
EXTRACT_SYSTEM = """Selecteer belangrijke feiten over een Nederlandse wijn-oorsprongsbenaming uit TWEE bronnen: het EU enig document / productdossier (regelgevende autoriteit) en een uittreksel uit de Nederlandstalige Wikipedia (referentie voor het brede publiek, sommelier-vocabularium).

Doelpubliek: wijnliefhebbers of geïnformeerde sommeliers die geïnteresseerd zijn in specifieke eigenheden van de oorsprong. Belangrijk is: geologische formaties met hun standaardnaam, specifieke bodemtypes (löss, leem, krijt, mergel, tuffeau, zandleem, klei, alluviale grond), lokaal klimaat en winden (gematigd zeeklimaat, gematigd maritiem klimaat, invloed van de Noordzee), traditionele Nederlandse druivenrassen + recente hybriden, kenmerkende wijnbouwpraktijken, een precies sensorisch profiel, gedateerde historische ankerpunten.
...
def _ask_line(label: str) -> str:
    return f"Behandelde subsectie: {label}"


def _document_block(lien_text: str) -> str:
    return f"Tekst van het enig document (Beschrijving van het verband):\n\n{lien_text}"
...
def _process_subsection(
    ...
    system = EXTRACT_SYSTEM.format(
        wiki_hint=wiki_hint or "(geen Wikipedia-uittreksel beschikbaar)",
        label=sub["label"],
        topics=sub["topics"],
...
def _process_record(provider, model_id: str, record: dict) -> dict:
    ...
    earned = earn_interactions(all_facts, "nl")
...
def emit_todo(out_path: Path, *, skip_cached: bool, limit: int = 0) -> int:
    ...
    print(f"[02d/nl] wrote {out_path} ({len(items)} items across "
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    for slug, slug_items in by_slug.items():
        ...
        counts[outcome] += 1
    print(f"[02d/nl] wrote {counts['wrote']} cache files; "
...
def _run_batch(args) -> int:
    ...
    if not targets:
        print("[02d/nl] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02d/nl] batch: {len(targets)} wines (provider={args.provider}, "
    ...
    def run_loop(prov):
        ...
        if getattr(prov, "kind", "") == "collecting":
            keep = CACHE_DIR
            CACHE_DIR = Path(tempfile.mkdtemp(prefix="batch-02d-nl-"))
            try:
                for rec in targets:
    ...
    batch.run_two_pass(
        ...
        sidecar=ROOT / "raw" / ".batch" / "02d-nl.json",
...
def main() -> int:
    ...
    if not WIKI_AOCS.exists():
        print(f"warning: {WIKI_AOCS} missing — bullets will be document-only. "
              "Run scripts/02b_fetch_aoc_lexicon.py --lang nl "
              "--source raw/nl/dokumenten-extracted to enable Wikipedia salience.",
              file=sys.stderr)
    ...
    terroir_verbatim.emit_for_country(
        country="nl", extracted_dir=EXTRACTED, cache_dir=CACHE_DIR,
        default_source_lang="nl", cahier_source_kind="eu-oj",
        only=args.only, log_prefix="[02d/nl]",
    )
    ...
    if not targets:
        print("[02d/nl] nothing to do — all caches valid.", file=sys.stderr)
        return 0
    ...
    if provider is None:
        print(f"[02d/nl] manual provider: {len(targets)} wines need extraction.",
              file=sys.stderr)
        return 1

    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    workers = max(1, args.workers)
    print(f"[02d/nl] {len(targets)} NL wines to extract "
    ...
    t0 = time.time()
    if workers <= 1:
        for rec in tqdm(targets, desc="terroir-nl", leave=False):
        ...
        with ThreadPoolExecutor(max_workers=workers) as ex:
            ...
            for fut in tqdm(as_completed(futures), total=len(targets), desc="terroir-nl", leave=False):
                try:
                    res = fut.result()
                    n_done += 1
                    n_facts += len(res.get("facts", []))
    ...
    print(f"[02d/nl] done: {n_done} wines, {n_facts} facts, "
