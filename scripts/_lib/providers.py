...
returning the model's text reply (stripped). The three providers are
...
cache field (`anthropic-api`, `ollama`, `manual`).

`max_tokens` and `num_ctx` are accepted by all providers; each takes what
it can use and ignores the rest. AnthropicProvider uses `max_tokens` only;
OllamaProvider uses `num_ctx` (max_tokens is silently ignored — Ollama's
default num_predict is unbounded).
"""
