"""Translate CZ per-AOC terroir-fact bullets (CS → en/fr/es/nl).

CZ analog of `scripts/sk/02e_translate_terroir_facts.py`. Same bounded-
...
is Czech (the JEDNOTNÝ DOKUMENT is canonical for CZ wines), so the
locale set is en/fr/es/nl.

Reads:  raw/terroir-facts/<slug>.json  (where country == "cz")
...
SYSTEM_PROMPT = """You translate short Czech bullets describing a Czech wine appellation's terroir, history, and wine character into {lang_name}. The bullets come from the EU "jednotný dokument" / specifikace výrobku and are aimed at sommelier students and wine enthusiasts.
...
- Geological era labels: translate to the standard {lang_name} form when one exists. When unsure, keep the Czech form.
...
SOURCE_LANG = "cs"
PROPER_NOUNS = """appellation names (Čechy, Morava, Litoměřická, Mělnická, Slovácká, Znojemská, Velkopavlovická, Mikulovská, Šobes, Znojmo, Novosedelské Slámové víno); wine-region names (Čechy, Morava); commune, podoblast and vineyard-site names; grape names (Veltlínské zelené, Ryzlink rýnský, Ryzlink vlašský, Tramín červený, Müller Thurgau, Rulandské bílé, Rulandské šedé, Rulandské modré, Frankovka, Modrý Portugal, Svatovavřinecké, Zweigeltrebe, André, Pálava, Aurelius, Hibernal, Cabernet Moravia, Neronet, Sauvignon, Chardonnay)"""
...
def emit_todo(out_path: Path, *, languages: tuple[str, ...], skip_cached: bool) -> int:
    ...
    print(f"[02e/cz] wrote {out_path} ({len(items)} items: {pretty})", file=sys.stderr)
    return 0
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    print(f"[02e/cz] {pretty}", file=sys.stderr)
    return 0
...
def _check_inputs() -> int:
    if not TERROIR_FACTS.exists():
        print(
            ...
            "scripts/cz/02d_extract_terroir_facts.py first",
            file=sys.stderr,
        )
        return 1
    return 0
...
def _run_translation_loop(
    ...
    if workers <= 1:
        for job in tqdm(jobs, desc="translate-cz", leave=False):
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in tqdm(as_completed(futures), total=len(jobs), desc="translate-cz", leave=False):
            job = futures[fut]
            try:
                ok, err = fut.result()
            except Exception as e:  # noqa: BLE001
...
def _run_batch(args, languages: tuple[str, ...]) -> int:
    ...
    if not jobs:
        print("[02e/cz] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02e/cz] batch: {len(jobs)} translations (provider={args.provider}, "
          f"model={model_id}, locales={','.join(languages)})", file=sys.stderr)
    batch.run_two_pass(
        provider=args.provider, model=model_id,
        sidecar=ROOT / "raw" / ".batch" / "02e-cz.json",
...
def main() -> int:
    ...
    if not jobs:
        print("[02e/cz] nothing to do — all caches up to date.", file=sys.stderr)
        return 0
    ...
    if provider is None:
        ...
        print(
            f"[02e/cz] manual provider: {len(jobs)} entries need translation. "
    ...
    print(
        f"[02e/cz] {len(jobs)} translations to fetch "
    ...
    print(f"[02e/cz] translated: {done}, skipped: {len(skipped)}", file=sys.stderr)
    if skipped:
        for lang, slug, err in skipped[:20]:
            print(f"  skip {lang}/{slug}: {err[:160]}", file=sys.stderr)
        print("[02e/cz] rerun this stage to retry the skipped entries.", file=sys.stderr)
    return 0
