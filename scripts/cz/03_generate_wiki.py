"""Generate one wiki/<slug>.md per CZ wine record + extend
wiki/_index.json with CZ entries.

Pipeline stage 03 (sk). Mirrors `scripts/si/03_generate_wiki.py` for
the Czech corpus. Reads `raw/cz/dokumenty-extracted/*.json`, emits
per-record markdown pages with Czech section headings, and merges SK
...
SECTION_LABELS = {
    "summary": "Shrnutí",
    "geo": "Vymezená zeměpisná oblast",
    "grapes": "Moštové odrůdy",
    "link": "Souvislost se zeměpisnou oblastí",
    "note": "Poznámka",
    "sources": "Zdroje",
}
...
def render_record(record: dict) -> str:
    ...
    if record.get("stub"):
        body += [
            "> ⚠️ **Jednotný dokument není k dispozici.** "
            f"Důvod: `{record.get('stub_reason') or 'unknown'}`. "
            "Název zůstává v rejstříku; obsah bude doplněn, jakmile bude "
            "k dispozici veřejný zdroj specifikace výrobku.",
            "",
        ]
    ...
    body += [
        ...
        f"- File number: `{record.get('file_number') or ''}`",
        "",
        "_Text jednotného dokumentu: © Evropská unie / EUR-Lex. "
        "Opětovné použití s uvedením zdroje._",
        "",
    ]
    return "\n".join(fm + body)
...
def main() -> int:
    if not EXTRACTED.exists():
        print(f"error: {EXTRACTED} missing — run scripts/cz/02_extract_pliegos.py first",
              file=sys.stderr)
        return 1
    ...
    written = 0
    cz_index: dict[str, dict] = {}
    for f in tqdm(files, desc="cz-wiki", leave=False):
        rec = json.loads(f.read_text(encoding="utf-8"))
        out_path = WIKI / f"{rec['slug']}.md"
        out_path.write_text(render_record(rec), encoding="utf-8")
        cz_index[rec["slug"]] = index_entry(rec)
        written += 1
    ...
    if WIKI_INDEX.exists():
        try:
            ...
            existing = {}
    other_kept = {k: v for k, v in existing.items() if v.get("country") != "cz"}
    merged = {**other_kept, **cz_index}
    WIKI_INDEX.write_text(json.dumps(merged, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
    print(
        f"[cz/03] wrote {written} CZ wiki pages, merged index "
        f"({len(other_kept)} non-CZ + {len(cz_index)} CZ = {len(merged)} entries) "
