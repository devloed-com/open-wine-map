"""One-off bootstrap: fetch the EUR-Lex Jednotný-dokument HTMLs that
`scripts/cz/01_fetch_pliegos.py` couldn't reach because EUR-Lex sits
...
Run: `.venv/bin/python scripts/cz/01b_solve_waf.py`
After: re-run `scripts/cz/02_extract_pliegos.py` to pick up the new HTMLs.
"""
...
def to_czech(url: str) -> str:
    ...
    new = re.sub(r"/legal-content/[A-Z]{2}/TXT(?:/HTML)?/", "/legal-content/CS/TXT/HTML/", url)
...
def main() -> int:
    ...
    if not MANIFEST_PATH.exists():
        print(f"error: {MANIFEST_PATH} missing — run scripts/cz/01_fetch_pliegos.py first",
              file=sys.stderr)
        return 1
    ...
    for w in wines:
        ...
        if override and override.get("url"):
            ...
            url = to_czech(base) if base else None
        if not url:
            continue
    ...
    with sync_playwright() as p:
        ...
        ctx = browser.new_context(
            locale="cs-CZ",
            extra_http_headers={"Accept-Language": "cs-CZ,sk;q=0.9,en;q=0.8"},
        )
        page = ctx.new_page()
