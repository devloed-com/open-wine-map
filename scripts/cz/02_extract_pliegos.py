"""Extract structured fields from each cached EUR-Lex Jednotný-dokument HTML.

Pipeline stage 02 (cz).

For each Czech wine GI in `raw/cz/eambrosia/index.json`:
  - if a cached HTML exists at `raw/cz/oj-pages/<slug>.html`, parse the
    "JEDNOTNÝ DOKUMENT" block into numbered sections (the EU wine GI
    template) and route them by Czech title keyword
  - else emit a stub record so the wine remains searchable

Output: one JSON per wine under `raw/cz/dokumenty-extracted/<slug>.json`,
plus a `_index.json` mapping slug → metadata.

v1 models the 13 Czech wine GIs as a flat corpus — vinařské podoblasti
(sub-regions of an oblast) are deferred to Phase 2 (gated on the
national-specification research), mirroring how Austria / Slovenia /
Slovakia shipped flat. In v1 none of the 13 CZ wines carry a fetchable
EU single document — they all ship as content-stubs — so this stage
mostly writes stub records and the parser ships pre-wired for any
single curator-pinned EU-OJ URL.
"""
...
from _lib.cz.region import derive_region  # noqa: E402
...
OJ_DIR = ROOT / "raw" / "cz" / "oj-pages"
OJ_MANIFEST = OJ_DIR / "manifest.json"
OUT_DIR = ROOT / "raw" / "cz" / "dokumenty-extracted"
...
def slice_jednotny_dokument(html: str) -> str | None:
...
# Czech grape-variety section: typically one item per line with `Name -
# synonym, synonym` form. Split on newlines + em-dash bullets, then on a
# plain hyphen for `Name - synonym`.
_BULLET_SPLIT_RE = re.compile(r"\s*[—•·]\s*|\n")
_NAME_SYN_SPLIT_RE = re.compile(r"\s+[-–]\s+")

_GRAPE_LINE_DROP = (
    "moštové odrůdy", "hlavní moštové", "odrůdy révy vinné", "doporučené",
    "povolené", "doplňkové",
)
...
def parse_grapes(section_text: str) -> dict:
    ...
    observation, details}. The Czech Jednotný dokument lists varieties
    without an explicit role split — default to `principal`. Unmatched
    names flow into `raw/cz/extraction-unknowns.json`."""
    out: dict[str, list] = {
        "principal": [],
        "accessory": [],
        "observation": [],
...
def parse_styles(sections: dict[str, str], titles: dict[str, str]) -> list[str]:
    ...
    for num, body in sections.items():
        ...
        if (
            "popis vína" in title_low
            or "popis vín" in title_low
            or "kategorie" in title_low
            or "další základní" in title_low
            or "další podmínky" in title_low
        ):
...
def _extract_from_html(cache: Path) -> tuple[dict[str, str], dict[str, str], str]:
    ...
    doc = slice_jednotny_dokument(html)
    if doc is None:
        return {}, {}, "no-jednotny-dokument-anchor"
...
def main() -> int:
    ...
    if not INDEX_IN.exists():
        print(f"error: {INDEX_IN} missing — run scripts/cz/00_fetch_data.py first",
              file=sys.stderr)
        return 1
    ...
    for w in tqdm(wines, desc="extract-dokumenty", leave=False):
        slug = w["slug"]
    ...
    unknowns_path = ROOT / "raw" / "cz" / "extraction-unknowns.json"
