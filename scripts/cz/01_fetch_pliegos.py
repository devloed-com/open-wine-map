"""Fetch the EU-OJ "Jednotný dokument" HTML page per CZ wine GI.

Pipeline stage 01 (cz).

For each wine in `raw/cz/eambrosia/index.json`, scan the `publications`
array for the first HTTP URL and request the Czech-language variant of
that EUR-Lex page. The page contains the canonical EU single document
("JEDNOTNÝ DOKUMENT") — the Czech equivalent of the FR cahier des
charges, the ES pliego, the IT documento unico, the AT/DE Einziges
Dokument, the SI Enotni dokument, the SK Jednotný dokument, etc.

Coverage:
  0 of the 13 CZ wines carry an EU-OJ publication URL in eAmbrosia —
  every CZ wine is an Art.107/Reg.1308/2013 grandfathered name with no
  public single-document URL. They all land as `no-publication` and
  stage 02 emits content-stubs. All 11 PDOs nonetheless appear on the
  map via Bétard 2022 (Czechia joined the EU in 2004; everything
  predates Bétard's Nov-2021 cutoff). The pipeline ships pre-wired so
  a single curator-pinned EU-OJ URL unlocks parsing without code
  changes. The WAF fallback is `scripts/cz/01b_solve_waf.py`.

Czech variant rules (same as SI/SK, language swap):
  /eli/.../oj      →  /eli/.../oj/ces
  /legal-content/EN/TXT/  →  /legal-content/CS/TXT/HTML/
  uri=...01.ENG    →  uri=...01.CES

Manual overrides:
  raw/cz/oj-pages/manual_overrides.json (gitignored, optional) keyed by
  giIdentifier or slug → {"url": "<czech-html-url>", "note": "..."}.
  Overrides bypass the eAmbrosia publications list entirely.

Outputs:
  raw/cz/oj-pages/<slug>.html          (the Czech Jednotný-dokument HTML)
  raw/cz/oj-pages/manifest.json         (per-slug status + final URL)
...
INDEX_PATH = ROOT / "raw" / "cz" / "eambrosia" / "index.json"
OUT_DIR = ROOT / "raw" / "cz" / "oj-pages"
...
def to_czech(url: str) -> str:
    """Best-effort URL rewrite: any EUR-Lex / EU-OJ URL → its Czech
    HTML variant. Idempotent — already-Czech URLs pass through."""
    if re.search(r"/oj/?(?:[#?]|$)", url) and not re.search(r"/oj/[a-z]{3}", url):
        return re.sub(r"/oj(/?)([#?]|$)", r"/oj/ces\2", url)
    new = re.sub(
        r"/legal-content/[A-Z]{2}/TXT(?:/HTML)?/",
        "/legal-content/CS/TXT/HTML/",
        url,
    )
    new = re.sub(r"(\d{2}\.)(?:ENG|FRA|DEU|ITA|POR|SPA|NLD|SLV|SLK|CES)(?=&|$)", r"\1CES", new)
    new = re.sub(r"(:)(?:ENG|FRA|DEU|ITA|POR|SPA|NLD|SLV|SLK|CES)(?=&|$)", r"\1CES", new)
    return new
...
def _is_corrigendum(pub: dict) -> bool:
    text = (pub.get("text") or "").lower()
    return "corrigendum" in text or "korigendum" in text or "oprava" in text
...
def looks_like_single_document(html: str) -> bool:
    ...
    publication. The Jednotný-dokument HTML is large (>8 KB) and contains
    literal Czech phrases."""
    if len(html) < 8000:
        return False
    low = html.lower()
    return (
        "jednotný dokument" in low
        or "specifikace výrobku" in low
        or "specifikace produktu" in low
        or "chráněné označení původu" in low
        or "chráněné zeměpisné označení" in low
        or "zeměpisné označení" in low
    )
...
def main() -> int:
    ...
    if not INDEX_PATH.exists():
        print(f"error: {INDEX_PATH} missing — run scripts/cz/00_fetch_data.py first",
              file=sys.stderr)
        return 1
    ...
    for w in tqdm(wines, desc="oj-pages", leave=False):
        ...
        if override and override.get("url"):
            ...
            source_url = to_czech(base) if base else None
