...
single document could not be fetched. Mirrors `audit_it_coverage.py`
for the Austrian corpus.

Run: `.venv/bin/python scripts/audit_at_coverage.py`
"""
...
def _load_figshare_file_numbers() -> set[str]:
    ...
    try:
        ...
        return set(
            ...
            if fn and fn.startswith(("PDO-AT", "PGI-AT"))
        )
    except ImportError:
        return set()
...
def main() -> int:
    ...
    print(f"Figshare AT PDOids:   {len(figshare_ids)}")
    print()
    ...
    grape_total = 0
    n_with_grapes = 0
    by_bundesland: Counter[str] = Counter()
    ...
    for w in wines:
        ...
        by_bundesland[rec.get("bundesland") or "?"] += 1
    ...
    print(f"  Missing from Figshare: {no_figshare} "
          f"(the 3 PGIs resolve via bundesland-pdo-union)")
    print()
    print("## Bundesland distribution")
    for b, n in sorted(by_bundesland.items(), key=lambda x: -x[1]):
        print(f"  {n:>4}  {b}")
    print()
