"""Translate wine-style Wikipedia extracts into target locales where the
native Wikipedia page is missing.

Pipeline stage 02b/styles-translate — sister stage to 02b_fetch_style_lexicon
and to the cahier translators (02c / 02e). For every taxonomy slug whose
target-locale Wikipedia entry is missing or 404, picks the best available
source-locale extract (preference order EN > FR > ES > NL), translates it
into the target locale, and caches the result under
`raw/translations/styles/<lang>/<slug>.json` with full source attribution.

Stage 04 merges the translated cache on top of the native fetch cache,
producing a unified per-locale style lexicon. The UI renders translated
entries with "translated from <source-locale> Wikipedia · CC BY-SA 4.0"
attribution instead of the FR-fallback marker.

Providers (matching 02c / 02e):
  anthropic — Anthropic Messages API (requires ANTHROPIC_API_KEY).
  mistral   — Mistral Chat API (requires MISTRAL_API_KEY; default mistral-large-latest).
...
Reads:
  raw/wikipedia/styles/<lang>/<slug>.json (native fetches; some `missing`)
  raw/wikipedia/style_overrides.json      (_curated list of slugs)

Writes:
  raw/translations/styles/<lang>/<slug>.json
  raw/translations/styles/manifest.json
"""
...
NATIVE_DIR = ROOT / "raw" / "wikipedia" / "styles"
OVERRIDES_FILE = ROOT / "raw" / "wikipedia" / "style_overrides.json"
CACHE_ROOT = ROOT / "raw" / "translations" / "styles"
MANIFEST = CACHE_ROOT / "manifest.json"

LOCALES = ("fr", "en", "es", "nl")
SOURCE_PREFERENCE = ("en", "fr", "es", "nl")
...
SYSTEM_PROMPT = """You translate short Wikipedia paragraphs about wine styles into the target language.

Style:
- Plain prose. No markdown, no list formatting, no quotes around the output, no trailing brackets like [...].
- Preserve proper nouns exactly as in the source: appellation names, region names, grape variety names, fermentation-style names (Crémant, Vin jaune, Sherry, Manzanilla, Amontillado, Tempranillo, etc.).
...
def load_native_entry(lang: str, slug: str) -> dict | None:
    """Read a native Wikipedia fetch from raw/wikipedia/styles. Returns None
    if absent, the entry dict otherwise (which may itself be a missing/error
    marker — callers must check `missing`/`error`)."""
    return cache.read_json_or_none(NATIVE_DIR / lang / f"{slug}.json")


def native_extract_for(lang: str, slug: str) -> dict | None:
    """Return a usable native entry, or None if missing/error/empty."""
...
def pick_source(slug: str, target_lang: str) -> dict | None:
    """Pick the best source-locale entry for translating into `target_lang`.
    Returns `{lang, extract, page_url, revision_id}` or None if no locale
    has a usable native entry."""
...
def _enumerate_jobs(
    curated: list[str], target_locales: tuple[str, ...], *, skip_cached: bool = True,
) -> list[dict]:
    """Build the work list: every (target_lang, slug) where:
    - The target_lang has no usable native entry, AND
    - At least one other locale has a usable native entry to translate from,
    - And the existing translation cache (if any) is stale or absent.
    """
    jobs: list[dict] = []
    for slug in curated:
        for tgt in target_locales:
            if native_extract_for(tgt, slug) is not None:
                continue  # native covers it; no translation needed.
            source = pick_source(slug, tgt)
            if source is None:
                continue  # no locale has it; nothing to translate from.
            if skip_cached:
...
def _translate_one(provider, *, source_extract: str, source_lang: str, target_lang: str) -> str:
    user = (
        f"Translate this {LOCALE_NAME[source_lang]} Wikipedia paragraph about a wine style "
...
def _run_translation_loop(provider, model_id: str, jobs: list[dict], workers: int):
    ...
    if workers <= 1:
        for job in tqdm(jobs, desc="translate-styles", leave=False):
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in tqdm(as_completed(futures), total=len(jobs), desc="translate-styles", leave=False):
            job = futures[fut]
            try:
                ok, err = fut.result()
            except Exception as e:  # noqa: BLE001
...
def emit_todo_file(
    out_path: Path, *, curated: list[str], target_locales: tuple[str, ...],
    ...
    for lang, items in blocks:
        ...
        for it in items:
            ...
            if not source["extract"]:
                skipped_empty += 1
                continue
            # Re-fetch current source so we can sha-mismatch-skip if it drifted.
            current = pick_source(slug, lang)
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="translation backend (default: ollama; matches the user's preference).",
    )
    ap.add_argument(
        "--model", default=None,
    ...
    if not curated:
        print("error: no curated slugs in style_overrides.json", file=sys.stderr)
        return 1

    if args.emit_todo:
        return emit_todo_file(
            Path(args.emit_todo), curated=curated, target_locales=target_locales,
    ...
    if args.provider == "manual":
        # List missing work and exit non-zero so CI can branch.
        jobs = _enumerate_jobs(curated, target_locales, skip_cached=not args.refresh)
        if not jobs:
            print("[02b-tx] nothing to translate (cache satisfies all needs).", file=sys.stderr)
            return 0
        print(f"[02b-tx] {len(jobs)} (slug, lang) pairs need translation:", file=sys.stderr)
        for j in jobs[:30]:
            src = j["source"]
    ...
    print(f"[02b-tx] translating {len(jobs)} (slug, lang) pairs with {provider.kind} "
