...
fetched. Mirrors `audit_gr_coverage.py` for the Greecen corpus.
...
EAMBROSIA = ROOT / "raw" / "gr" / "eambrosia" / "index.json"
EXTRACTED = ROOT / "raw" / "gr" / "dokumenti-extracted"
OJ_MANIFEST = ROOT / "raw" / "gr" / "oj-pages" / "manifest.json"
OVERRIDES = ROOT / "raw" / "gr" / "oj-pages" / "manual_overrides.json"
...
def _load_figshare_file_numbers() -> set[str]:
    ...
    try:
        ...
        return set(
            ...
            if fn and (fn.startswith("PDO-GR") or fn.startswith("PGI-GR"))
        )
    except ImportError:
        return set()
...
def main() -> int:
    ...
    print(f"Figshare GR PDOids:   {len(figshare_ids)}")
    print()
    ...
    if not curator_targets:
        print("  (empty — every Greecen wine extracted)")
    print()
    ...
    try:
        ...
        _vb_count, _vb_records = _verbatim.count_verbatim_records(
            Path(__file__).resolve().parent.parent / "raw" / "terroir-facts", "gr",
        )
        if _vb_count:
