...
`scripts/at/01_fetch_pliegos.py` couldn't reach because EUR-Lex sits
...
Run: `.venv/bin/python scripts/at/01b_solve_waf.py`
After: re-run `scripts/at/02_extract_pliegos.py` to pick up the new HTMLs.
"""
...
def main() -> int:
    ...
    if not MANIFEST_PATH.exists():
        print(f"error: {MANIFEST_PATH} missing — run scripts/at/01_fetch_pliegos.py first",
              file=sys.stderr)
        return 1
