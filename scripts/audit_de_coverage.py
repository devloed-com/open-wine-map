...
Run: `.venv/bin/python scripts/audit_de_coverage.py`
"""
...
EAMBROSIA = ROOT / "raw" / "de" / "eambrosia" / "index.json"
EXTRACTED = ROOT / "raw" / "de" / "dokumente-extracted"
OJ_MANIFEST = ROOT / "raw" / "de" / "oj-pages" / "manifest.json"
OVERRIDES = ROOT / "raw" / "de" / "oj-pages" / "manual_overrides.json"
...
def _load_figshare_file_numbers() -> set[str]:
    ...
    try:
        ...
        return set(
            ...
            if fn and fn.startswith(("PDO-DE", "PGI-DE"))
        )
    except ImportError:
        return set()
...
def main() -> int:
    ...
    print(f"Figshare DE PDOids:   {len(figshare_ids)}")
    print()
    ...
    print(f"  Einzellage sub-denominations: {n_sub_denom}")
    print()
    print("## Stub reasons")
    ...
    print(f"  Missing from Figshare: {no_figshare} "
          f"(Einzellage subs inherit parent; PGIs use region-pdo-union)")
    print()
    ...
    if not curator_targets:
        print("  (empty — every German wine extracted)")
    print()
    ...
    try:
        ...
        _vb_count, _vb_records = _verbatim.count_verbatim_records(
            Path(__file__).resolve().parent.parent / "raw" / "terroir-facts", "de",
        )
        if _vb_count:
