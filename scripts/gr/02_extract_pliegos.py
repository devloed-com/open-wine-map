...
Pipeline stage 02 (gr).

For each Greek wine GI in `raw/gr/eambrosia/index.json`:
  - if a cached HTML exists at `raw/gr/oj-pages/<slug>.html`, parse the
...
In addition to the standard role-routed sections, the GR extractor
...
Output: one JSON per wine under `raw/gr/dokumenti-extracted/<slug>.json`,
...
from _lib.gr.commune import parse_commune_list  # noqa: E402
...
OUT_DIR = ROOT / "raw" / "gr" / "dokumenti-extracted"
...
def main() -> int:
    ...
    if not INDEX_IN.exists():
        print(f"error: {INDEX_IN} missing — run scripts/gr/00_fetch_data.py first",
              file=sys.stderr)
        return 1
    ...
    unknowns_path = ROOT / "raw" / "gr" / "extraction-unknowns.json"
