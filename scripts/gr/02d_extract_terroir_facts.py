...
differences are:
  ...
  - Wikipedia hint = `raw/wikipedia/aocs/el/<slug>.json` (fetched by
    `scripts/02b_fetch_aoc_lexicon.py --lang bg --source raw/gr/dokumenti-extracted`).
  - LLM prompt is in Bulgarian and asks for Bulgarian bullets.
  - Output cache = `raw/terroir-facts/<slug>.json` (single shared dir;
    `country: "gr"` distinguishes from FR/ES/PT/IT/AT/SI/HR/RO records).
...
MANIFEST = CACHE_DIR / "manifest-gr.json"
...
def emit_todo(out_path: Path, *, skip_cached: bool, limit: int = 0) -> int:
    ...
    print(
        f"[02d/gr] wrote {out_path} ({len(items)} items across "
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    for slug, slug_items in by_slug.items():
        ...
        counts[outcome] += 1
    print(
        f"[02d/gr] wrote {counts['wrote']} cache files; "
...
def _run_batch(args) -> int:
    ...
    if not targets:
        print("[02d/gr] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02d/gr] batch: {len(targets)} wines (provider={args.provider}, "
    ...
    def run_loop(prov):
        ...
        if getattr(prov, "kind", "") == "collecting":
            keep = CACHE_DIR
            CACHE_DIR = Path(tempfile.mkdtemp(prefix="batch-02d-gr-"))
            try:
                for rec in targets:
    ...
    batch.run_two_pass(
        ...
        sidecar=ROOT / "raw" / ".batch" / "02d-gr.json",
...
def main() -> int:
    ...
    if not EXTRACTED.exists():
        print(
            f"error: {EXTRACTED} missing — run scripts/gr/02_extract_pliegos.py first",
            file=sys.stderr,
        )
        return 1
    if not WIKI_AOCS.exists():
        print(
            ...
            "--source raw/gr/dokumenti-extracted to enable Wikipedia salience.",
            file=sys.stderr,
        )
    ...
    if not targets:
        print("[02d/gr] nothing to do — all caches valid.", file=sys.stderr)
        return 0
    ...
    if provider is None:
        print(
            f"[02d/gr] manual provider: {len(targets)} wines need extraction. "
    ...
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    print(
        f"[02d/gr] {len(targets)} BG wines to extract "
    ...
    t0 = time.time()
    if workers <= 1:
        for rec in tqdm(targets, desc="terroir-gr", leave=False):
        ...
        with ThreadPoolExecutor(max_workers=workers) as ex:
            ...
            for fut in tqdm(as_completed(futures), total=len(targets), desc="terroir-gr", leave=False):
                try:
                    res = fut.result()
                    n_done += 1
                    n_facts += len(res.get("facts", []))
    ...
    print(
        f"[02d/gr] done: {n_done} wines, {n_facts} facts, "
