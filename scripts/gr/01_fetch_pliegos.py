"""Fetch the EU-OJ "ΕΝΙΑΙΟ ΕΓΓΡΑΦΟ" HTML page per GR wine GI.

Pipeline stage 01 (gr).

For each wine in `raw/gr/eambrosia/index.json`, scan the `publications`
array for the first HTTP URL and request the Greek-language variant of
that EUR-Lex page. The page contains the canonical EU single document
("ΕΝΙΑΙΟ ΕΓΓΡΑΦΟ") — the Greek equivalent of the FR cahier des charges,
the ES pliego, the IT documento unico, the AT Einziges Dokument, the SI
Enotni dokument, the HR Jedinstveni dokument, the HU Egységes dokumentum,
the RO DOCUMENT UNIC, and the BG Единен документ.

Coverage:
  Only ~11 of the 147 GR wines carry a fetchable OJ publication URL —
  the other ~136 are Art.107 / Reg.1308/2013 grandfathered names with
  only an `Ares(...)` summary-sheet reference. They land as
  `no-publication` and stage 02 emits content-stubs (the IT/ES/SI/HR/HU
  /RO/BG curator-queue pattern). The other failure mode is the EUR-Lex
  AWS WAF (HTTP 202 + JavaScript challenge); `scripts/gr/01b_solve_waf.py`
  bootstraps the blocked subset via headless Chromium.

Greek variant rules (same as ES/IT/AT/SI/HR/HU/RO/BG, with the language
swap):
  /eli/.../oj  →  /eli/.../oj/ell  (3-letter ISO 639-2/T for Greek)
  legal-content/EN/TXT/  →  legal-content/EL/TXT/HTML/
  uri=...01.ENG&...     →  uri=...01.ELL&...

Manual overrides:
  raw/gr/oj-pages/manual_overrides.json (gitignored, optional) keyed by
  giIdentifier or slug → {"url": "<greek-html-url>", "note": "..."}.
  Overrides bypass the eAmbrosia publications list entirely.

Outputs:
  raw/gr/oj-pages/<slug>.html          (the Greek ΕΝΙΑΙΟ ΕΓΓΡΑΦΟ HTML)
  raw/gr/oj-pages/manifest.json        (per-slug status + final URL)
...
INDEX_PATH = ROOT / "raw" / "gr" / "eambrosia" / "index.json"
OUT_DIR = ROOT / "raw" / "gr" / "oj-pages"
...
def to_greek(url: str) -> str:
    """Best-effort URL rewrite: any EUR-Lex / EU-OJ URL → its Greek
    HTML variant. Idempotent — already-Greek URLs pass through."""
    if re.search(r"/oj/?(?:[#?]|$)", url) and not re.search(r"/oj/[a-z]{3}", url):
        return re.sub(r"/oj(/?)([#?]|$)", r"/oj/ell\2", url)
    new = re.sub(
        r"/legal-content/[A-Z]{2}/TXT(?:/HTML)?/",
        "/legal-content/EL/TXT/HTML/",
        url,
    )
    new = re.sub(
        r"(\d{2}\.)(?:ENG|FRA|DEU|ITA|POR|SPA|NLD|SLV|HRV|HUN|RON|BUL|ELL)(?=&|$)",
        r"\1ELL", new,
    )
    new = re.sub(
        r"(:)(?:ENG|FRA|DEU|ITA|POR|SPA|NLD|SLV|HRV|HUN|RON|BUL|ELL)(?=&|$)",
        r"\1ELL", new,
    )
    return new
...
def _is_corrigendum(pub: dict) -> bool:
    text = (pub.get("text") or "").lower()
    return "corrigendum" in text or "διορθωτικό" in text or "διορθωση" in text
...
def fetch_page(session: requests.Session, url: str) -> requests.Response | None:
    """Fetch with 202-aware retry. EUR-Lex generates `legal-content/EL/TXT/HTML`
    pages on demand: the first request returns HTTP 202 and triggers
    server-side rendering. The AWS WAF also returns 202 — those stay
    202 across retries and get handed off to stage 01b."""
    backoff = (1.0, 3.0, 9.0)
...
def looks_like_single_document(html: str) -> bool:
    ...
    publication. The ΕΝΙΑΙΟ ΕΓΓΡΑΦΟ HTML is large (>8 KB) and contains
    literal Greek phrases."""
    if len(html) < 8000:
        return False
    low = html.lower()
    return (
        "ενιαίο έγγραφο" in low
        or "ενιαιο εγγραφο" in low
        or "προστατευόμενη ονομασία προέλευσης" in low
        or "προστατευόμενη γεωγραφική ένδειξη" in low
        or "γεωγραφική ένδειξη" in low
        or "ονομασία προέλευσης" in low
        or "προδιαγραφές προϊόντος" in low
    )
...
def main() -> int:
    ...
    if not INDEX_PATH.exists():
        print(f"error: {INDEX_PATH} missing — run scripts/gr/00_fetch_data.py first",
              file=sys.stderr)
        return 1
    ...
    for w in tqdm(wines, desc="oj-pages", leave=False):
        ...
        if override and override.get("url"):
            ...
            source_url = to_greek(base) if base else None
