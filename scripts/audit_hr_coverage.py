...
single document could not be fetched. Mirrors `audit_si_coverage.py`
for the Croatian corpus.

Run: `.venv/bin/python scripts/audit_hr_coverage.py`
"""
...
def _load_figshare_file_numbers() -> set[str]:
    ...
    try:
        ...
        return set(
            ...
            if fn and fn.startswith("PDO-HR")
        )
    except ImportError:
        return set()
...
def main() -> int:
    ...
    print(f"Figshare HR PDOids:   {len(figshare_ids)}")
    print()
    ...
    print(f"  Missing from Figshare: {no_figshare} "
          f"(should be 0 — Croatia has no IGPs and all PDOs are in Bétard)")
    print()
