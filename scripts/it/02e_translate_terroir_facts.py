"""Translate IT per-AOC terroir-fact bullets (IT → en/fr/es/nl).

IT analog of `scripts/02e_translate_terroir_facts.py` (FR),
`scripts/es/02e_translate_terroir_facts.py` (ES), and
`scripts/pt/02e_translate_terroir_facts.py` (PT). Same bounded-narrative-
layer rules — cache per source SHA, attribution preserved, round-trip
flow for forks without API access — but the source language is Italian
(the documento unico / MASAF disciplinare is canonical for IT wines),
so the locale set becomes en/fr/es/nl (FR + ES are now targets, not
sources).

Reads:  raw/terroir-facts/<slug>.json  (where country == 'it')
Writes: raw/translations/terroir-facts/<lang>/<slug>.json

Per locale cache shape mirrors the FR/ES/PT 02e output (slug, lang,
facts, source_facts_sha, wiki_source_url, cahier_source_pdf_url,
translator, translator_kind, fetched_at) plus `country: "it"` and
`source_lang: "it"` so stage 04 can branch on origin if it ever needs
to.
"""
...
SYSTEM_PROMPT = """You translate short Italian bullets describing an Italian wine appellation's terroir, history, and wine character into {lang_name}. The bullets come from the MASAF / EU documento unico disciplinare di produzione and are aimed at sommelier students and wine enthusiasts.
...
def _load_it_source(path: Path) -> dict | None:
    """Return IT terroir-facts payload for `path` if it has facts; else None."""
...
def emit_todo(out_path: Path, *, languages: tuple[str, ...], skip_cached: bool) -> int:
    ...
    print(f"[02e/it] wrote {out_path} ({len(items)} items: {pretty})", file=sys.stderr)
    return 0
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    print(f"[02e/it] {pretty}", file=sys.stderr)
    return 0
...
def _check_inputs() -> int:
    if not TERROIR_FACTS.exists():
        print(
            "error: raw/terroir-facts is missing — run scripts/it/02d_extract_terroir_facts.py first",
            file=sys.stderr,
        )
        return 1
    return 0
...
def _print_manual_listing(jobs: list[dict]) -> int:
    ...
    print(
        f"[02e/it] manual provider: {len(jobs)} entries need translation. "
...
def _run_translation_loop(
    ...
    if workers <= 1:
        for job in tqdm(jobs, desc="translate-it", leave=False):
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in tqdm(as_completed(futures), total=len(jobs), desc="translate-it", leave=False):
            job = futures[fut]
            try:
                ok, err = fut.result()
            except Exception as e:  # noqa: BLE001
...
def _run_batch(args, languages: tuple[str, ...]) -> int:
    ...
    if not jobs:
        print("[02e/it] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02e/it] batch: {len(jobs)} translations (provider={args.provider}, "
          f"model={model_id}, locales={','.join(languages)})", file=sys.stderr)
    batch.run_two_pass(
        provider=args.provider, model=model_id,
        sidecar=ROOT / "raw" / ".batch" / "02e-it.json",
...
def main() -> int:
    ...
    if not jobs:
        print("[02e/it] nothing to do — all caches up to date.", file=sys.stderr)
        return 0
    ...
    print(
        f"[02e/it] {len(jobs)} translations to fetch "
    ...
    print(f"[02e/it] translated: {done}, skipped: {len(skipped)}", file=sys.stderr)
    if skipped:
        ...
        print("[02e/it] rerun this stage to retry the skipped entries.", file=sys.stderr)
    return 0
