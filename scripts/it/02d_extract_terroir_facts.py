...
def _ask_line(label: str) -> str:
    return f"Sotto-sezione da trattare: {label}"


def _document_block(lien_text: str) -> str:
    return f"Testo del disciplinare (Legame con la zona geografica):\n\n{lien_text}"
...
def _process_record(provider, model_id: str, record: dict) -> dict:
    ...
    earned = earn_interactions(all_facts, "it")
...
def main() -> int:
    ...
    terroir_verbatim.emit_for_country(
        country="it", extracted_dir=EXTRACTED, cache_dir=CACHE_DIR,
        default_source_lang="it", cahier_source_kind="eu-oj",
        only=args.only, log_prefix="[02d/it]",
    )
