"""Fetch the EU-OJ "documento unico" HTML page per IT wine GI.

Pipeline stage 01 (it).

For each wine in `raw/it/eambrosia/index.json`, scan the `publications`
array for the first HTTP URL and request the Italian-language variant
of that EUR-Lex page. The page contains the canonical EU "documento
unico" (template sections 1–9 or 1–10) — the Italian equivalent of the
FR cahier des charges and the ES pliego.

Coverage caveat:
  Only 139 of 531 IT wines (26 %) have an EUR-Lex URL in their eAmbrosia
  publications array. A further 62 have only Commission-internal
  `Ares(YYYY)NNNNN` references with numeric URIs (not directly
  fetchable); 330 have no publications at all. Together that's 392
  wines (74 %) that need the MASAF fallback path (stage 02f) or the
  curator queue. Manifest entries reflect this — `status:
  "no-publication"` for both the empty-publications case and the
  ares-only case.

URL forms handled (same as ES, with EN→IT swap):
...
Italian variant rules:
  /eli/.../oj  →  /eli/.../oj/ita  (3-letter ISO 639-2)
  legal-content/EN/TXT/  →  legal-content/IT/TXT/HTML/
  uri=...01.ENG&...     →  uri=...01.ITA&...

Manual overrides:
  raw/it/oj-pages/manual_overrides.json (gitignored, optional) keyed by
  giIdentifier or slug → {"url": "<italian-html-url>", "note": "..."}.
  Overrides bypass the eAmbrosia publications list entirely. Used for
  wines whose publications list contains only Ares-internal refs (the
  curator finds the real OJ URL by other means).

Outputs:
  raw/it/oj-pages/<slug>.html          (the Italian documento-unico HTML)
  raw/it/oj-pages/manifest.json         (per-slug status + final URL)
...
def to_italian(url: str) -> str:
    """Best-effort URL rewrite: any EUR-Lex / EU-OJ URL → its Italian HTML
    variant. Idempotent — already-Italian URLs pass through unchanged."""
...
def _is_oj_c_publication(pub: dict) -> bool:
    """OJ Series C carries the documento unico; OJ Series L is the
    implementing regulation. Distinguish via the `text` and URL form
    (see scripts/es/01_fetch_pliegos.py for the rationale)."""
...
def first_http_publication(pubs: list[dict]) -> str | None:
    """eAmbrosia's `publications` interleaves OJ-cite URLs (http) with
    internal Ares document numbers (e.g. uri="86888" with no scheme).
    We want the first http one, preferring OJ C (documento unico) over
    OJ L (modification regulation)."""
    for pub in pubs:
        uri = pub.get("uri") or ""
...
def fetch_page(session: requests.Session, url: str) -> requests.Response | None:
    """Fetch with 202-aware retry. EUR-Lex generates `legal-content/IT/TXT/HTML`
    pages on demand: the first request returns HTTP 202 and triggers
    server-side rendering; subsequent requests return 200."""
    backoff = (1.0, 3.0, 9.0)
...
def looks_like_single_document(html: str) -> bool:
    """Heuristic that flags pages that are NOT a documento-unico publication.
    EUR-Lex serves generic index / not-found pages in some cases. The
    documento-unico HTML is large (>30 KB) and contains literal Italian
    phrases."""
    if len(html) < 8000:
        return False
    low = html.lower()
...
def _save_response(
    ...
    MASAF / Gazzetta Ufficiale overrides may serve `application/pdf`.
    Returns (cache_path, format) on success, or None if the response is
    an HTML page that doesn't look like a documento unico."""
...
def main() -> int:
    ...
    if not INDEX_PATH.exists():
        print(f"error: {INDEX_PATH} missing — run scripts/it/00_fetch_data.py first",
              file=sys.stderr)
        return 1
    ...
    for w in tqdm(wines, desc="oj-pages", leave=False):
        ...
        if override and override.get("url"):
            ...
            source_url = to_italian(base) if base else None
