...
for the Slovak corpus.

Run: `.venv/bin/python scripts/audit_sk_coverage.py`
"""
...
EAMBROSIA = ROOT / "raw" / "sk" / "eambrosia" / "index.json"
...
def _load_figshare_file_numbers() -> set[str]:
    ...
    try:
        ...
        return set(
            ...
            if fn and fn.startswith(("PDO-SK", "PGI-SK"))
        )
    except ImportError:
        return set()
...
def main() -> int:
    ...
    print(f"Figshare SK PDOids:   {len(figshare_ids)}")
    print()
    ...
    print(f"  Missing from Figshare: {no_figshare} "
          f"(the 1 PGI resolves via region-pdo-union; the post-Bétard Tokajské víno PDO aliases the Vinohradnícka oblasť Tokaj polygon)")
    print()
    ...
    if not curator_targets:
        print("  (empty — every Slovak wine extracted)")
    print()
    ...
    try:
        ...
        _vb_count, _vb_records = _verbatim.count_verbatim_records(
            Path(__file__).resolve().parent.parent / "raw" / "terroir-facts", "sk",
        )
        if _vb_count:
