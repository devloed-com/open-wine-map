"""Audit BE pipeline coverage.

Prints extraction status per kind, stub reasons, Bétard geometry
coverage, grape extraction, region distribution, and a curator queue
for any wine whose single document could not be fetched.

Run: `.venv/bin/python scripts/audit_be_coverage.py`
"""
...
EAMBROSIA = ROOT / "raw" / "be" / "eambrosia" / "index.json"
EXTRACTED = ROOT / "raw" / "be" / "dokumenten-extracted"
OJ_MANIFEST = ROOT / "raw" / "be" / "oj-pages" / "manifest.json"
OVERRIDES = ROOT / "raw" / "be" / "oj-pages" / "manual_overrides.json"
...
def main() -> int:
    ...
    for w in wines:
        slug = w["slug"]
        kind = w.get("kind", "?")
        lang = w.get("source_lang", "?")
        by_kind_total[kind] += 1
        by_source_lang[lang] += 1
    ...
    print()
    print("## By source language")
    for lang, n in sorted(by_source_lang.items(), key=lambda x: -x[1]):
        print(f"  {n:>4}  {lang}")
    print()
    print("## Stub reasons")
    ...
    print(f"  Missing from Figshare: {no_figshare} "
          f"(the 2 PGIs resolve via region-pdo-union)")
    print()
    ...
    for w in wines:
        ...
        curator_targets.append((rec.get("stub_reason") or "unknown", slug,
                                 w.get("fileNumber", ""), w.get("source_lang", "?")))
    curator_targets.sort()

    print(f"## Curator queue ({len(curator_targets)} stubs needing input)")
    for reason, slug, fn, lang in curator_targets:
        print(f"  [{reason:30s}] [{lang}] {slug:40s} {fn}")
    if not curator_targets:
        print("  (empty — every Belgian wine extracted)")
    print()
    ...
    try:
        ...
        _vb_count, _vb_records = _verbatim.count_verbatim_records(
            Path(__file__).resolve().parent.parent / "raw" / "terroir-facts", "be",
        )
        if _vb_count:
