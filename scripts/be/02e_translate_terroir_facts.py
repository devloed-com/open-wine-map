"""Translate BE per-AOC terroir-fact bullets to en/fr/es/nl, with
per-record source_lang awareness (Flemish wines: nl→{en,fr,es};
Walloon wines: fr→{en,nl,es}).

BE analog of `scripts/ch/02e_translate_terroir_facts.py`. Same bounded-
...
is per-record, so the locale set per job is computed as
TARGET_LOCALES - {source_lang}.

Reads:  raw/terroir-facts/<slug>.json  (where country == "be")
...
SOURCE_LANG_NAME = {"nl": "Dutch", "fr": "French"}


SYSTEM_PROMPT_NL = """You translate short Dutch bullets describing a Belgian wine appellation's terroir, history, and wine character into {lang_name}. The bullets come from the EU "ENIG DOCUMENT" / productdossier and are aimed at sommelier students and wine enthusiasts.
...
SYSTEM_PROMPT_FR = """You translate short French bullets describing a Belgian wine appellation's terroir, history, and wine character into {lang_name}. The bullets come from the EU "DOCUMENT UNIQUE" / cahier des charges and are aimed at sommelier students and wine enthusiasts.

Rules:
- Output a JSON array of strings, one translated bullet per input bullet, in the SAME order. The array length must equal the input list length.
- Geological era labels: translate to the standard {lang_name} form when one exists. When unsure, keep the French form.
- Translate descriptive vocabulary naturally for a wine-literate reader.
- Match each source bullet's length and register; do not add commentary, footnotes, or explanations.
- Output ONLY the JSON array, no preface, no markdown fences."""


SYSTEM_PROMPTS = {"nl": SYSTEM_PROMPT_NL, "fr": SYSTEM_PROMPT_FR}

PROPER_NOUNS = {
    "nl": """appellation names (Hagelandse wijn, Haspengouwse wijn, Heuvellandse wijn, Vlaamse landwijn, Vlaamse mousserende kwaliteitswijn, Maasvallei Limburg); region names (Vlaanderen, Hageland, Haspengouw, Heuvelland, Maasvallei); commune and village names; grape names (Acolon, Dornfelder, Pinotin, Cabernet Cortis, Regent, Solaris, Johanniter, Auxerrois, Riesling, Gewürztraminer, Müller-Thurgau, Chardonnay, Pinot blanc, Pinot gris, Pinot noir, Siegerrebe)""",
    "fr": """appellation names (Côtes de Sambre et Meuse, Crémant de Wallonie, Vin mousseux de qualité de Wallonie, Vin de pays des jardins de Wallonie); region names (Wallonie, Sambre, Meuse, Hesbaye, Condroz, Ardenne); commune names; grape names (Chardonnay, Pinot noir, Pinot blanc, Pinot gris, Auxerrois, Müller-Thurgau, Acolon, Régent, Solaris, Johanniter, Pinotin)""",
}
...
def targets_for(source_lang: str) -> tuple[str, ...]:
    """For a given source_lang, return the target locales (TARGET_LOCALES
    minus the source language itself)."""
    return tuple(loc for loc in TARGET_LOCALES if loc != source_lang)
...
def _load_be_source(path: Path) -> dict | None:
...
def enumerate_jobs(
    languages: tuple[str, ...] | None = None, *, skip_cached: bool = True,
) -> list[dict]:
    jobs: list[dict] = []
    for f in sorted(TERROIR_FACTS.glob("*.json")):
        if f.name.startswith("_") or f.name.startswith("manifest"):
            continue
        src = _load_be_source(f)
        if src is None:
            continue
        src_facts = src["facts"]
        sha = facts_sha(src_facts)
        src_lang = src.get("source_lang") or "nl"
        per_record_targets = targets_for(src_lang)
        wanted = (
            tuple(t for t in languages if t in per_record_targets)
            if languages is not None
            else per_record_targets
        )
        for lang in wanted:
...
def translate_one(provider, job: dict) -> tuple[list[str] | None, str | None]:
    src_lang = job["source_lang"]
    system = translation_system_prompt(
        SYSTEM_PROMPTS[src_lang].format(lang_name=LOCALE_NAME[job["lang"]]),
...
def emit_todo(out_path: Path, *, languages: tuple[str, ...] | None, skip_cached: bool) -> int:
    ...
    print(f"[02e/be] wrote {out_path} ({len(items)} items)", file=sys.stderr)
    return 0
...
def _import_one(it: dict, src_index: dict, *, translator_id: str, translator_kind: str) -> str:
    ...
    write_cache(
        lang=lang,
        slug=slug,
        source_lang=src.get("source_lang") or "nl",
        src_facts=src_facts,
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    print(f"[02e/be] {pretty}", file=sys.stderr)
    return 0
...
def _build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--lang", action="append", choices=TARGET_LOCALES, default=None,
                    help="restrict to a specific locale (repeatable)")
...
def _check_inputs() -> int:
    if not TERROIR_FACTS.exists():
        print("error: raw/terroir-facts is missing — run "
              "scripts/be/02d_extract_terroir_facts.py first", file=sys.stderr)
        return 1
    return 0


def _dispatch_emit_or_import(args, languages: tuple[str, ...] | None) -> int | None:
...
def _run_translation_loop(
    ...
    if workers <= 1:
        for job in tqdm(jobs, desc="translate-be", leave=False):
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in tqdm(as_completed(futures), total=len(jobs), desc="translate-be", leave=False):
            job = futures[fut]
            try:
                ok, err = fut.result()
            except Exception as e:  # noqa: BLE001
...
def _run_batch(args, languages: tuple[str, ...] | None) -> int:
    ...
    if not jobs:
        print("[02e/be] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02e/be] batch: {len(jobs)} translations (provider={args.provider}, "
          f"model={model_id})", file=sys.stderr)
    batch.run_two_pass(
        provider=args.provider, model=model_id,
        sidecar=ROOT / "raw" / ".batch" / "02e-be.json",
...
def main() -> int:
    ...
    languages = tuple(args.lang) if args.lang else None
    ...
    if args.limit:
        jobs = jobs[: args.limit]
    if not jobs:
        print("[02e/be] nothing to do — all caches up to date.", file=sys.stderr)
        return 0
    ...
    if provider is None:
        print(f"[02e/be] manual provider: {len(jobs)} entries need translation. "
              "Use --emit-todo PATH, fill in translated_bullets, then --import PATH "
              "--translator-id <id>.", file=sys.stderr)
        return 1

    workers = max(1, args.workers)
    print(f"[02e/be] {len(jobs)} translations to fetch "
    ...
    print(f"[02e/be] translated: {done}, skipped: {len(skipped)}", file=sys.stderr)
    if skipped:
