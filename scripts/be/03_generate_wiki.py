"""Generate one wiki/<slug>.md per BE wine record + extend
wiki/_index.json with BE entries.

Pipeline stage 03 (be). Per-record source_lang drives the section labels
used in the markdown — Dutch labels for Flemish wines + Maasvallei,
French labels for Walloon wines.
"""
...
SECTION_LABELS: dict[str, dict[str, str]] = {
    ...
    "fr": {
        "summary": "Résumé",
        "geo": "Zone géographique délimitée",
        "grapes": "Cépages",
...
STUB_NOTICE: dict[str, str] = {
    "nl": (
        "> ⚠️ **Enig document niet beschikbaar.** "
        "Reden: `{reason}`. Naam blijft vermeld; de inhoud wordt aangevuld "
        "zodra een openbare bron van het productdossier beschikbaar is."
    ),
    "fr": (
        "> ⚠️ **Document unique non disponible.** "
        "Motif : `{reason}`. Le nom reste référencé ; le contenu sera "
        "complété quand une source publique du cahier des charges sera disponible."
    ),
}


SOURCE_ATTRIBUTION: dict[str, str] = {
    "nl": "_Tekst van het enig document : © Europese Unie / EUR-Lex. Hergebruik met bronvermelding._",
    "fr": "_Texte du document unique : © Union européenne / EUR-Lex. Réutilisation avec mention de la source._",
}


EU_OJ_LABEL: dict[str, str] = {
    "nl": "EUR-Lex (ENIG DOCUMENT)",
    "fr": "EUR-Lex (DOCUMENT UNIQUE)",
}
...
def _render_note_section(slug: str, labels: dict[str, str]) -> list[str]:
...
def render_record(record: dict) -> str:
    ...
    if record.get("stub"):
        body += [
            STUB_NOTICE[lang].format(reason=record.get("stub_reason") or "unknown"),
            "",
        ]
        body += _render_note_section(slug, labels)
        body += [
            f"## {labels['sources']}",
            "",
    ...
    body += [
        f"## {labels['sources']}",
        "",
        f"- {EU_OJ_LABEL[lang]}: <{src.get('final_url') or src.get('source_url') or ''}>",
...
def index_entry(record: dict) -> dict:
    return {
        "country": "be",
        "source_lang": record.get("source_lang") or "nl",
...
def main() -> int:
    ...
    written = 0
    be_index: dict[str, dict] = {}
    for f in tqdm(files, desc="be-wiki", leave=False):
        rec = json.loads(f.read_text(encoding="utf-8"))
        out_path = WIKI / f"{rec['slug']}.md"
        out_path.write_text(render_record(rec), encoding="utf-8")
        be_index[rec["slug"]] = index_entry(rec)
        written += 1
    ...
    if WIKI_INDEX.exists():
        try:
            ...
            existing = {}
    other_kept = {k: v for k, v in existing.items() if v.get("country") != "be"}
    merged = {**other_kept, **be_index}
    WIKI_INDEX.write_text(json.dumps(merged, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
    print(f"[be/03] wrote {written} BE wiki pages, merged index "
          f"({len(other_kept)} non-BE + {len(be_index)} BE = {len(merged)} entries) "
