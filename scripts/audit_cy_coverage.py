...
fetched. Mirrors `audit_gr_coverage.py` for the Cyprusn corpus.

Run: `.venv/bin/python scripts/audit_gr_coverage.py`
"""
...
EAMBROSIA = ROOT / "raw" / "cy" / "eambrosia" / "index.json"
EXTRACTED = ROOT / "raw" / "cy" / "dokumenti-extracted"
NATIONAL_SPECS = ROOT / "raw" / "cy" / "national-specs-extracted"
OJ_MANIFEST = ROOT / "raw" / "cy" / "oj-pages" / "manifest.json"
OVERRIDES = ROOT / "raw" / "cy" / "oj-pages" / "manual_overrides.json"
...
def _load_figshare_file_numbers() -> set[str]:
    ...
    try:
        ...
        return set(
            ...
            if fn and (fn.startswith("PDO-CY") or fn.startswith("PGI-CY"))
        )
    except ImportError:
        return set()
...
def main() -> int:
    ...
    print(f"Figshare CY PDOids:   {len(figshare_ids)}")
    print()
    ...
    for w in wines:
        ...
        if not n_grapes:
            # All CY wines are stubs; grapes live in the national-spec
            # sidecar (merged in-memory at stage 04). Read it directly.
            sc = NATIONAL_SPECS / f"{slug}.json"
            if sc.exists():
                try:
                    n_grapes = json.loads(sc.read_text(encoding="utf-8")).get("n_grapes", 0)
                except (ValueError, OSError):
                    n_grapes = 0
        if n_grapes:
            n_with_grapes += 1
    ...
    print(f"  Missing from Figshare: {no_figshare} "
          f"(expected 4 = the 4 district PGIs, resolved via GISCO district-union)")
    print()
    ...
    print()
    print("## Region distribution (4 wine districts / επαρχίες)")
    ...
    if not curator_targets:
        print("  (empty — every Cyprusn wine extracted)")
    print()
    ...
    try:
        ...
        _vb_count, _vb_records = _verbatim.count_verbatim_records(
            Path(__file__).resolve().parent.parent / "raw" / "terroir-facts", "cy",
        )
        if _vb_count:
