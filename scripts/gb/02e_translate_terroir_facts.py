"""Translate GB per-wine terroir-fact bullets into the missing locales.

GB analog of `scripts/mt/02e_translate_terroir_facts.py`. The UK's source
language is always English (the DEFRA product specifications are written
in English, and EN is the canonical rendered surface), so the bullets are
already in EN — this stage only produces fr / es / nl
(`{en, fr, es, nl} - {en}`).

Reads:  raw/terroir-facts/<slug>.json  (where country == 'gb')
...
SYSTEM_PROMPT_TEMPLATE = """You translate short {source_lang_name} bullets describing a United Kingdom wine appellation's terroir, history, and wine character into {lang_name}. The bullets come from the appellation's DEFRA product specification + English Wikipedia context, aimed at sommelier students and wine enthusiasts.
...
def _load_gb_source(path: Path) -> dict | None:
...
def emit_todo(out_path: Path, *, languages: tuple[str, ...], skip_cached: bool) -> int:
    ...
    print(f"[02e/gb] wrote {out_path} ({len(items)} items: {pretty})", file=sys.stderr)
    return 0
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    print(f"[02e/gb] {pretty}", file=sys.stderr)
    return 0
...
def _run_translation_loop(provider, model_id: str, jobs: list[dict], workers: int = 1):
    ...
    if workers <= 1:
        for job in tqdm(jobs, desc="translate-gb", leave=False):
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in tqdm(as_completed(futures), total=len(jobs), desc="translate-gb", leave=False):
            job = futures[fut]
            try:
                ok, err = fut.result()
            except Exception as e:  # noqa: BLE001
...
def _run_batch(args, languages: tuple[str, ...]) -> int:
    ...
    if not jobs:
        print("[02e/gb] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02e/gb] batch: {len(jobs)} translations (provider={args.provider}, "
          f"model={model_id}, locales={','.join(languages)})", file=sys.stderr)
    batch.run_two_pass(
        provider=args.provider, model=model_id,
        sidecar=ROOT / "raw" / ".batch" / "02e-gb.json",
...
def main() -> int:
    ...
    if not TERROIR_FACTS.exists():
        print("error: raw/terroir-facts is missing — run "
              "scripts/gb/02d_extract_terroir_facts.py first", file=sys.stderr)
        return 1
    ...
    if args.limit:
        jobs = jobs[: args.limit]
    if not jobs:
        print("[02e/gb] nothing to do — all caches up to date.", file=sys.stderr)
        return 0
    ...
    if provider is None:
        print(f"[02e/gb] manual provider: {len(jobs)} entries need translation.",
              file=sys.stderr)
        return 1

    workers = max(1, args.workers)
    print(f"[02e/gb] {len(jobs)} translations to fetch "
    ...
    print(f"[02e/gb] translated: {done}, skipped: {len(skipped)}", file=sys.stderr)
    if skipped:
