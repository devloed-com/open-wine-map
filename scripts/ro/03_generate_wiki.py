"""Generate one wiki/<slug>.md per RO wine record + extend
wiki/_index.json with RO entries.

Pipeline stage 03 (ro). Mirrors `scripts/hr/03_generate_wiki.py` for the
RO corpus. Reads `raw/ro/dokumente-extracted/*.json`, emits per-record
markdown pages with Romanian section headings, and merges RO entries
into `wiki/_index.json` (preserving any pre-existing FR/ES/PT/IT/AT/SI/
HR/HU entries).

A curated cross-border note is rendered as a `## Notă` section when
...
SECTION_LABELS = {
    "summary": "Rezumat",
    "geo": "Aria geografică delimitată",
    "grapes": "Soiuri de struguri",
    "link": "Descrierea legăturii",
    "note": "Notă",
    "sources": "Surse",
}
...
def _render_note_section(slug: str) -> list[str]:
    """Render the curated cross-border note as a `## Notă` block, with
...
def render_record(record: dict) -> str:
    ...
    if record.get("stub"):
        body += [
            "> ⚠️ **Documentul unic nu este disponibil.** "
            f"Motiv: `{record.get('stub_reason') or 'unknown'}`. "
            "Denumirea rămâne în index; conținutul va fi completat când "
            "va fi disponibilă o sursă publică a caietului de sarcini.",
            "",
        ]
    ...
    body += [
        f"## {SECTION_LABELS['sources']}",
        "",
        f"- EUR-Lex (DOCUMENT UNIC): <{src.get('final_url') or src.get('source_url') or ''}>",
        ...
        f"- File number: `{record.get('file_number') or ''}`",
        "",
        "_Textul documentului unic: © Uniunea Europeană / EUR-Lex. "
        "Reutilizare cu menționarea sursei._",
        "",
    ]
    return "\n".join(fm + body)
...
def main() -> int:
    if not EXTRACTED.exists():
        print(f"error: {EXTRACTED} missing — run scripts/ro/02_extract_pliegos.py first",
              file=sys.stderr)
        return 1
    ...
    written = 0
    ro_index: dict[str, dict] = {}
    for f in tqdm(files, desc="ro-wiki", leave=False):
        rec = json.loads(f.read_text(encoding="utf-8"))
        out_path = WIKI / f"{rec['slug']}.md"
        out_path.write_text(render_record(rec), encoding="utf-8")
        ro_index[rec["slug"]] = index_entry(rec)
        written += 1
    ...
    if WIKI_INDEX.exists():
        try:
            ...
            existing = {}
    other_kept = {k: v for k, v in existing.items() if v.get("country") != "ro"}
    merged = {**other_kept, **ro_index}
    WIKI_INDEX.write_text(json.dumps(merged, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
    print(
        f"[ro/03] wrote {written} RO wiki pages, merged index "
        f"({len(other_kept)} non-RO + {len(ro_index)} RO = {len(merged)} entries) "
