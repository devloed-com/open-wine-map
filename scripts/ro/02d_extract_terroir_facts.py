"""Extract noteworthy terroir facts from each RO wine's "Descrierea
legăturii" (DOCUMENT UNIC link section) using a bounded LLM layer with
dual-source grounding.

RO analog of `scripts/hr/02d_extract_terroir_facts.py`. Same per-bullet
provenance, fuzzy-grounding, and cache-invalidation rules; the
differences are:

  - source = RO record's `link_to_terroir` (DOCUMENT UNIC link section,
    straight from EUR-Lex — Romania has no national-spec fallback
    layer wired in v1).
  - Wikipedia hint = `raw/wikipedia/aocs/ro/<slug>.json` (fetched by
    `scripts/02b_fetch_aoc_lexicon.py --lang ro --source raw/ro/dokumente-extracted`).
  - LLM prompt is in Romanian and asks for Romanian bullets.
  - Output cache = `raw/terroir-facts/<slug>.json` (single shared dir;
    `country: "ro"` distinguishes from FR/ES/PT/IT/AT/SI/HR records).

In v1 ~34 of 54 wines carry a non-stub DOCUMENT UNIC, so this stage
extracts facts for those records; the other ~20 are content-stubs.

Providers: anthropic / mistral / ollama / manual (mirrors HR 02d).
"""
...
# Same 4-bucket structure as FR/ES/PT/IT/AT/SI/HR 02d, with Romanian
# topic vocabulary matched to the DOCUMENT UNIC "Descrierea legăturii"
# content pattern.
SUBSECTIONS = [
    {
        "key": "facteurs_naturels",  # FR key kept for stage-04 render parity
        "label": "Factori naturali (geologie, soluri, climă, relief)",
        "topics": (
            "geologie, alcătuirea solului, climă, relief, ape, "
            "expunere, situri și localități"
        ),
        "max_bullets": 5,
    },
    {
        "key": "facteurs_humains",
        "label": "Factori istorici și umani (istorie, practici)",
        "topics": (
            "istoria denumirii de origine, practici viticole și oenologice, "
            "soiuri tradiționale de struguri"
        ),
        "max_bullets": 2,
    },
    {
        "key": "produit",
        "label": "Caracteristicile produsului (profil senzorial)",
        "topics": "culori, arome, structură, maturare, profil senzorial al vinurilor",
        "max_bullets": 2,
    },
    {
        "key": "interactions",
        "label": "Relații cauzale (legătura terroir / vin)",
        "topics": (
            "legătura explicită între terroir și caracterul vinurilor, "
            "exprimarea solului sau a climei în vin"
        ),
        "max_bullets": 1,
    },
]
...
# Romanian Wikipedia section names that map to each DOCUMENT UNIC sub-
# section as a salience hint. Romanian wine-region articles vary in
# section naming; this list is permissive.
WIKI_TO_SUBSECTION: dict[str, list[str]] = {
    "facteurs_naturels": [
        "Geografie", "Așezare", "Geologie", "Sol", "Soluri", "Climă",
        "Relief", "Podgorie", "Viticultură", "Peisaj", "Vii",
    ],
    "facteurs_humains": [
        "Istoric", "Istorie", "Etimologie", "Denumire", "Soiuri",
        "Soiuri de struguri", "Viticultură", "Vinificație",
        "Tradiție", "Cultura vinului",
    ],
    "produit": [
        "Vinuri", "Vin", "Tipuri de vin", "Caracteristici", "Profil senzorial",
    ],
    "interactions": [],
}
...
EXTRACT_SYSTEM = """Extrage caracteristici demne de remarcat ale unei denumiri de origine vinicole românești din DOUĂ surse: documentul unic / caietul de sarcini (autoritate de reglementare) și extrasul din Wikipedia română (referință pentru publicul larg, vocabular de somelier).

Cititorul țintă: pasionați de vin sau somelieri cu experiență, interesați de specificitățile concrete ale originii. Demne de menționat sunt: formațiuni geologice cu denumirea lor tehnică, tipuri specifice de sol (cernoziom, brun-roșcat, sol carbonatic, sol scheletic, loess, marnă, calcar, gresie), climă locală și vânturi (climă continentală, climă temperat-continentală, influență mediteraneană, crivăț, austru, băltăreț, climă pontică), soiuri tradiționale de struguri, practici viticole caracteristice, profil senzorial precis, momente istorice datate.

═══ SURSA 1: extrasul Wikipedia (secțiunile relevante) ═══
{wiki_hint}

═══ SURSA 2: textul documentului unic (în mesajul utilizatorului de mai jos) ═══

Subsecțiunea de procesat: {label}
Categorii prioritare: {topics}

Reguli stricte:
- Fiecare element TREBUIE să fie susținut DE CEL PUȚIN UN citat: `cahier_quote` (literal din documentul unic) SAU `wiki_quote` (literal din extrasul Wikipedia de mai sus). Ambele sunt foarte recomandate când ambele surse acoperă aceeași informație.
- Acordă prioritate documentului unic ca sursă primară: folosește `cahier_quote` ori de câte ori faptul apare acolo.
- Adaugă `wiki_quote` când Wikipedia oferă vocabularul tehnic standard pentru un fapt pe care documentul unic îl descrie cu propriile cuvinte.
- Dacă un fapt notabil este menționat numai de Wikipedia, îl poți păstra cu `wiki_quote` singur — la afișare va fi atribuit Wikipediei.
- Citatele sunt LITERALE (copiate) din sursa corespunzătoare. NICIODATĂ nu atribui unei surse un text care nu apare acolo.
- Fără judecăți de valoare ("excepțional", "prestigios" …).
- Fără concluzii externe. Fără cifre care nu apar în niciuna dintre cele două surse.
- Maximum {max_bullets} elemente; fiecare element este o propoziție completă de aproximativ 120–220 de caractere — niciodată un fragment telegrafic.
- Dacă nici documentul unic, nici Wikipedia nu conțin un fapt concret demn de menționat pentru această subsecțiune, returnează o listă goală.

Răspunde DOAR în JSON, fără text înainte sau după:
{{"facts": [{{"bullet": "...", "cahier_quote": "...", "wiki_quote": "..."}}, ...]}}
Pentru un citat care lipsește, folosește un șir gol ""."""
...
def _wiki_hint_for_subsection(
    ...
    """Pull the relevant Wikipedia RO section text for a sub-section.
...
def collect_targets() -> list[dict]:
    """Return RO records with a long-enough link_to_terroir. Romania has
...
def _ask_line(label: str) -> str:
    return f"Subsecțiunea de procesat: {label}"


def _document_block(lien_text: str) -> str:
    return f"Textul documentului unic (Descrierea legăturii):\n\n{lien_text}"
...
def _process_subsection(
    ...
    system = EXTRACT_SYSTEM.format(
        wiki_hint=wiki_hint or "(extras Wikipedia indisponibil)",
        label=sub["label"],
        topics=sub["topics"],
...
def _process_record(provider, model_id: str, record: dict) -> dict:
    ...
    earned = earn_interactions(all_facts, "ro")
...
def emit_todo(out_path: Path, *, skip_cached: bool, limit: int = 0) -> int:
    ...
    print(
        f"[02d/ro] wrote {out_path} ({len(items)} items across "
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    for slug, slug_items in by_slug.items():
        ...
        counts[outcome] += 1
    print(
        f"[02d/ro] wrote {counts['wrote']} cache files; "
...
def _run_batch(args) -> int:
    """Extract terroir facts for every RO wine via the provider Batch API."""
    ...
    if not targets:
        print("[02d/ro] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02d/ro] batch: {len(targets)} wines (provider={args.provider}, "
    ...
    def run_loop(prov):
        ...
        if getattr(prov, "kind", "") == "collecting":
            keep = CACHE_DIR
            CACHE_DIR = Path(tempfile.mkdtemp(prefix="batch-02d-ro-"))
            try:
                for rec in targets:
    ...
    batch.run_two_pass(
        ...
        sidecar=ROOT / "raw" / ".batch" / "02d-ro.json",
...
def main() -> int:
    ...
    if not EXTRACTED.exists():
        print(
            f"error: {EXTRACTED} missing — run scripts/ro/02_extract_pliegos.py first",
            file=sys.stderr,
        )
        return 1
    if not WIKI_AOCS.exists():
        print(
            ...
            "Run scripts/02b_fetch_aoc_lexicon.py --lang ro "
            "--source raw/ro/dokumente-extracted to enable Wikipedia salience.",
            file=sys.stderr,
        )
    ...
    terroir_verbatim.emit_for_country(
        country="ro", extracted_dir=EXTRACTED, cache_dir=CACHE_DIR,
        default_source_lang="ro", cahier_source_kind="eu-oj",
        only=args.only, log_prefix="[02d/ro]",
    )
    ...
    if not targets:
        print("[02d/ro] nothing to do — all caches valid.", file=sys.stderr)
        return 0
    ...
    if provider is None:
        print(
            f"[02d/ro] manual provider: {len(targets)} wines need extraction. "
    ...
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    print(
        f"[02d/ro] {len(targets)} RO wines to extract "
    ...
    t0 = time.time()
    if workers <= 1:
        for rec in tqdm(targets, desc="terroir-ro", leave=False):
        ...
        with ThreadPoolExecutor(max_workers=workers) as ex:
            ...
            for fut in tqdm(as_completed(futures), total=len(targets), desc="terroir-ro", leave=False):
                try:
                    res = fut.result()
                    n_done += 1
                    n_facts += len(res.get("facts", []))
    ...
    print(
        f"[02d/ro] done: {n_done} wines, {n_facts} facts, "
