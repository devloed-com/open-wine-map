"""Translate RO per-AOC terroir-fact bullets (RO → en/fr/es/nl).

RO analog of `scripts/hr/02e_translate_terroir_facts.py`. Same bounded-
...
is Romanian (the DOCUMENT UNIC is canonical for RO wines), so the
locale set is en/fr/es/nl.

Reads:  raw/terroir-facts/<slug>.json  (where country == 'ro')
...
SYSTEM_PROMPT = """You translate short Romanian bullets describing a Romanian wine appellation's terroir, history, and wine character into {lang_name}. The bullets come from the EU "document unic" / caiet de sarcini and are aimed at sommelier students and wine enthusiasts.
...
def _load_ro_source(path: Path) -> dict | None:
    """Return RO terroir-facts payload for `path` if it has facts; else None."""
...
def emit_todo(out_path: Path, *, languages: tuple[str, ...], skip_cached: bool) -> int:
    ...
    print(f"[02e/ro] wrote {out_path} ({len(items)} items: {pretty})", file=sys.stderr)
    return 0
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    print(f"[02e/ro] {pretty}", file=sys.stderr)
    return 0
...
def _check_inputs() -> int:
    if not TERROIR_FACTS.exists():
        print(
            ...
            "scripts/ro/02d_extract_terroir_facts.py first",
            file=sys.stderr,
        )
        return 1
    return 0
...
def _run_translation_loop(
    ...
    if workers <= 1:
        for job in tqdm(jobs, desc="translate-ro", leave=False):
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in tqdm(as_completed(futures), total=len(jobs), desc="translate-ro", leave=False):
            job = futures[fut]
            try:
                ok, err = fut.result()
            except Exception as e:  # noqa: BLE001
...
def _run_batch(args, languages: tuple[str, ...]) -> int:
    ...
    if not jobs:
        print("[02e/ro] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02e/ro] batch: {len(jobs)} translations (provider={args.provider}, "
          f"model={model_id}, locales={','.join(languages)})", file=sys.stderr)
    batch.run_two_pass(
        provider=args.provider, model=model_id,
        sidecar=ROOT / "raw" / ".batch" / "02e-ro.json",
...
def main() -> int:
    ...
    if not jobs:
        print("[02e/ro] nothing to do — all caches up to date.", file=sys.stderr)
        return 0
    ...
    if provider is None:
        ...
        print(
            f"[02e/ro] manual provider: {len(jobs)} entries need translation. "
    ...
    print(
        f"[02e/ro] {len(jobs)} translations to fetch "
    ...
    print(f"[02e/ro] translated: {done}, skipped: {len(skipped)}", file=sys.stderr)
    if skipped:
        for lang, slug, err in skipped[:20]:
            print(f"  skip {lang}/{slug}: {err[:160]}", file=sys.stderr)
        print("[02e/ro] rerun this stage to retry the skipped entries.", file=sys.stderr)
    return 0
