"""Extract structured fields from each cached EUR-Lex Document-unic HTML.

Pipeline stage 02 (ro).

For each Romanian wine GI in `raw/ro/eambrosia/index.json`:
  - if a cached HTML exists at `raw/ro/oj-pages/<slug>.html`, parse the
    "DOCUMENT UNIC" block into numbered sections (the EU wine GI
    template) and route them by Romanian title keyword
  - else emit a stub record so the wine remains searchable

In addition to the standard role-routed sections, the RO extractor
also harvests the **commune list** from the section-6 area body
(`geo_communes`) so stage 04's `ROPolygonIndex.commune_union` can
resolve IGP geometry (and the 3 newer PDOs missing from Bétard 2022)
against the shared GISCO LAU index. This is the same shape as the ES
IGP-fallback chain.

Output: one JSON per wine under `raw/ro/dokumente-extracted/<slug>.json`,
plus a `_index.json` mapping slug → metadata.

v1 models the 54 Romanian wine GIs as a flat corpus — the regulatory
hierarchy (8 regiuni viticole) is preserved via the `region` facet
...
from _lib.ro.region import derive_region  # noqa: E402
...
INDEX_IN = ROOT / "raw" / "ro" / "eambrosia" / "index.json"
OJ_DIR = ROOT / "raw" / "ro" / "oj-pages"
OJ_MANIFEST = OJ_DIR / "manifest.json"
OUT_DIR = ROOT / "raw" / "ro" / "dokumente-extracted"
...
# Romanian grape-variety sections enumerate one variety per line, with
# em-dash bullets or newlines as separators; the canonical Romanian
# name is the segment before a `-` synonym separator (e.g.
# `Fetească Albă - Mädchentraube` → `Fetească Albă`).
_BULLET_SPLIT_RE = re.compile(r"\s*[—•·]\s*|\n")
_NAME_SYN_SPLIT_RE = re.compile(r"\s+[-–]\s+")

# Some RO source HTML concatenates multiple "Name COLOUR Name COLOUR ..."
# entries without bullets — Însurătei's section 7 puts all 12 varieties on
# one line. Split on a colour-letter (B / N / G / Rs / Rg) immediately
# followed by a capitalised variety name.
_CONCAT_SPLIT_RE = re.compile(r"(?<=\s[BNG])\s+(?=[A-ZĂÎÂȘȚ])|(?<=\s(?:Rs|Rg))\s+(?=[A-ZĂÎÂȘȚ])")

_GRAPE_LINE_DROP = (
    "soiul/soiurile principale de struguri",
    "soiul (soiurile) principal(e)",
    "soiurile principale", "soiul principal",
    "soiuri de struguri", "soiul de struguri",
)
...
def _grape_items(section_text: str) -> list[str]:
    ...
    for chunk in _BULLET_SPLIT_RE.split(section_text or ""):
        ...
        if not chunk:
            continue
        for sub in _CONCAT_SPLIT_RE.split(chunk):
            sub = sub.strip(" \t;,.-")
            if sub:
                items.append(sub)
    return items
...
def _item_candidates(item: str) -> list[str]:
    ...
    its synonyms (the segment(s) after a plain hyphen). Some RO docs (e.g.
    the Lechința bulletin) use commas — not ` - ` — to separate synonyms;
    when the head carries commas and no hyphen separator is present, try
    the comma-segments rather than the whole head (which would never match
    and would just clutter `extraction-unknowns.json`)."""
    parts = _NAME_SYN_SPLIT_RE.split(item, maxsplit=1)
    head = parts[0]
    syn_blob = parts[1] if len(parts) > 1 else ""
    head_pieces = (
        [p.strip() for p in head.split(",")]
        if "," in head and not syn_blob
        else [head]
    )
    out: list[str] = []
    for c in [*head_pieces, *syn_blob.split(",")]:
...
def parse_grapes(section_text: str) -> dict:
    ...
    observation, details}. Romanian DOCUMENT UNIC publications rarely
    carry a principal/accessory split — we default to `principal` for
    every match. Unmatched names flow into
    `raw/ro/extraction-unknowns.json`."""
    out: dict[str, list] = {
        "principal": [],
        "accessory": [],
        "observation": [],
...
def parse_styles(sections: dict[str, str], titles: dict[str, str]) -> list[str]:
    ...
    sections. Romanian colour adjectives + sparkling / late-harvest /
    dessert markers."""
    blob_parts = []
    for num, body in sections.items():
        title_low = titles.get(num, "").lower()
        if (
            "descrierea vinurilor" in title_low
            or "descrierea vinului" in title_low
            or "categoriile" in title_low
            or "categorii" in title_low
            or "alte condi" in title_low
        ):
...
def build_stub(wine: dict, oj_meta: dict, reason: str) -> dict:
    return {
        ...
        "kind": wine["kind"],
        "is_sub_denomination": False,
        # The curated file_number → region map covers (incrementally)
        # every wine; a content-stub keeps its region facet — empty
        # string is acceptable when curation hasn't reached this slug.
...
def _extract_from_html(cache: Path) -> tuple[dict[str, str], dict[str, str], str]:
    ...
    if doc is None:
        return {}, {}, "no-document-unic-anchor"
...
def main() -> int:
    ...
    if not INDEX_IN.exists():
        print(f"error: {INDEX_IN} missing — run scripts/ro/00_fetch_data.py first",
              file=sys.stderr)
        return 1
    ...
    unknowns_path = ROOT / "raw" / "ro" / "extraction-unknowns.json"
