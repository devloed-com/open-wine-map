"""Regenerate the manual_overrides.json template for RO wines whose
single-document URL is not auto-discovered / not fetchable.

Reads:
  raw/ro/eambrosia/index.json — full RO wine list
  raw/ro/oj-pages/manifest.json — stage 01/01b fetch outcomes per wine

Writes:
  raw/ro/oj-pages/manual_overrides.json — one entry per wine that needs
...
20 of the 54 Romanian wines are Art.107 / Reg.1308/2013 grandfathered
...
(EUR-Lex OJ-C page, or the ONVPV / *Monitorul Oficial* national
caiet de sarcini once a parser branch exists). After filling URLs,
re-run scripts/ro/01_fetch_pliegos.py → 02_extract_pliegos.py → stage 04.
"""
...
EAMBROSIA_INDEX = ROOT / "raw" / "ro" / "eambrosia" / "index.json"
OJ_MANIFEST = ROOT / "raw" / "ro" / "oj-pages" / "manifest.json"
OVERRIDES_PATH = ROOT / "raw" / "ro" / "oj-pages" / "manual_overrides.json"


def main() -> int:
    if not EAMBROSIA_INDEX.exists():
        print(f"error: {EAMBROSIA_INDEX} missing — run scripts/ro/00_fetch_data.py first",
              file=sys.stderr)
        return 1
    ...
    out["__doc__"] = (
        "Curator-filled overrides for RO wines whose single-document "
        "URL is not auto-discovered or not fetchable. Fill the `url` "
        "field for each entry to point at a public, licence-clear "
        "specification (EUR-Lex OJ-C page, or the ONVPV / Monitorul "
        "Oficial national caiet de sarcini). Stage 01 reads this file "
        "before falling back to eAmbrosia publications. Re-run "
        "scripts/ro/01_fetch_pliegos.py after editing. Entries with "
