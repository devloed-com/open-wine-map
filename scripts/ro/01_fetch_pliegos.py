"""Fetch the EU-OJ "Document unic" HTML page per RO wine GI.

Pipeline stage 01 (ro).

For each wine in `raw/ro/eambrosia/index.json`, scan the `publications`
array for the first HTTP URL and request the Romanian-language variant
of that EUR-Lex page. The page contains the canonical EU single document
("DOCUMENT UNIC") — the Romanian equivalent of the FR cahier des charges,
the ES pliego, the IT documento unico, the AT Einziges Dokument,
the SI Enotni dokument, the HR Jedinstveni dokument, and the
HU Egységes dokumentum.

Coverage:
  34 of the 54 RO wines carry a fetchable OJ publication URL — the
  other 20 are Art.107 / Reg.1308/2013 grandfathered names with no
  public single-document URL. They land as `no-publication` and
  stage 02 emits content-stubs (the IT/ES/SI/HR/HU curator-queue
  pattern). The other failure mode is the EUR-Lex AWS WAF
  (HTTP 202 + JavaScript challenge); `scripts/ro/01b_solve_waf.py`
  bootstraps the blocked subset via headless Chromium.

Romanian variant rules (same as ES/IT/AT/SI/HR/HU, with the language swap):
  /eli/.../oj  →  /eli/.../oj/ron  (3-letter ISO 639-2/T for Romanian)
  legal-content/EN/TXT/  →  legal-content/RO/TXT/HTML/
  uri=...01.ENG&...     →  uri=...01.RON&...

Manual overrides:
  raw/ro/oj-pages/manual_overrides.json (gitignored, optional) keyed by
  giIdentifier or slug → {"url": "<romanian-html-url>", "note": "..."}.
  Overrides bypass the eAmbrosia publications list entirely.

Outputs:
  raw/ro/oj-pages/<slug>.html          (the Romanian Document-unic HTML)
  raw/ro/oj-pages/manifest.json        (per-slug status + final URL)
...
INDEX_PATH = ROOT / "raw" / "ro" / "eambrosia" / "index.json"
OUT_DIR = ROOT / "raw" / "ro" / "oj-pages"
...
def to_romanian(url: str) -> str:
    """Best-effort URL rewrite: any EUR-Lex / EU-OJ URL → its Romanian
    HTML variant. Idempotent — already-Romanian URLs pass through."""
    if re.search(r"/oj/?(?:[#?]|$)", url) and not re.search(r"/oj/[a-z]{3}", url):
        return re.sub(r"/oj(/?)([#?]|$)", r"/oj/ron\2", url)
    new = re.sub(
        r"/legal-content/[A-Z]{2}/TXT(?:/HTML)?/",
        "/legal-content/RO/TXT/HTML/",
        url,
    )
    new = re.sub(r"(\d{2}\.)(?:ENG|FRA|DEU|ITA|POR|SPA|NLD|SLV|HRV|HUN|RON)(?=&|$)", r"\1RON", new)
    new = re.sub(r"(:)(?:ENG|FRA|DEU|ITA|POR|SPA|NLD|SLV|HRV|HUN|RON)(?=&|$)", r"\1RON", new)
    return new
...
def _is_corrigendum(pub: dict) -> bool:
    """A Corrigendum / Rectificare publication is a short correction
    notice, not the full single document — deprioritise it."""
    text = (pub.get("text") or "").lower()
    return "corrigendum" in text or "rectificare" in text
...
def fetch_page(session: requests.Session, url: str) -> requests.Response | None:
    """Fetch with 202-aware retry. EUR-Lex generates `legal-content/RO/TXT/HTML`
...
def looks_like_single_document(html: str) -> bool:
    ...
    publication. The Document-unic HTML is large (>8 KB) and contains
    literal Romanian phrases."""
    if len(html) < 8000:
        return False
    low = html.lower()
    return (
        "document unic" in low
        or "documentul unic" in low
        or "caiet de sarcini" in low
        or "indicație geografică" in low
        or "indicatie geografica" in low
        or "denumire de origine" in low
    )
...
def main() -> int:
    ...
    if not INDEX_PATH.exists():
        print(f"error: {INDEX_PATH} missing — run scripts/ro/00_fetch_data.py first",
              file=sys.stderr)
        return 1
    ...
    for w in tqdm(wines, desc="oj-pages", leave=False):
        ...
        if override and override.get("url"):
            ...
            source_url = to_romanian(base) if base else None
