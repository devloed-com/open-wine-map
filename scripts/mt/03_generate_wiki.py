"""Generate one wiki/<slug>.md per MT wine record + extend
wiki/_index.json with MT entries.

Pipeline stage 03 (mt). Mirrors `scripts/cz/03_generate_wiki.py` for the
Maltese corpus. Reads `raw/mt/dokumente-extracted/*.json`, emits
per-record markdown pages (English section headings — Malta's source
language is English), and merges MT entries into `wiki/_index.json`
...
SECTION_LABELS = {
    "summary": "Summary",
    "geo": "Demarcated geographical area",
...
def render_record(record: dict) -> str:
    ...
    if record.get("stub"):
        body += [
            "> ⚠️ **No single document available.** "
            f"Reason: `{record.get('stub_reason') or 'unknown'}`. "
            "The name stays in the register; content will be filled in once a "
            "public product-specification source is available.",
            "",
        ]
    ...
    body += [
        f"## {SECTION_LABELS['sources']}",
        "",
        f"- EUR-Lex (SINGLE DOCUMENT): <{src.get('final_url') or src.get('source_url') or ''}>",
        ...
        f"- File number: `{record.get('file_number') or ''}`",
        "",
        "_Single-document text: © European Union / EUR-Lex. "
        "Reuse with attribution._",
        "",
    ]
    return "\n".join(fm + body)
...
def main() -> int:
    ...
    written = 0
    mt_index: dict[str, dict] = {}
    for f in tqdm(files, desc="mt-wiki", leave=False):
        rec = json.loads(f.read_text(encoding="utf-8"))
        out_path = WIKI / f"{rec['slug']}.md"
        out_path.write_text(render_record(rec), encoding="utf-8")
        mt_index[rec["slug"]] = index_entry(rec)
        written += 1
    ...
    if WIKI_INDEX.exists():
        try:
            ...
            existing = {}
    other_kept = {k: v for k, v in existing.items() if v.get("country") != "mt"}
    merged = {**other_kept, **mt_index}
    WIKI_INDEX.write_text(json.dumps(merged, ensure_ascii=False, indent=2, sort_keys=True),
                          encoding="utf-8")
    print(
        f"[mt/03] wrote {written} MT wiki pages, merged index "
        f"({len(other_kept)} non-MT + {len(mt_index)} MT = {len(merged)} entries) "
