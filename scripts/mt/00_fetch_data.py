"""Fetch the public reference datasets the Malta pipeline depends on.

Pipeline stage 00 (mt).

Sources:

1. **eAmbrosia EU register** (spine — list of MT wine GIs)
   `https://webgate.ec.europa.eu/eambrosia-api/api/v1/geographical-indications`
   Filtered client-side to country=MT + productType=WINE + status=registered
   → 3 wine GIs (2 PDO + 1 PGI):
     - Malta            PDO-MT-A1630  (EU-OJ single document, English)
     - Gozo             PDO-MT-A1629  (EU-OJ single document, English)
     - Maltese Islands  PGI-MT-A1631  (no-publication stub)
   The endpoint returns the full register (~6.5 MB, ~4000 GIs across all
   EU); we share the response with the ES/PT/IT/AT/SI/HR/HU/RO/BG/GR/DE/
   SK/CZ/NL pipelines via separate per-country index files.

   Coverage in eAmbrosia: the 2 PDOs carry a fetchable EU-OJ "SINGLE
   DOCUMENT" URL (published in English — Malta is bilingual mt/en and the
   EU-OJ Maltese-wine documents are issued in English). The "Maltese
   Islands" PGI has no publication and ships as a content-stub; it still
   appears on the map because its geometry resolves as the union of the
   two PDO polygons.
...
2. **Figshare EU_PDO.gpkg** (Bétard et al. 2022, CC0, ~42 MB) — *already
   ...
   both MT DOPs (`PDO-MT-A1629`, `PDO-MT-A1630`). Stage 04 uses this as
   the geometry source; the PGI is the union of the two.

Outputs:
- raw/mt/eambrosia/index.json — filtered MT-wine list with derived slug + kind
- raw/mt/eambrosia/manifest.json — fetch metadata for the eAmbrosia call
"""
...
def normalise_kind(gi_type: str) -> str:
    """eAmbrosia uses `PDO`/`PGI`; the MT convention (same as ES/PT/IT/AT/
    SI/HR/HU/RO/BG/GR/DE/SK/CZ/NL) is `DOP`/`IGP`."""
...
def main() -> int:
    ...
    mt_wines_all = [
        g for g in full
        if "MT" in (g.get("countries") or []) and g.get("productType") == "WINE"
    ]
    mt_wines = [g for g in mt_wines_all if g.get("status") == "registered"]
    n_skipped_applied = len(mt_wines_all) - len(mt_wines)

    by_slug: dict[str, list[dict]] = {}
    projected = [project(rec) for rec in mt_wines]
    for p in projected:
        by_slug.setdefault(p["slug"], []).append(p)
    collisions = {s: ps for s, ps in by_slug.items() if len(ps) > 1}
    if collisions:
        print(f"[warn] {len(collisions)} slug collisions inside MT wines:", file=sys.stderr)
    ...
    print(
        f"[done] eAmbrosia: total_eu={len(full)} mt_wines={len(projected)} "
