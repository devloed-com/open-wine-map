...
`scripts/mt/01_fetch_pliegos.py` couldn't reach because EUR-Lex sits
...
already-cached. (Malta's 2 fetchable wines cleared on the first stage-01
run; this bootstrap is here for parity / re-fetch resilience.)

Run: `.venv/bin/python scripts/mt/01b_solve_waf.py`
After: re-run `scripts/mt/02_extract_pliegos.py` to pick up the new HTMLs.
"""
...
def to_english(url: str) -> str:
    ...
    url = re.sub(r"/legal-content/[A-Z]{2}/TXT(?:/HTML)?/", "/legal-content/EN/TXT/HTML/", url)
...
def _is_corrigendum(pub: dict) -> bool:
    return "corrigendum" in (pub.get("text") or "").lower()
...
def main() -> int:
    ...
    if not MANIFEST_PATH.exists():
        print(f"error: {MANIFEST_PATH} missing — run scripts/mt/01_fetch_pliegos.py first",
              file=sys.stderr)
        return 1
    ...
    for w in wines:
        ...
        if info.get("status") not in ("fetch-error", "playwright-error", "not-single-document"):
            continue
        ...
        if override and override.get("url"):
            ...
            url = to_english(base) if base else None
        if not url:
            continue
    ...
    print(f"[01b] {len(targets)} WAF-blocked wines to bootstrap via Chromium", file=sys.stderr)
    if not targets:
        return 0
    ...
    with sync_playwright() as p:
        ...
        for i, (w, url) in enumerate(targets):
            ...
            try:
                ...
                print(f"  [{i+1}/{len(targets)}] {slug}: navigate failed: {exc}", file=sys.stderr)
                by_slug[slug] = {
            ...
            n_ok += 1
            print(f"  [{i+1}/{len(targets)}] {slug}: ok ({len(html)} B)", file=sys.stderr)
            if i + 1 < len(targets):
    ...
    manifest_root["counts"] = {
        ...
        "fetch_error_or_not_single_document": (
            counts["fetch-error"] + counts["not-single-document"] + counts["playwright-error"]
        ),
        "playwright_solved": sum(1 for v in by_slug.values() if v.get("from_playwright")),
    }
