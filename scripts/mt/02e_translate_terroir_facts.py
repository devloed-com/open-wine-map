"""Translate MT per-wine terroir-fact bullets into the missing locales.

MT analog of `scripts/ch/02e_translate_terroir_facts.py`. Malta's source
language is always English (the EU single documents are English, EN is
the canonical surface), so the bullets are already in EN — this stage
only produces fr / es / nl (`{en, fr, es, nl} - {en}`).

Reads:  raw/terroir-facts/<slug>.json  (where country == 'mt')
...
SOURCE_LANG_NAME = {"en": "English"}


SYSTEM_PROMPT_TEMPLATE = """You translate short {source_lang_name} bullets describing a Maltese wine appellation's terroir, history, and wine character into {lang_name}. The bullets come from the appellation's Wikipedia article + EU single-document context, aimed at sommelier students and wine enthusiasts.
...
- Preserve Maltese proper nouns verbatim: appellation / island names (Malta, Gozo, Għawdex, Maltese Islands), the indigenous grape varieties Ġellewża (red) and Girgentina (white), other grape names (Chardonnay, Cabernet Sauvignon, Syrah, Vermentino, Moscato, …), geological / soil terms (Globigerina limestone, Coralline limestone, blue clay, terra rossa), and Maltese wine terms (Passito, Imqadded, Riżerva).
...
def write_cache(
    *, lang: str, slug: str, source_lang: str, src_facts: list[dict],
    ...
    facts_out = [
        {
            ...
            "provenance": src_facts[i].get("provenance", "wiki"),
        }
...
def _load_mt_source(path: Path) -> dict | None:
...
def enumerate_jobs(languages: tuple[str, ...], *, skip_cached: bool = True) -> list[dict]:
    jobs: list[dict] = []
    for f in sorted(TERROIR_FACTS.glob("*.json")):
        if f.name.startswith("_") or f.name.startswith("manifest"):
            continue
        src = _load_mt_source(f)
        if src is None:
            continue
        ...
        for lang in languages:
            if lang == source_lang:
                continue
            if skip_cached and _is_fresh_cache(
                load_existing(lang, src["slug"]), sha, len(src_facts),
            ):
                continue
            jobs.append({
                "slug": src["slug"], "lang": lang, "source_lang": source_lang,
                "src_facts": src_facts, "src_data": src, "sha": sha,
            })
    return jobs
...
def emit_todo(out_path: Path, *, languages: tuple[str, ...], skip_cached: bool) -> int:
    ...
    items = [
        {
            "slug": j["slug"], "lang": j["lang"], "source_lang": j["source_lang"],
    ...
    cache.write_json(out_path, {
        ...
        "languages": list(languages), "n_items": len(items), "items": items,
    })
    counts = {loc: sum(1 for j in jobs if j["lang"] == loc) for loc in languages}
    pretty = ", ".join(f"{loc}={n}" for loc, n in counts.items())
    print(f"[02e/mt] wrote {out_path} ({len(items)} items: {pretty})", file=sys.stderr)
    return 0
...
def _import_one(it: dict, src_index: dict, *, translator_id: str, translator_kind: str) -> str:
    ...
    write_cache(
        lang=lang, slug=slug,
        source_lang=it.get("source_lang") or src.get("source_lang") or "en",
        src_facts=src_facts, translated_bullets=[s.strip() for s in translated],
        src_data=src, translator=translator_id, translator_kind=translator_kind,
    )
    return "wrote"
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    for it in payload.get("items") or []:
        counts[_import_one(it, src_index, translator_id=translator_id,
                           translator_kind=translator_kind)] += 1
    pretty = ", ".join(f"{k}={v}" for k, v in counts.items() if v)
    print(f"[02e/mt] {pretty}", file=sys.stderr)
    return 0
...
def _process_one_job(provider, model_id: str, job: dict) -> tuple[bool, str | None]:
    ...
    write_cache(
        lang=job["lang"], slug=job["slug"], source_lang=job["source_lang"],
        src_facts=job["src_facts"], translated_bullets=translated,
        src_data=job["src_data"], translator=model_id, translator_kind=provider.kind,
    )
    return True, None


def _run_translation_loop(provider, model_id: str, jobs: list[dict], workers: int = 1):
    done = 0
    skipped: list[tuple[str, str, str]] = []
    if workers <= 1:
        for job in tqdm(jobs, desc="translate-mt", leave=False):
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in tqdm(as_completed(futures), total=len(jobs), desc="translate-mt", leave=False):
            job = futures[fut]
            try:
                ok, err = fut.result()
            except Exception as e:  # noqa: BLE001
...
def _run_batch(args, languages: tuple[str, ...]) -> int:
    ...
    if not jobs:
        print("[02e/mt] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02e/mt] batch: {len(jobs)} translations (provider={args.provider}, "
          f"model={model_id}, locales={','.join(languages)})", file=sys.stderr)
    batch.run_two_pass(
        provider=args.provider, model=model_id,
        sidecar=ROOT / "raw" / ".batch" / "02e-mt.json",
...
def _dispatch_emit_or_import(args, languages: tuple[str, ...]) -> int | None:
    ...
    if args.import_path:
        return import_todo(Path(args.import_path), translator_id=args.translator_id,
...
def main() -> int:
    ...
    if not TERROIR_FACTS.exists():
        print("error: raw/terroir-facts is missing — run "
              "scripts/mt/02d_extract_terroir_facts.py first", file=sys.stderr)
        return 1
    ...
    if args.limit:
        jobs = jobs[: args.limit]
    if not jobs:
        print("[02e/mt] nothing to do — all caches up to date.", file=sys.stderr)
        return 0
    ...
    if provider is None:
        print(f"[02e/mt] manual provider: {len(jobs)} entries need translation.",
              file=sys.stderr)
        return 1

    workers = max(1, args.workers)
    print(f"[02e/mt] {len(jobs)} translations to fetch "
    ...
    print(f"[02e/mt] translated: {done}, skipped: {len(skipped)}", file=sys.stderr)
    if skipped:
