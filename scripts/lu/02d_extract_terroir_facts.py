...
- Maximum {max_bullets} entrées ; chaque entrée est une phrase complète d'environ 120 à 220 caractères — jamais un fragment télégraphique.
...
def _ask_line(label: str) -> str:
    return f"Sous-section visée : {label}"


def _document_block(lien_text: str) -> str:
    return f"Texte du cahier des charges (Lien avec l'aire géographique) :\n\n{lien_text}"
...
def _process_record(provider, model_id: str, record: dict) -> dict:
    ...
    earned = earn_interactions(all_facts, "fr")
...
def main() -> int:
    ...
    terroir_verbatim.emit_for_country(
        country="lu", extracted_dir=EXTRACTED, cache_dir=CACHE_DIR,
        default_source_lang="fr", cahier_source_kind="national-cahier",
        only=args.only, log_prefix="[02d/lu]",
    )
