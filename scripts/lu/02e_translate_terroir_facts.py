"""Translate LU per-AOC terroir-fact bullets (fr → en/es/nl).

LU analog of `scripts/sk/02e_translate_terroir_facts.py`. The source
language for LU is French (the IVV cahier des charges is canonical),
so the target locales are en/es/nl — fr is the source, not a target.

Reads:  raw/terroir-facts/<slug>.json  (where country == "lu")
...
SYSTEM_PROMPT = """You translate short French bullets describing the AOP-Moselle Luxembourgeoise's terroir, history, and wine character into {lang_name}. The bullets come from the IVV (Institut Viti-Vinicole) cahier des charges (the canonical Luxembourg wine specification) and are aimed at sommelier students and wine enthusiasts.
...
def _load_lu_source(path: Path) -> dict | None:
...
def emit_todo(out_path: Path, *, languages: tuple[str, ...], skip_cached: bool) -> int:
    ...
    print(f"[02e/lu] wrote {out_path} ({len(items)} items: {pretty})", file=sys.stderr)
    return 0
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    print(f"[02e/lu] {pretty}", file=sys.stderr)
    return 0
...
def _build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        ...
        help="restrict to a specific locale (repeatable); default: all 3 (en/es/nl)",
    )
...
def _check_inputs() -> int:
    if not TERROIR_FACTS.exists():
        print(
            ...
            "scripts/lu/02d_extract_terroir_facts.py first",
            file=sys.stderr,
        )
        return 1
    return 0
...
def _run_translation_loop(
    ...
    if workers <= 1:
        for job in tqdm(jobs, desc="translate-lu", leave=False):
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in tqdm(as_completed(futures), total=len(jobs), desc="translate-lu", leave=False):
            job = futures[fut]
            try:
                ok, err = fut.result()
            except Exception as e:  # noqa: BLE001
...
def _run_batch(args, languages: tuple[str, ...]) -> int:
    ...
    if not jobs:
        print("[02e/lu] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02e/lu] batch: {len(jobs)} translations (provider={args.provider}, "
          f"model={model_id}, locales={','.join(languages)})", file=sys.stderr)
    batch.run_two_pass(
        provider=args.provider, model=model_id,
        sidecar=ROOT / "raw" / ".batch" / "02e-lu.json",
...
def main() -> int:
    ...
    if args.limit:
        jobs = jobs[: args.limit]
    if not jobs:
        print("[02e/lu] nothing to do — all caches up to date.", file=sys.stderr)
        return 0
    ...
    if provider is None:
        ...
        print(
            f"[02e/lu] manual provider: {len(jobs)} entries need translation. "
    ...
    print(
        f"[02e/lu] {len(jobs)} translations to fetch "
    ...
    print(f"[02e/lu] translated: {done}, skipped: {len(skipped)}", file=sys.stderr)
    if skipped:
        for lang, slug, err in skipped[:20]:
            print(f"  skip {lang}/{slug}: {err[:160]}", file=sys.stderr)
        print("[02e/lu] rerun this stage to retry the skipped entries.", file=sys.stderr)
    return 0
