...
for the German (DE) wine corpus.

Pipeline stage 02 (de).

For each German wine GI in `raw/de/eambrosia/index.json`:
  - if a cached HTML exists at `raw/de/oj-pages/<slug>.html`, parse the
...
Output: one JSON per wine under `raw/de/dokumente-extracted/<slug>.json`,
plus a `_index.json` mapping slug → metadata.

Sub-denomination model: the 6 Einzellage PDOs (Bürgstadter Berg, Würzburger
Stein-Berg, Monzinger Niederberg, Uhlen Blaufüsser Lay / Laubach / Roth Lay)
are tagged `is_sub_denomination=true` with parent_slug pointing at their
parent Anbaugebiet (Franken / Nahe / Mosel). Same shape as FR DGCs / ES
subzonas / IT sottozone. The eAmbrosia annotator in stage 00 wrote the
parent-link fields into the index; this script just preserves them.

Großlagen (the larger Weingesetz-defined wine collectivities like
Bocksbeutel, Niersteiner Gutes Domtal, etc.) are NOT in eAmbrosia and
are deferred to Phase 2 (mirrors IT MGAs / Weingesetz Anhang 1 parser).
"""
...
from _lib.de.region import derive_region  # noqa: E402
...
INDEX_IN = ROOT / "raw" / "de" / "eambrosia" / "index.json"
OJ_DIR = ROOT / "raw" / "de" / "oj-pages"
OJ_MANIFEST = OJ_DIR / "manifest.json"
OUT_DIR = ROOT / "raw" / "de" / "dokumente-extracted"
...
# Section 7 lists varieties one per line as `Offizieller Name - Synonym1,
# Synonym2, …` (e.g. `Spätburgunder - Pinot Noir, Blauburgunder`).
...
def build_record(wine: dict, sections: dict[str, str], titles: dict[str, str],
    ...
    return {
        ...
        "name": wine["name"],
        "kind": wine["kind"],
        "is_sub_denomination": bool(wine.get("is_sub_denomination")),
        "parent_id_eambrosia": wine.get("parent_id_eambrosia", ""),
        "parent_slug": wine.get("parent_slug", ""),
        "parent_name": wine.get("parent_name", ""),
        "region": region,
...
def build_stub(wine: dict, oj_meta: dict, reason: str) -> dict:
    return {
        ...
        "parent_slug": wine.get("parent_slug", ""),
        "parent_name": wine.get("parent_name", ""),
        # Curated file_number → region map covers every wine, so stubs
        # still keep their region facet.
...
def main() -> int:
    ...
    if not INDEX_IN.exists():
        print(f"error: {INDEX_IN} missing — run scripts/de/00_fetch_data.py first",
              file=sys.stderr)
        return 1
    ...
    for w in tqdm(wines, desc="extract-dokumente", leave=False):
        ...
        index[slug] = {
            ...
            "is_sub_denomination": bool(w.get("is_sub_denomination")),
    ...
    unknowns_path = ROOT / "raw" / "de" / "extraction-unknowns.json"
