"""Translate DE per-AOC terroir-fact bullets (DE → en/fr/es/nl).

DE analog of `scripts/at/02e_translate_terroir_facts.py`. Same bounded-
...
is German for the German wine corpus.

Reads:  raw/terroir-facts/<slug>.json  (where country == 'de')
...
SYSTEM_PROMPT = """You translate short German bullets describing a German wine appellation's terroir, history, and wine character into {lang_name}. The bullets come from the EU "Einziges Dokument" / Produktspezifikation and are aimed at sommelier students and wine enthusiasts.
...
def _load_de_source(path: Path) -> dict | None:
...
def emit_todo(out_path: Path, *, languages: tuple[str, ...], skip_cached: bool) -> int:
    ...
    print(f"[02e/de] wrote {out_path} ({len(items)} items: {pretty})", file=sys.stderr)
    return 0
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    print(f"[02e/de] {pretty}", file=sys.stderr)
    return 0
...
def _check_inputs() -> int:
    if not TERROIR_FACTS.exists():
        print(
            ...
            "scripts/de/02d_extract_terroir_facts.py first",
            file=sys.stderr,
        )
        return 1
    return 0
...
def _run_translation_loop(
    ...
    if workers <= 1:
        for job in tqdm(jobs, desc="translate-de", leave=False):
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in tqdm(as_completed(futures), total=len(jobs), desc="translate-de", leave=False):
            job = futures[fut]
            try:
                ok, err = fut.result()
            except Exception as e:  # noqa: BLE001
...
def _run_batch(args, languages: tuple[str, ...]) -> int:
    ...
    if not jobs:
        print("[02e/de] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02e/de] batch: {len(jobs)} translations (provider={args.provider}, "
          f"model={model_id}, locales={','.join(languages)})", file=sys.stderr)
    batch.run_two_pass(
        provider=args.provider, model=model_id,
        sidecar=ROOT / "raw" / ".batch" / "02e-de.json",
...
def main() -> int:
    ...
    if not jobs:
        print("[02e/de] nothing to do — all caches up to date.", file=sys.stderr)
        return 0
    ...
    if provider is None:
        ...
        print(
            f"[02e/de] manual provider: {len(jobs)} entries need translation. "
    ...
    print(
        f"[02e/de] {len(jobs)} translations to fetch "
    ...
    print(f"[02e/de] translated: {done}, skipped: {len(skipped)}", file=sys.stderr)
    if skipped:
        for lang, slug, err in skipped[:20]:
            print(f"  skip {lang}/{slug}: {err[:160]}", file=sys.stderr)
        print("[02e/de] rerun this stage to retry the skipped entries.", file=sys.stderr)
    return 0
