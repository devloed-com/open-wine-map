"""Fetch the EU-OJ "Einziges Dokument" HTML page per DE wine GI.

Pipeline stage 01 (de).

For each wine in `raw/de/eambrosia/index.json`, scan the `publications`
...
("EINZIGES DOKUMENT", template sections 1–9 / 1–10) — the German
equivalent of the FR cahier des charges, the ES pliego, and the IT
documento unico. Structurally identical to the AT pipeline (same EU-OJ
template, same German language).

Coverage:
  27 of 46 DE wines carry an OJ-C publication URL. The other 19 are
  Art. 107 / Reg. 1308/2013 grandfathered names whose only eAmbrosia
  reference is a non-fetchable `Ares(...)` summary — they ship as
  content-stubs (the IT/ES curator-queue pattern) and nonetheless land
  on the map because Bétard 2022 covers every traditional Anbaugebiet.

URL forms handled (same as ES/IT/AT, with EN→DE swap):
...
Manual overrides:
  raw/de/oj-pages/manual_overrides.json (gitignored, optional) keyed by
  giIdentifier or slug → {"url": "<german-html-url>", "note": "..."}.
  Overrides bypass the eAmbrosia publications list entirely.

Outputs:
  raw/de/oj-pages/<slug>.html          (the German Einziges-Dokument HTML)
  raw/de/oj-pages/manifest.json         (per-slug status + final URL)
...
INDEX_PATH = ROOT / "raw" / "de" / "eambrosia" / "index.json"
OUT_DIR = ROOT / "raw" / "de" / "oj-pages"
...
def first_http_publication(pubs: list[dict]) -> str | None:
    ...
    (the single document itself) → any OJ-C → any http URL."""
    for pub in pubs:
        uri = pub.get("uri") or ""
...
def _save_response(
    ...
    overrides may serve `application/pdf`."""
...
def main() -> int:
    ...
    if not INDEX_PATH.exists():
        print(f"error: {INDEX_PATH} missing — run scripts/de/00_fetch_data.py first",
              file=sys.stderr)
        return 1
