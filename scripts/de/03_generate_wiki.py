"""Generate one wiki/<slug>.md per DE wine record + extend wiki/_index.json
with DE entries.

Pipeline stage 03 (de). Mirrors `scripts/at/03_generate_wiki.py`. Reads
`raw/de/dokumente-extracted/*.json`, emits per-record markdown pages with
German section headings, and merges DE entries into `wiki/_index.json`
(preserving any pre-existing FR / ES / PT / IT / AT / SI / HR / HU / RO /
BG / GR entries).
...
def main() -> int:
    if not EXTRACTED.exists():
        print(f"error: {EXTRACTED} missing — run scripts/de/02_extract_pliegos.py first",
              file=sys.stderr)
        return 1
    ...
    if WIKI_INDEX.exists():
        try:
            ...
            existing = {}
    other_kept = {k: v for k, v in existing.items() if v.get("country") != "de"}
    merged = {**other_kept, **de_index}
    WIKI_INDEX.write_text(json.dumps(merged, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
    print(
        f"[de/03] wrote {written} DE wiki pages, merged index "
        f"({len(other_kept)} non-DE + {len(de_index)} DE = {len(merged)} entries) "
