"""Extract noteworthy terroir facts from each DE wine's "Beschreibung des
Zusammenhangs" (Einziges Dokument section 8) using a bounded LLM layer
with dual-source grounding.

DE analog of `scripts/at/02d_extract_terroir_facts.py`. Same per-bullet
provenance, fuzzy-grounding, and cache-invalidation rules; the
differences are:

  - source = DE record's `link_to_terroir` (Einziges Dokument section 8,
    straight from EUR-Lex — Germany has no national-spec fallback layer
    in v1; the Phase-2 Weingesetz/Weinverordnung parser will fill this in).
  - Wikipedia hint = `raw/wikipedia/aocs/de/<slug>.json` (fetched by
    `scripts/02b_fetch_aoc_lexicon.py --lang de --source raw/de/dokumente-extracted`).
  - LLM prompt is in German and asks for German bullets.
  - Output cache = `raw/terroir-facts/<slug>.json` (single shared dir;
    `country: "de"` distinguishes from FR/ES/PT/IT/AT/SI/HR/HU/RO/BG/GR records).

Sub-denominations (the 6 Einzellage PDOs) are skipped — they inherit the
parent Anbaugebiet's bullets at the rendering layer.

Providers: anthropic / mistral / ollama / manual (mirrors AT 02d).
"""
...
EXTRACTED = ROOT / "raw" / "de" / "dokumente-extracted"
PRODUKTSPEZIFIKATION = ROOT / "raw" / "de" / "produktspezifikationen-extracted"
WIKI_AOCS = ROOT / "raw" / "wikipedia" / "aocs" / "de"
CACHE_DIR = ROOT / "raw" / "terroir-facts"
MANIFEST = CACHE_DIR / "manifest-de.json"
...
WIKI_TO_SUBSECTION: dict[str, list[str]] = {
    "facteurs_naturels": [
        ...
        "Böden", "Klima", "Weinbaugebiet", "Anbaugebiet", "Bereiche",
        "Großlagen", "Einzellagen", "Lagen", "Weingärten", "Landschaft",
    ],
    "facteurs_humains": [
...
EXTRACT_SYSTEM = """Extrahiere bemerkenswerte Fakten über eine deutsche Wein-Ursprungsbezeichnung aus ZWEI Quellen: dem Einzigen Dokument / der Produktspezifikation (Regulierungsbehörde) und dem Auszug der deutschsprachigen Wikipedia (Referenz für ein breites Publikum, Sommelier-Vokabular).

Zielleser: Weinliebhaber oder informierte Sommeliers, die sich für die konkreten Besonderheiten einer Herkunft interessieren. Bemerkenswert ist: geologische Formationen mit ihrem Standardnamen (Buntsandstein, Muschelkalk, Keuper, Schiefer, Devon, Rotliegendes, Löss, Lehm, Mergel, Basalt, Porphyr, Granit, Vulkangestein), spezifische Bodentypen, Lokalklima und Winde (Rheinebene, Föhn, Mosel-Schleifen-Klima, atlantischer und kontinentaler Einfluss), traditionelle Rebsorten, markante weinbauliche Praktiken (Steillagen, Mosel-Steillagen, Terrassen, Spalierschnitt), präzises sensorisches Profil, datierte historische Anker (Klosterweinbau, Adelshäuser, Weinbauverbände, VDP-Klassifikation).
...
- Maximal {max_bullets} Einträge; jeder Eintrag ist ein vollständiger Satz von etwa 120–220 Zeichen — nie ein telegrafisches Fragment.
...
def _resolve_lien_and_source(rec: dict) -> tuple[str, dict]:
    """Resolve the terroir-source text for one DE record.

    Preference:
      1. EU Einziges Dokument's `link_to_terroir` (the regulator's own
         narrative, where present and long enough)
      2. BLE Produktspezifikation's §8/§9 "Zusammenhang" block — for
         records where the EU document is empty (Ahr, Baden,
         Hessische Bergstraße, Rheingau, Sachsen, Saale-Unstrut). The
         sidecar at raw/de/produktspezifikationen-extracted/<slug>.json
         carries this verbatim text along with the BLE PDF URL.
    """
    ...
    # EU is empty / too thin — try the BLE Produktspezifikation sidecar.
    slug = rec.get("slug") or ""
    sidecar_path = PRODUKTSPEZIFIKATION / f"{slug}.json"
    if sidecar_path.exists():
        try:
            sidecar = json.loads(sidecar_path.read_text(encoding="utf-8"))
            bz = (sidecar.get("zusammenhang_text") or "").strip()
            if len(bz) >= MIN_LIEN_CHARS:
                ble_url = (sidecar.get("source") or {}).get("url") or ""
                return bz, {"pdf_url": ble_url, "kind": "ble-produktspezifikation"}
        except (ValueError, OSError):
            pass
    # Fall back to whatever's there (will be filtered out by the
    # MIN_LIEN_CHARS guard in collect_targets if too short).
    eu_url = src.get("final_url") or src.get("source_url") or ""
    return lien, {"pdf_url": eu_url, "kind": "eu-oj"}


def collect_targets() -> list[dict]:
    """Return DE non-sub-denomination records with a long-enough
    link_to_terroir. The 6 Einzellage sub-denominations are skipped —
    they inherit the parent Anbaugebiet's bullets at render time."""
    out = []
...
def _ask_line(label: str) -> str:
    return f"Zu behandelnder Unterabschnitt: {label}"


def _document_block(lien_text: str) -> str:
    return f"Text des Einzigen Dokuments (Beschreibung des Zusammenhangs):\n\n{lien_text}"
...
def _process_record(provider, model_id: str, record: dict) -> dict:
    ...
    earned = earn_interactions(all_facts, "de")
...
def emit_todo(out_path: Path, *, skip_cached: bool, limit: int = 0) -> int:
    ...
    print(
        f"[02d/de] wrote {out_path} ({len(items)} items across "
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    for slug, slug_items in by_slug.items():
        ...
        counts[outcome] += 1
    print(
        f"[02d/de] wrote {counts['wrote']} cache files; "
...
def _run_batch(args) -> int:
    ...
    if not targets:
        print("[02d/de] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02d/de] batch: {len(targets)} wines (provider={args.provider}, "
    ...
    def run_loop(prov):
        ...
        if getattr(prov, "kind", "") == "collecting":
            keep = CACHE_DIR
            CACHE_DIR = Path(tempfile.mkdtemp(prefix="batch-02d-de-"))
            try:
                for rec in targets:
    ...
    batch.run_two_pass(
        ...
        sidecar=ROOT / "raw" / ".batch" / "02d-de.json",
...
def main() -> int:
    ...
    if not EXTRACTED.exists():
        print(
            f"error: {EXTRACTED} missing — run scripts/de/02_extract_pliegos.py first",
            file=sys.stderr,
        )
        return 1
    if not WIKI_AOCS.exists():
        print(
            ...
            "--source raw/de/dokumente-extracted to enable Wikipedia salience.",
            file=sys.stderr,
        )
    ...
    terroir_verbatim.emit_for_country(
        country="de", extracted_dir=EXTRACTED, cache_dir=CACHE_DIR,
        default_source_lang="de", cahier_source_kind="eu-oj",
        only=args.only, log_prefix="[02d/de]",
    )
    ...
    if not targets:
        print("[02d/de] nothing to do — all caches valid.", file=sys.stderr)
        return 0
    ...
    if provider is None:
        print(
            f"[02d/de] manual provider: {len(targets)} wines need extraction. "
    ...
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    print(
        f"[02d/de] {len(targets)} DE wines to extract "
    ...
    t0 = time.time()
    if workers <= 1:
        for rec in tqdm(targets, desc="terroir-de", leave=False):
        ...
        with ThreadPoolExecutor(max_workers=workers) as ex:
            ...
            for fut in tqdm(as_completed(futures), total=len(targets), desc="terroir-de", leave=False):
                try:
                    res = fut.result()
                    n_done += 1
                    n_facts += len(res.get("facts", []))
    ...
    print(
        f"[02d/de] done: {n_done} wines, {n_facts} facts, "
