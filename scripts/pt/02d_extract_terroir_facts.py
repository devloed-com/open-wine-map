...
- Máximo {max_bullets} itens; cada item é uma frase completa de cerca de 120–220 caracteres — nunca um fragmento telegráfico.
...
def _ask_line(label: str) -> str:
    return f"Sub-secção a tratar: {label}"


def _document_block(lien_text: str) -> str:
    return f"Texto do caderno (Relação com a área geográfica):\n\n{lien_text}"
...
def _process_record(provider, model_id: str, record: dict) -> dict:
    ...
    earned = earn_interactions(all_facts, "pt")
...
def main() -> int:
    ...
    terroir_verbatim.emit_for_country(
        country="pt", extracted_dir=EXTRACTED, cache_dir=CACHE_DIR,
        default_source_lang="pt", cahier_source_kind="national-caderno",
        only=args.only, log_prefix="[02d/pt]",
    )
