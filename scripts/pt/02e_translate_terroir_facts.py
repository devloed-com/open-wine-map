"""Translate PT per-AOC terroir-fact bullets (PT → en/fr/es/nl).

PT analog of `scripts/02e_translate_terroir_facts.py` (FR) and
`scripts/es/02e_translate_terroir_facts.py` (ES). Same bounded-narrative-
layer rules — cache per source SHA, attribution preserved, round-trip
flow for forks without API access — but the source language is
Portuguese (the caderno is canonical for PT wines), so the locale set
becomes en/fr/es/nl (FR + ES are now targets, not sources).

Reads:  raw/terroir-facts/<slug>.json  (where country == 'pt')
Writes: raw/translations/terroir-facts/<lang>/<slug>.json

Per locale cache shape mirrors the FR/ES 02e output (slug, lang, facts,
source_facts_sha, wiki_source_url, cahier_source_pdf_url, translator,
translator_kind, fetched_at) plus `country: "pt"` and
`source_lang: "pt"` so stage 04 can branch on origin if it ever needs to.
"""
...
SYSTEM_PROMPT = """You translate short Portuguese bullets describing a Portuguese wine appellation's terroir, history, and wine character into {lang_name}. The bullets come from the IVV caderno de especificações and are aimed at sommelier students and wine enthusiasts.
...
- Preserve Portuguese proper nouns verbatim: appellation names ("Douro", "Vinho Verde", "Alentejo", "Dão", "Bairrada", "Madeira", "Porto"), region names ("Trás-os-Montes", "Beira Interior", "Península de Setúbal"), commune names, grape variety names ("Touriga Nacional", "Touriga Franca", "Tinta Roriz", "Trincadeira", "Alvarinho", "Loureiro", "Arinto", "Encruzado", "Baga", "Castelão", "Fernão Pires"), named geological formations and soil types ("xisto", "granito", "schistos", "complexo xisto-grauváquico"), named winds ("nortada", "suestada"), local landscape and historical terms ("socalcos", "quinta", "lagar", "talha", "estufagem", "canteiro").
- Geological era labels: translate to the standard {lang_name} form when one exists. When unsure, keep the Portuguese form.
...
def _load_pt_source(path: Path) -> dict | None:
    """Return PT terroir-facts payload for `path` if it has facts; else None."""
...
def emit_todo(out_path: Path, *, languages: tuple[str, ...], skip_cached: bool) -> int:
    ...
    print(f"[02e/pt] wrote {out_path} ({len(items)} items: {pretty})", file=sys.stderr)
    return 0
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    print(f"[02e/pt] {pretty}", file=sys.stderr)
    return 0
...
def _check_inputs() -> int:
    if not TERROIR_FACTS.exists():
        print(
            "error: raw/terroir-facts is missing — run scripts/pt/02d_extract_terroir_facts.py first",
            file=sys.stderr,
        )
        return 1
    return 0
...
def _print_manual_listing(jobs: list[dict]) -> int:
    ...
    print(
        f"[02e/pt] manual provider: {len(jobs)} entries need translation. "
...
def _run_translation_loop(
    ...
    if workers <= 1:
        for job in tqdm(jobs, desc="translate-pt", leave=False):
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in tqdm(as_completed(futures), total=len(jobs), desc="translate-pt", leave=False):
            job = futures[fut]
            try:
                ok, err = fut.result()
            except Exception as e:  # noqa: BLE001
...
def _run_batch(args, languages: tuple[str, ...]) -> int:
    ...
    if not jobs:
        print("[02e/pt] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02e/pt] batch: {len(jobs)} translations (provider={args.provider}, "
          f"model={model_id}, locales={','.join(languages)})", file=sys.stderr)
    batch.run_two_pass(
        provider=args.provider, model=model_id,
        sidecar=ROOT / "raw" / ".batch" / "02e-pt.json",
...
def main() -> int:
    ...
    if not jobs:
        print("[02e/pt] nothing to do — all caches up to date.", file=sys.stderr)
        return 0
    ...
    print(
        f"[02e/pt] {len(jobs)} translations to fetch "
    ...
    print(f"[02e/pt] translated: {done}, skipped: {len(skipped)}", file=sys.stderr)
    if skipped:
        ...
        print("[02e/pt] rerun this stage to retry the skipped entries.", file=sys.stderr)
    return 0
