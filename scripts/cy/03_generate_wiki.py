"""Generate one wiki/<slug>.md per CY wine record + extend wiki/_index.json
with CY entries.

Pipeline stage 03 (cy). Mirrors `scripts/bg/03_generate_wiki.py` for the
CY corpus. Reads `raw/cy/dokumenti-extracted/*.json`, emits per-record
markdown pages with Greek section headings, and merges CY entries into
...
def main() -> int:
    if not EXTRACTED.exists():
        print(f"error: {EXTRACTED} missing — run scripts/cy/02_extract_pliegos.py first",
              file=sys.stderr)
        return 1
    ...
    for f in tqdm(files, desc="cy-wiki", leave=False):
    ...
    if WIKI_INDEX.exists():
        try:
            ...
            existing = {}
    other_kept = {k: v for k, v in existing.items() if v.get("country") != "cy"}
    ...
    print(
        f"[cy/03] wrote {written} CY wiki pages, merged index "
        f"({len(other_kept)} non-CY + {len(gr_index)} CY = {len(merged)} entries) "
