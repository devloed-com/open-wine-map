"""Fetch the EU-OJ "ΕΝΙΑΙΟ ΕΓΓΡΑΦΟ" HTML page per CY wine GI.

Pipeline stage 01 (cy).

For each wine in `raw/cy/eambrosia/index.json`, scan the `publications`
...
Coverage:
  Only ~11 of the 147 CY wines carry a fetchable OJ publication URL —
  ...
  AWS WAF (HTTP 202 + JavaScript challenge); `scripts/cy/01b_solve_waf.py`
...
Manual overrides:
  raw/cy/oj-pages/manual_overrides.json (gitignored, optional) keyed by
  giIdentifier or slug → {"url": "<greek-html-url>", "note": "..."}.
  Overrides bypass the eAmbrosia publications list entirely.

Outputs:
  raw/cy/oj-pages/<slug>.html          (the Greek ΕΝΙΑΙΟ ΕΓΓΡΑΦΟ HTML)
  raw/cy/oj-pages/manifest.json        (per-slug status + final URL)
