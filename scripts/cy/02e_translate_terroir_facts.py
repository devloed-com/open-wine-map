"""Translate BG per-AOC terroir-fact bullets (BG → en/fr/es/nl).

BG analog of `scripts/ro/02e_translate_terroir_facts.py`. Same bounded-
...
is Bulgarian (the ЕДИНЕН ДОКУМЕНТ is canonical for BG wines), so the
locale set is en/fr/es/nl.

Reads:  raw/terroir-facts/<slug>.json  (where country == 'bg')
...
SYSTEM_PROMPT = """You translate short Greek bullets describing a Greek wine appellation's terroir, history, and wine character into {lang_name}. The bullets come from the EU "Ενιαίο Έγγραφο" / προδιαγραφές προϊόντος and are aimed at sommelier students and wine enthusiasts.
...
def _load_bg_source(path: Path) -> dict | None:
...
def emit_todo(out_path: Path, *, languages: tuple[str, ...], skip_cached: bool) -> int:
    ...
    print(f"[02e/cy] wrote {out_path} ({len(items)} items: {pretty})", file=sys.stderr)
    return 0
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    print(f"[02e/cy] {pretty}", file=sys.stderr)
    return 0
...
def _check_inputs() -> int:
    if not TERROIR_FACTS.exists():
        print(
            ...
            "scripts/cy/02d_extract_terroir_facts.py first",
            file=sys.stderr,
        )
        return 1
    return 0
...
def _run_translation_loop(
    ...
    if workers <= 1:
        for job in tqdm(jobs, desc="translate-bg", leave=False):
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in tqdm(as_completed(futures), total=len(jobs), desc="translate-bg", leave=False):
            job = futures[fut]
            try:
                ok, err = fut.result()
            except Exception as e:  # noqa: BLE001
...
def _run_batch(args, languages: tuple[str, ...]) -> int:
    ...
    if not jobs:
        print("[02e/cy] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02e/cy] batch: {len(jobs)} translations (provider={args.provider}, "
          f"model={model_id}, locales={','.join(languages)})", file=sys.stderr)
    batch.run_two_pass(
        provider=args.provider, model=model_id,
        sidecar=ROOT / "raw" / ".batch" / "02e-cy.json",
...
def main() -> int:
    ...
    if not jobs:
        print("[02e/cy] nothing to do — all caches up to date.", file=sys.stderr)
        return 0
    ...
    if provider is None:
        ...
        print(
            f"[02e/cy] manual provider: {len(jobs)} entries need translation. "
    ...
    print(
        f"[02e/cy] {len(jobs)} translations to fetch "
    ...
    print(f"[02e/cy] translated: {done}, skipped: {len(skipped)}", file=sys.stderr)
    if skipped:
        for lang, slug, err in skipped[:20]:
            print(f"  skip {lang}/{slug}: {err[:160]}", file=sys.stderr)
        print("[02e/cy] rerun this stage to retry the skipped entries.", file=sys.stderr)
    return 0
