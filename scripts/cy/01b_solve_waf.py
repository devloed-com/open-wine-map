"""One-off bootstrap: fetch the EUR-Lex ΕΝΙΑΙΟ ΕΓΓΡΑΦΟ HTMLs that
`scripts/cy/01_fetch_pliegos.py` couldn't reach because EUR-Lex sits
behind an AWS WAF that returns HTTP 202 + a JavaScript challenge for
non-browser clients.

Headless Chromium runs the WAF challenge JS automatically. We then
save the HTML at the same cache path as stage 01 so subsequent runs
see it as already-cached and skip.

Run: `.venv/bin/python scripts/cy/01b_solve_waf.py`
After: re-run `scripts/cy/02_extract_pliegos.py` to pick up the new HTMLs.
"""
...
def to_greek(url: str) -> str:
    ...
    new = re.sub(r"/legal-content/[A-Z]{2}/TXT(?:/HTML)?/", "/legal-content/EL/TXT/HTML/", url)
    new = re.sub(
...
def main() -> int:
    ...
    if not MANIFEST_PATH.exists():
        print(f"error: {MANIFEST_PATH} missing — run scripts/cy/01_fetch_pliegos.py first",
              file=sys.stderr)
        return 1
    ...
    for w in wines:
        ...
        if override and override.get("url"):
            ...
            url = to_greek(base) if base else None
        if not url:
            continue
    ...
    with sync_playwright() as p:
        ...
        ctx = browser.new_context(
            locale="el-CY",
            extra_http_headers={"Accept-Language": "el-CY,el;q=0.9,en;q=0.8"},
        )
        page = ctx.new_page()
