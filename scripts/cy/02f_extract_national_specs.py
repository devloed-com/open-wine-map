...
into per-wine sidecars under `raw/cy/national-specs-extracted/`.

Pipeline stage 02f (cy). Sibling of the ES MAPA / IT MASAF / DE BLE /
HR–SI specifikacije layers: it augments the 138 CY stub records (wines
with no fetchable EU-OJ ΕΝΙΑΙΟ ΕΓΓΡΑΦΟ) by parsing the ΥΠΑΑΤ national
spec (`scripts/_lib/cy/specifikacija.py`). Stage 04's
...
Re-runnable per slug or in sweep mode:
    .venv/bin/python scripts/cy/02f_extract_national_specs.py --slug samos
    .venv/bin/python scripts/cy/02f_extract_national_specs.py --all
"""
...
OUT_DIR = ROOT / "raw" / "cy" / "national-specs-extracted"
OUT_INDEX = OUT_DIR / "_index.json"
UNKNOWNS = ROOT / "raw" / "cy" / "extraction-unknowns-national.json"

_SPEC_EXTS = ("pdf", "doc", "docx")


EXTRACTED_DIR = ROOT / "raw" / "cy" / "dokumenti-extracted"
...
def main() -> int:
    ...
    if MANIFEST_PATH.exists():
        try:
            ...
            manifest = {}
    if not overrides and not manifest:
        print(f"error: neither {OVERRIDES_PATH} nor {MANIFEST_PATH} present — "
              "run stage 01c first", file=sys.stderr)
        return 1

    # CY discovery is scrape-driven (stage 01c writes the manifest, not an
    # overrides file), so the slug universe is the union of fetched-spec
    # slugs (manifest status=ok) and any manual overrides.
    known = sorted(set(overrides) | {
        s for s, m in manifest.items() if (m or {}).get("status") == "ok"
    })
    if args.slug:
        targets = [args.slug]
    elif args.only:
        needles = [s.lower() for s in args.only]
        targets = [s for s in known if any(n in s.lower() for n in needles)]
    else:
        targets = known
    ...
    for slug in targets:
        ...
        sidecar = {
            ...
            "source": {
                ...
                "source_org": m.get("source_org", "moa-cy"),
                "sha256": m.get("sha256", ""),
