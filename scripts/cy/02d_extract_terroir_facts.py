...
differences are:
  ...
  - Wikipedia hint = `raw/wikipedia/aocs/el/<slug>.json` (fetched by
    `scripts/02b_fetch_aoc_lexicon.py --lang bg --source raw/cy/dokumenti-extracted`).
  - LLM prompt is in Bulgarian and asks for Bulgarian bullets.
  - Output cache = `raw/terroir-facts/<slug>.json` (single shared dir;
    `country: "cy"` distinguishes from FR/ES/PT/IT/AT/SI/HR/RO records).
...
EXTRACTED = ROOT / "raw" / "cy" / "dokumenti-extracted"
WIKI_AOCS = ROOT / "raw" / "wikipedia" / "aocs" / "el"
CACHE_DIR = ROOT / "raw" / "terroir-facts"
MANIFEST = CACHE_DIR / "manifest-cy.json"
...
SUBSECTIONS = [
    {
        ...
        "label": "Φυσικοί παράγοντες (γεωλογία, εδάφη, κλίμα, ανάγλυφο)",
        "topics": (
            "γεωλογία, σύσταση του εδάφους, κλίμα, ανάγλυφο, ύδατα, "
            "έκθεση, τοποθεσίες και οικισμοί"
        ),
        "max_bullets": 5,
    },
    {
        "key": "facteurs_humains",
        "label": "Ιστορικοί και ανθρώπινοι παράγοντες (ιστορία, πρακτικές)",
        "topics": (
            "ιστορία της ονομασίας προέλευσης, αμπελοκομικές και "
            "οινοποιητικές πρακτικές, παραδοσιακές ποικιλίες σταφυλιού"
        ),
        "max_bullets": 2,
    },
    {
        "key": "produit",
        "label": "Χαρακτηριστικά του προϊόντος (οργανοληπτικό προφίλ)",
        "topics": "χρώματα, αρώματα, δομή, παλαίωση, οργανοληπτικό προφίλ των οίνων",
        "max_bullets": 2,
    },
    {
        "key": "interactions",
        "label": "Αιτιώδεις δεσμοί (σύνδεσμος terroir / οίνου)",
        "topics": (
            "σαφής σύνδεσμος μεταξύ του terroir και του χαρακτήρα των "
            "οίνων, έκφραση του εδάφους ή του κλίματος στο κρασί"
        ),
        "max_bullets": 1,
    },
]
...
# Greek Wikipedia section headings that map to each ΕΝΙΑΙΟ ΕΓΓΡΑΦΟ
# sub-section. Greek wine-region articles use a wide range of
# headings; the list is permissive.
WIKI_TO_SUBSECTION: dict[str, list[str]] = {
    "facteurs_naturels": [
        "Γεωγραφία", "Τοποθεσία", "Γεωλογία", "Εδάφη", "Έδαφος", "Κλίμα",
        "Ανάγλυφο", "Αμπελώνες", "Αμπελοκαλλιέργεια", "Φύση",
        "Φυσικές συνθήκες", "Περιοχή",
    ],
    "facteurs_humains": [
        "Ιστορία", "Ετυμολογία", "Ονομασία", "Ποικιλίες",
        "Ποικιλίες σταφυλιού", "Αμπελοκαλλιέργεια", "Οινοποίηση",
        "Οινοπαραγωγή", "Παράδοση", "Οινική κουλτούρα",
    ],
    "produit": [
        "Οίνοι", "Κρασιά", "Είδη οίνων", "Χαρακτηριστικά",
        "Οργανοληπτικό προφίλ",
    ],
    "interactions": [],
}
...
EXTRACT_SYSTEM = """Εξάγαγε αξιοσημείωτα χαρακτηριστικά ενός ελληνικού οίνου με προστατευόμενη ονομασία προέλευσης ή προστατευόμενη γεωγραφική ένδειξη από ΔΥΟ πηγές: το ενιαίο έγγραφο / τις προδιαγραφές του προϊόντος (ρυθμιστική αυθεντία) και το απόσπασμα της ελληνικής Wikipedia (κοινόχρηστο εγκυκλοπαιδικό λεξικό, λεξιλόγιο σομελιέ).

Στοχευόμενος αναγνώστης: λάτρεις του κρασιού ή έμπειροι σομελιέ που ενδιαφέρονται για τις συγκεκριμένες ιδιαιτερότητες της προέλευσης. Αξίζει να αναφερθούν: γεωλογικοί σχηματισμοί με την τεχνική τους ονομασία, συγκεκριμένοι τύποι εδαφών (αλλουβιακά, μάργες, σχιστόλιθος, ασβεστόλιθος, πυριγενή πετρώματα, ηφαιστειακή ασπρα γη της Σαντορίνης, μαυρόχωμα, αμμώδη, χαλικώδη, αργιλώδη), μικροκλίμα και τοπικοί άνεμοι (μελτέμια, ετήσιοι, μεσογειακό κλίμα, ηπειρωτικό κλίμα, ορεινό μικροκλίμα), παραδοσιακές ποικιλίες σταφυλιού, χαρακτηριστικές αμπελοκομικές πρακτικές (κουλούρα, αναβαθμίδες, ξερολιθιές, στρέμμα), ακριβές οργανοληπτικό προφίλ, χρονολογημένα ιστορικά στοιχεία.

═══ ΠΗΓΗ 1: απόσπασμα από Wikipedia (σχετικές ενότητες) ═══
{wiki_hint}

═══ ΠΗΓΗ 2: κείμενο του ενιαίου εγγράφου (στο μήνυμα χρήστη παρακάτω) ═══

Υποενότητα προς επεξεργασία: {label}
Θεματικές προτεραιότητες: {topics}

Αυστηροί κανόνες:
- Κάθε στοιχείο ΠΡΕΠΕΙ να υποστηρίζεται από ΤΟΥΛΑΧΙΣΤΟΝ ΕΝΑ απόσπασμα: `cahier_quote` (αυτολεξεί από το ενιαίο έγγραφο) Ή `wiki_quote` (αυτολεξεί από το απόσπασμα της Wikipedia παραπάνω). Και τα δύο συνιστώνται όταν και οι δύο πηγές περιγράφουν την ίδια πληροφορία.
- Προτεραιοποίησε το ενιαίο έγγραφο ως κύρια πηγή: χρησιμοποίησε `cahier_quote` όποτε το γεγονός εμφανίζεται εκεί.
- Πρόσθεσε `wiki_quote` όταν η Wikipedia παρέχει το τυπικό τεχνικό λεξιλόγιο για ένα γεγονός που το ενιαίο έγγραφο περιγράφει με άλλα λόγια.
- Αν ένα αξιοσημείωτο γεγονός αναφέρεται μόνο στη Wikipedia, μπορείς να το κρατήσεις μόνο με `wiki_quote` — κατά την εμφάνιση θα αποδοθεί στη Wikipedia.
- Τα αποσπάσματα είναι ΑΥΤΟΛΕΞΕΙ (αντιγραμμένα) από την αντίστοιχη πηγή. ΠΟΤΕ μην αποδώσεις σε μια πηγή κείμενο που δεν εμφανίζεται εκεί.
- Χωρίς αξιολογικές κρίσεις ("εξαίσιος", "πρεστιζιακός" …).
- Χωρίς εξωτερικά συμπεράσματα. Χωρίς αριθμούς που δεν εμφανίζονται σε καμία από τις δύο πηγές.
- Μέγιστο {max_bullets} στοιχεία· κάθε στοιχείο είναι μία πλήρης πρόταση περίπου 120–220 χαρακτήρων — ποτέ τηλεγραφικό απόσπασμα.
- Αν ούτε το ενιαίο έγγραφο ούτε η Wikipedia περιέχουν συγκεκριμένο γεγονός άξιο προσοχής για αυτή την υποενότητα, επέστρεψε κενή λίστα.

Απάντησε ΜΟΝΟ σε JSON, χωρίς κείμενο πριν ή μετά:
{{"facts": [{{"bullet": "...", "cahier_quote": "...", "wiki_quote": "..."}}, ...]}}
Για ελλείπον απόσπασμα χρησιμοποίησε κενό αλφαριθμητικό ""."""
...
NATIONAL_SPECS = ROOT / "raw" / "cy" / "national-specs-extracted"
...
def _ask_line(label: str) -> str:
    return f"Υποενότητα προς επεξεργασία: {label}"


def _document_block(lien_text: str) -> str:
    return f"Κείμενο του ενιαίου εγγράφου (Περιγραφή του δεσμού):\n\n{lien_text}"
...
def _process_subsection(
    ...
    system = EXTRACT_SYSTEM.format(
        wiki_hint=wiki_hint or "(απόσπασμα Wikipedia μη διαθέσιμο)",
        label=sub["label"],
        topics=sub["topics"],
...
def _process_record(provider, model_id: str, record: dict) -> dict:
    ...
    earned = earn_interactions(all_facts, "el")
...
def emit_todo(out_path: Path, *, skip_cached: bool, limit: int = 0) -> int:
    ...
    print(
        f"[02d/cy] wrote {out_path} ({len(items)} items across "
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    for slug, slug_items in by_slug.items():
        ...
        counts[outcome] += 1
    print(
        f"[02d/cy] wrote {counts['wrote']} cache files; "
...
def _run_batch(args) -> int:
    ...
    if not targets:
        print("[02d/cy] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02d/cy] batch: {len(targets)} wines (provider={args.provider}, "
    ...
    def run_loop(prov):
        ...
        if getattr(prov, "kind", "") == "collecting":
            keep = CACHE_DIR
            CACHE_DIR = Path(tempfile.mkdtemp(prefix="batch-02d-cy-"))
            try:
                for rec in targets:
    ...
    batch.run_two_pass(
        ...
        sidecar=ROOT / "raw" / ".batch" / "02d-cy.json",
...
def main() -> int:
    ...
    if not EXTRACTED.exists():
        print(
            f"error: {EXTRACTED} missing — run scripts/cy/02_extract_pliegos.py first",
            file=sys.stderr,
        )
        return 1
    if not WIKI_AOCS.exists():
        print(
            ...
            "Run scripts/02b_fetch_aoc_lexicon.py --lang el "
            "--source raw/cy/dokumenti-extracted to enable Wikipedia salience.",
            file=sys.stderr,
        )
    ...
    terroir_verbatim.emit_for_country(
        country="cy", extracted_dir=EXTRACTED, cache_dir=CACHE_DIR,
        default_source_lang="el", cahier_source_kind="eu-oj",
        only=args.only, log_prefix="[02d/cy]",
    )
    ...
    if not targets:
        print("[02d/cy] nothing to do — all caches valid.", file=sys.stderr)
        return 0
    ...
    if provider is None:
        print(
            f"[02d/cy] manual provider: {len(targets)} wines need extraction. "
    ...
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    print(
        f"[02d/cy] {len(targets)} BG wines to extract "
    ...
    t0 = time.time()
    if workers <= 1:
        for rec in tqdm(targets, desc="terroir-cy", leave=False):
        ...
        with ThreadPoolExecutor(max_workers=workers) as ex:
            ...
            for fut in tqdm(as_completed(futures), total=len(targets), desc="terroir-cy", leave=False):
                try:
                    res = fut.result()
                    n_done += 1
                    n_facts += len(res.get("facts", []))
    ...
    print(
        f"[02d/cy] done: {n_done} wines, {n_facts} facts, "
