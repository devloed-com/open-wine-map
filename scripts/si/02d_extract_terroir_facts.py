...
- Največ {max_bullets} vnosov; vsak vnos je en poln stavek z približno 120–220 znaki — nikoli telegrafski fragment.
...
def _ask_line(label: str) -> str:
    return f"Podrazdelek za obravnavo: {label}"


def _document_block(lien_text: str) -> str:
    return f"Besedilo enotnega dokumenta (Povezava z geografskim območjem):\n\n{lien_text}"
...
def _process_record(provider, model_id: str, record: dict) -> dict:
    ...
    earned = earn_interactions(all_facts, "sl")
