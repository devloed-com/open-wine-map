"""Regenerate the manual_overrides.json template for SI wines whose
single-document URL is not auto-discovered / not fetchable.

Reads:
  raw/si/eambrosia/index.json — full SI wine list
  raw/si/oj-pages/manifest.json — stage 01/01b fetch outcomes per wine

Writes:
  raw/si/oj-pages/manual_overrides.json — one entry per wine that needs
...
16 of the 17 Slovenian wines are Art.107/Reg.1308/2013 grandfathered
...
(EUR-Lex OJ-C page, or the MKGP national specification once a parser
branch exists). After filling URLs, re-run scripts/si/01_fetch_pliegos.py
...
EAMBROSIA_INDEX = ROOT / "raw" / "si" / "eambrosia" / "index.json"
...
def main() -> int:
    if not EAMBROSIA_INDEX.exists():
        print(f"error: {EAMBROSIA_INDEX} missing — run scripts/si/00_fetch_data.py first",
              file=sys.stderr)
        return 1
    ...
    out["__doc__"] = (
        "Curator-filled overrides for SI wines whose single-document "
        "URL is not auto-discovered or not fetchable. Fill the `url` "
        "field for each entry to point at a public, licence-clear "
        "specification (EUR-Lex OJ-C page, or the MKGP national "
        "specification). Stage 01 reads this file before falling back "
        "to eAmbrosia publications. Re-run scripts/si/01_fetch_pliegos.py "
