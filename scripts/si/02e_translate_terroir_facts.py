"""Translate SI per-AOC terroir-fact bullets (SL → en/fr/es/nl).

SI analog of `scripts/at/02e_translate_terroir_facts.py`. Same bounded-
...
is Slovenian (the ENOTNI DOKUMENT is canonical for SI wines), so the
locale set is en/fr/es/nl.

Reads:  raw/terroir-facts/<slug>.json  (where country == 'si')
...
SYSTEM_PROMPT = """You translate short Slovenian bullets describing a Slovenian wine appellation's terroir, history, and wine character into {lang_name}. The bullets come from the EU "enotni dokument" / specifikacija proizvoda and are aimed at sommelier students and wine enthusiasts.
...
def emit_todo(out_path: Path, *, languages: tuple[str, ...], skip_cached: bool) -> int:
    ...
    print(f"[02e/si] wrote {out_path} ({len(items)} items: {pretty})", file=sys.stderr)
    return 0
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    print(f"[02e/si] {pretty}", file=sys.stderr)
    return 0
...
def _check_inputs() -> int:
    if not TERROIR_FACTS.exists():
        print(
            ...
            "scripts/si/02d_extract_terroir_facts.py first",
            file=sys.stderr,
        )
        return 1
    return 0
...
def _run_translation_loop(
    ...
    if workers <= 1:
        for job in tqdm(jobs, desc="translate-si", leave=False):
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in tqdm(as_completed(futures), total=len(jobs), desc="translate-si", leave=False):
            job = futures[fut]
            try:
                ok, err = fut.result()
            except Exception as e:  # noqa: BLE001
...
def _run_batch(args, languages: tuple[str, ...]) -> int:
    ...
    if not jobs:
        print("[02e/si] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02e/si] batch: {len(jobs)} translations (provider={args.provider}, "
          f"model={model_id}, locales={','.join(languages)})", file=sys.stderr)
    batch.run_two_pass(
        provider=args.provider, model=model_id,
        sidecar=ROOT / "raw" / ".batch" / "02e-si.json",
...
def main() -> int:
    ...
    if not jobs:
        print("[02e/si] nothing to do — all caches up to date.", file=sys.stderr)
        return 0
    ...
    if provider is None:
        ...
        print(
            f"[02e/si] manual provider: {len(jobs)} entries need translation. "
    ...
    print(
        f"[02e/si] {len(jobs)} translations to fetch "
    ...
    print(f"[02e/si] translated: {done}, skipped: {len(skipped)}", file=sys.stderr)
    if skipped:
        for lang, slug, err in skipped[:20]:
            print(f"  skip {lang}/{slug}: {err[:160]}", file=sys.stderr)
        print("[02e/si] rerun this stage to retry the skipped entries.", file=sys.stderr)
    return 0
