"""Fetch the public reference datasets the Croatia pipeline depends on.

Pipeline stage 00 (hr).

Sources:

1. **eAmbrosia EU register** (spine — list of HR wine GIs)
   `https://webgate.ec.europa.eu/eambrosia-api/api/v1/geographical-indications`
   Filtered client-side to country=HR + productType=WINE + status=registered
   → 18 wine GIs (all PDOs; Croatia has no wine IGPs in eAmbrosia). The
   endpoint returns the full register (~6.5 MB, ~4000 GIs across all EU);
   we share the response with the ES + PT + IT + AT + SI pipelines via
   separate per-country index files.

   Coverage: only 2 of the 18 HR wines (Muškat momjanski, Ponikve) carry
   a fetchable EU single-document URL — the other 16 are Art.107/
   Reg.1308/2013 grandfathered names with no public single-document URL
   and only a non-fetchable `Ares(...)` summary-sheet. They land as
   `no-publication` and stage 02 emits content-stubs (the IT / ES / SI
   curator-queue pattern). Geometry resolves independently: all 18 are
   in Bétard 2022, so every wine has a polygon regardless.
...
2. **Figshare EU_PDO.gpkg** (Bétard et al. 2022, CC0, ~42 MB) — *already
   ...
   all 18 Croatian PDOs (`PDO-HR-*` file numbers). No new fetch — stage
   04 reads the shared artifact via `HRPolygonIndex`.

Outputs:
- raw/hr/eambrosia/index.json — filtered HR-wine list with derived slug + kind
- raw/hr/eambrosia/manifest.json — fetch metadata for the eAmbrosia call
"""
...
OUT_DIR = ROOT / "raw" / "hr" / "eambrosia"
INDEX_PATH = OUT_DIR / "index.json"
MANIFEST_PATH = OUT_DIR / "manifest.json"

# Shared upstream artifact already fetched by the ES pipeline. Asserted
# here so a fresh HR-only checkout fails loudly rather than silently
...
def normalise_kind(gi_type: str) -> str:
    """eAmbrosia uses `PDO`/`PGI`; the HR convention (same as ES/PT/IT/AT/SI)
    is `DOP`/`IGP`."""
...
def main() -> int:
    ...
    hr_wines_all = [
        g for g in full
        if "HR" in (g.get("countries") or []) and g.get("productType") == "WINE"
    ]
    hr_wines = [g for g in hr_wines_all if g.get("status") == "registered"]
    n_skipped_applied = len(hr_wines_all) - len(hr_wines)

    by_slug: dict[str, list[dict]] = {}
    projected = [project(rec) for rec in hr_wines]
    for p in projected:
        by_slug.setdefault(p["slug"], []).append(p)
    collisions = {s: ps for s, ps in by_slug.items() if len(ps) > 1}
    if collisions:
        print(
            f"[warn] {len(collisions)} slug collisions inside HR wines:",
            file=sys.stderr,
        )
    ...
    print(
        f"[done] eAmbrosia: total_eu={len(full)} hr_wines={len(projected)} "
