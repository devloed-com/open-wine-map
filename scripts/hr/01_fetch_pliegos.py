"""Fetch the EU-OJ "Jedinstveni dokument" HTML page per HR wine GI.

Pipeline stage 01 (hr).

For each wine in `raw/hr/eambrosia/index.json`, scan the `publications`
array for the first HTTP URL and request the Croatian-language variant
of that EUR-Lex page. The page contains the canonical EU single document
("JEDINSTVENI DOKUMENT") — the Croatian equivalent of the FR cahier des
charges, the ES pliego, the IT documento unico, the AT Einziges Dokument
and the SI Enotni dokument.

Coverage:
  Only 2 of the 18 HR wines (Muškat momjanski, Ponikve) carry a fetchable
  OJ publication URL — the other 16 are Art.107/Reg.1308/2013 grandfathered
  names with no public single-document URL. They land as `no-publication`
  and stage 02 emits content-stubs (the IT/ES/SI curator-queue pattern).
  The other failure mode is the EUR-Lex AWS WAF (HTTP 202 + JavaScript
  challenge); `scripts/hr/01b_solve_waf.py` bootstraps the blocked subset
  via headless Chromium.

Croatian variant rules (same as ES/IT/AT/SI, with the language swap):
  /eli/.../oj  →  /eli/.../oj/hrv  (3-letter ISO 639-2)
  legal-content/EN/TXT/  →  legal-content/HR/TXT/HTML/
  uri=...01.ENG&...     →  uri=...01.HRV&...

Manual overrides:
  raw/hr/oj-pages/manual_overrides.json (gitignored, optional) keyed by
  giIdentifier or slug → {"url": "<croatian-html-url>", "note": "..."}.
  Overrides bypass the eAmbrosia publications list entirely.

Outputs:
  raw/hr/oj-pages/<slug>.html          (the Croatian Jedinstveni-dokument HTML)
  raw/hr/oj-pages/manifest.json         (per-slug status + final URL)
...
INDEX_PATH = ROOT / "raw" / "hr" / "eambrosia" / "index.json"
OUT_DIR = ROOT / "raw" / "hr" / "oj-pages"
...
def to_croatian(url: str) -> str:
    """Best-effort URL rewrite: any EUR-Lex / EU-OJ URL → its Croatian
    HTML variant. Idempotent — already-Croatian URLs pass through."""
    if re.search(r"/oj/?(?:[#?]|$)", url) and not re.search(r"/oj/[a-z]{3}", url):
        return re.sub(r"/oj(/?)([#?]|$)", r"/oj/hrv\2", url)
    new = re.sub(
        r"/legal-content/[A-Z]{2}/TXT(?:/HTML)?/",
        "/legal-content/HR/TXT/HTML/",
        url,
    )
    new = re.sub(r"(\d{2}\.)(?:ENG|FRA|DEU|ITA|POR|SPA|NLD|SLV|HRV)(?=&|$)", r"\1HRV", new)
    new = re.sub(r"(:)(?:ENG|FRA|DEU|ITA|POR|SPA|NLD|SLV|HRV)(?=&|$)", r"\1HRV", new)
    return new
...
def _is_corrigendum(pub: dict) -> bool:
    """A Corrigendum / Ispravak publication is a short correction notice,
    not the full single document — deprioritise it."""
    text = (pub.get("text") or "").lower()
    return "corrigendum" in text or "ispravak" in text
...
def fetch_page(session: requests.Session, url: str) -> requests.Response | None:
    """Fetch with 202-aware retry. EUR-Lex generates `legal-content/HR/TXT/HTML`
...
def looks_like_single_document(html: str) -> bool:
    ...
    publication. The Jedinstveni-dokument HTML is large (>8 KB) and
    contains literal Croatian phrases."""
    if len(html) < 8000:
        return False
    low = html.lower()
    return (
        "jedinstveni dokument" in low
        or "specifikacija proizvoda" in low
        or "oznaka izvornosti" in low
        or "zaštićena oznaka" in low
        or "zasticena oznaka" in low
    )
...
def main() -> int:
    ...
    if not INDEX_PATH.exists():
        print(f"error: {INDEX_PATH} missing — run scripts/hr/00_fetch_data.py first",
              file=sys.stderr)
        return 1
    ...
    for w in tqdm(wines, desc="oj-pages", leave=False):
        ...
        if override and override.get("url"):
            ...
            source_url = to_croatian(base) if base else None
