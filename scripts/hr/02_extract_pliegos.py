"""Extract structured fields from each cached EUR-Lex Jedinstveni-dokument HTML.

Pipeline stage 02 (hr).

For each Croatian wine GI in `raw/hr/eambrosia/index.json`:
  - if a cached HTML exists at `raw/hr/oj-pages/<slug>.html`, parse the
    "JEDINSTVENI DOKUMENT" block into numbered sections (the EU wine GI
    template) and route them by Croatian title keyword
  - else emit a stub record so the wine remains searchable

Output: one JSON per wine under `raw/hr/dokumenti-extracted/<slug>.json`,
plus a `_index.json` mapping slug → metadata.

v1 models the 18 Croatian wine GIs as a flat corpus — the regulatory
hierarchy (3 macro regions ⊃ sub-regions ⊃ positions) is preserved via
the `region` facet rather than as parent / sub-denomination records.
"""
...
from _lib.hr.region import derive_region  # noqa: E402
from _lib.grape_entity import (  # noqa: E402
    flush_unknowns_queue, match_variety, set_pliego_context,
)

INDEX_IN = ROOT / "raw" / "hr" / "eambrosia" / "index.json"
OJ_DIR = ROOT / "raw" / "hr" / "oj-pages"
OJ_MANIFEST = OJ_DIR / "manifest.json"
OUT_DIR = ROOT / "raw" / "hr" / "dokumenti-extracted"
...
def slice_jedinstveni_dokument(html: str) -> str | None:
...
# Croatian grape-variety sections list varieties one per line as
# `Canonical name – Synonym, Synonym, ...` (often with an en-dash). The
# canonical name is the segment before the dash.
_BULLET_SPLIT_RE = re.compile(r"\s*[—•·]\s*|\n")
_NAME_SYN_SPLIT_RE = re.compile(r"\s+[-–]\s+")

_GRAPE_LINE_DROP = (
    "glavne sorte", "vinske sorte", "sorte vinove loze", "sorta vinove loze",
    "dopuštene", "dopustene", "preporučene", "preporucene",
)
...
def parse_styles(sections: dict[str, str], titles: dict[str, str]) -> list[str]:
    ...
    sections. Croatian colour adjectives + Predikat / sparkling markers."""
    blob_parts = []
    for num, body in sections.items():
        ...
        if (
            ...
            or "daljnji" in title_low
            or "dodatni" in title_low
        ):
...
def _extract_from_html(cache: Path) -> tuple[dict[str, str], dict[str, str], str]:
    ...
    doc = slice_jedinstveni_dokument(html)
    if doc is None:
        return {}, {}, "no-jedinstveni-dokument-anchor"
...
def main() -> int:
    ...
    if not INDEX_IN.exists():
        print(f"error: {INDEX_IN} missing — run scripts/hr/00_fetch_data.py first",
              file=sys.stderr)
        return 1
    ...
    unknowns_path = ROOT / "raw" / "hr" / "extraction-unknowns.json"
