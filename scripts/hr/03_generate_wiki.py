"""Generate one wiki/<slug>.md per HR wine record + extend
wiki/_index.json with HR entries.

Pipeline stage 03 (hr). Mirrors `scripts/si/03_generate_wiki.py` for the
HR corpus. Reads `raw/hr/dokumenti-extracted/*.json`, emits per-record
markdown pages with Croatian section headings, and merges HR entries
into `wiki/_index.json` (preserving any pre-existing FR/ES/PT/IT/AT/SI
entries).

A curated cross-border note (e.g. the Hrvatska Istra ↔ Teran labelling
note) is rendered as a `## Napomena` section when
`scripts/_lib/appellation_notes.json` carries an entry for the slug —
see that file for the sourced-annotation contract.
"""
...
SECTION_LABELS = {
    "summary": "Sažetak",
    "geo": "Razgraničeno zemljopisno područje",
    "grapes": "Sorte vinove loze",
    "link": "Opis povezanosti",
    "note": "Napomena",
    "sources": "Izvori",
}
...
def _render_note_section(slug: str) -> list[str]:
    """Render the curated cross-border note as a `## Napomena` block, with
...
def render_record(record: dict) -> str:
    ...
    if record.get("stub"):
        body += [
            "> ⚠️ **Jedinstveni dokument nije dostupan.** "
            f"Razlog: `{record.get('stub_reason') or 'unknown'}`. "
            "Naziv ostaje u kazalu; sadržaj će biti dopunjen kad bude "
            "dostupan javni izvor specifikacije proizvoda.",
            "",
        ]
    ...
    body += [
        f"## {SECTION_LABELS['sources']}",
        "",
        f"- EUR-Lex (JEDINSTVENI DOKUMENT): <{src.get('final_url') or src.get('source_url') or ''}>",
        ...
        f"- File number: `{record.get('file_number') or ''}`",
        "",
        "_Tekst jedinstvenog dokumenta: © Europska unija / EUR-Lex. "
        "Ponovna uporaba uz navođenje izvora._",
        "",
    ]
    return "\n".join(fm + body)
...
def main() -> int:
    if not EXTRACTED.exists():
        print(f"error: {EXTRACTED} missing — run scripts/hr/02_extract_pliegos.py first",
              file=sys.stderr)
        return 1
    ...
    written = 0
    hr_index: dict[str, dict] = {}
    for f in tqdm(files, desc="hr-wiki", leave=False):
        rec = json.loads(f.read_text(encoding="utf-8"))
        out_path = WIKI / f"{rec['slug']}.md"
        out_path.write_text(render_record(rec), encoding="utf-8")
        hr_index[rec["slug"]] = index_entry(rec)
        written += 1
    ...
    if WIKI_INDEX.exists():
        try:
            ...
            existing = {}
    other_kept = {k: v for k, v in existing.items() if v.get("country") != "hr"}
    merged = {**other_kept, **hr_index}
    WIKI_INDEX.write_text(json.dumps(merged, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
    print(
        f"[hr/03] wrote {written} HR wiki pages, merged index "
        f"({len(other_kept)} non-HR + {len(hr_index)} HR = {len(merged)} entries) "
