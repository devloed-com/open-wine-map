"""Regenerate the manual_overrides.json template for HR wines whose
single-document URL is not auto-discovered / not fetchable.

Reads:
  raw/hr/eambrosia/index.json — full HR wine list
  raw/hr/oj-pages/manifest.json — stage 01/01b fetch outcomes per wine

Writes:
  raw/hr/oj-pages/manual_overrides.json — one entry per wine that needs
...
16 of the 18 Croatian wines are Art.107/Reg.1308/2013 grandfathered
...
(EUR-Lex OJ-C page, or the MPS / NN gazette pliego once a parser
branch exists). After filling URLs, re-run scripts/hr/01_fetch_pliegos.py
...
EAMBROSIA_INDEX = ROOT / "raw" / "hr" / "eambrosia" / "index.json"
OJ_MANIFEST = ROOT / "raw" / "hr" / "oj-pages" / "manifest.json"
OVERRIDES_PATH = ROOT / "raw" / "hr" / "oj-pages" / "manual_overrides.json"


def main() -> int:
    if not EAMBROSIA_INDEX.exists():
        print(f"error: {EAMBROSIA_INDEX} missing — run scripts/hr/00_fetch_data.py first",
              file=sys.stderr)
        return 1
