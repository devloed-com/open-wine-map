"""Extract noteworthy terroir facts from each HR wine's "Opis povezanosti"
(JEDINSTVENI DOKUMENT link section) using a bounded LLM layer with
dual-source grounding.

HR analog of `scripts/si/02d_extract_terroir_facts.py`. Same per-bullet
provenance, fuzzy-grounding, and cache-invalidation rules; the
differences are:

  - source = HR record's `link_to_terroir` (JEDINSTVENI DOKUMENT link
    section, straight from EUR-Lex — Croatia has no national-spec
    fallback layer wired in v1).
  - Wikipedia hint = `raw/wikipedia/aocs/hr/<slug>.json` (fetched by
    `scripts/02b_fetch_aoc_lexicon.py --lang hr --source raw/hr/dokumenti-extracted`).
  - LLM prompt is in Croatian and asks for Croatian bullets.
  - Output cache = `raw/terroir-facts/<slug>.json` (single shared dir;
    `country: "hr"` distinguishes from FR/ES/PT/IT/AT/SI records).

In v1 only Muškat momjanski and Ponikve have non-stub JEDINSTVENI
DOKUMENTI, so this stage extracts facts for those records; the other 16
wines are content-stubs.
...
WIKI_AOCS = ROOT / "raw" / "wikipedia" / "aocs" / "hr"
CACHE_DIR = ROOT / "raw" / "terroir-facts"
MANIFEST = CACHE_DIR / "manifest-hr.json"
...
# Same 4-bucket structure as FR/ES/PT/IT/AT/SI 02d, with Croatian topic
# vocabulary matched to the JEDINSTVENI-DOKUMENT "Opis povezanosti"
# content pattern.
SUBSECTIONS = [
    {
        "key": "facteurs_naturels",  # FR key kept for stage-04 render parity
        "label": "Prirodni čimbenici (geologija, tla, klima, reljef)",
        "topics": (
            "geologija, sastav tla, klima, reljef, vode, izloženost, "
            "položaji i općine"
        ),
        "max_bullets": 5,
    },
    {
        ...
        "label": "Povijesni i ljudski čimbenici (povijest, prakse)",
        "topics": (
            "povijest oznake izvornosti, vinogradarske i enološke prakse, "
            "tradicionalne sorte vinove loze"
        ),
        "max_bullets": 2,
    },
    {
        "key": "produit",
        "label": "Značajke proizvoda (senzorni profil)",
        "topics": "boje, arome, struktura, dozrijevanje, senzorni profil vina",
        "max_bullets": 2,
    },
    {
        "key": "interactions",
        "label": "Uzročne veze (poveznica terroir / vino)",
        "topics": (
            "izričita veza između terroira i karaktera vina, izražaj tla "
            "ili klime u vinu"
        ),
        "max_bullets": 1,
    },
...
# Croatian Wikipedia section names that map to each JEDINSTVENI-DOKUMENT
# sub-section as a salience hint. Croatian wine-region articles vary in
# section naming; this list is permissive.
WIKI_TO_SUBSECTION: dict[str, list[str]] = {
    "facteurs_naturels": [
        "Geografija", "Položaj", "Geologija", "Tlo", "Tla", "Klima", "Reljef",
        "Vinogorje", "Vinogradarska regija", "Krajolik", "Vinogradi",
    ],
    "facteurs_humains": [
        "Povijest", "Etimologija", "Naziv", "Sorte vinove loze", "Sorte",
        "Vinogradarstvo", "Vinarstvo", "Tradicija", "Vinska kultura",
    ],
    "produit": [
        "Vina", "Vino", "Vrste vina", "Značajke", "Senzorika",
    ],
    "interactions": [],
}
...
EXTRACT_SYSTEM = """Izdvoji vrijedna obilježja hrvatske vinske oznake izvornosti iz DVA izvora: jedinstvenog dokumenta / specifikacije proizvoda (regulatorno tijelo) i izvatka hrvatske Wikipedije (referenca za širu javnost, somelijerski vokabular).

Ciljni čitatelj: ljubitelji vina ili obrazovani somelijeri koje zanimaju konkretne posebnosti porijekla. Vrijedno spomena je: geološke formacije s njihovim stručnim imenom, posebne vrste tla (fliš, lapor, vapnenac, pješčenjak, ilovača, crljenica, crvenica, terra rossa), lokalna klima i vjetrovi (mediteranska klima, submediteranska klima, kontinentalna klima, panonski utjecaj, bura, jugo), tradicionalne sorte vinove loze, karakteristične vinogradarske prakse, precizan senzorni profil, datirani povijesni mejaši.

═══ IZVOR 1: izvadak Wikipedije (relevantni odsjeci) ═══
{wiki_hint}

═══ IZVOR 2: tekst jedinstvenog dokumenta (u korisničkoj poruci ispod) ═══

Pododjeljak za obradu: {label}
Prioritetne kategorije: {topics}

Stroga pravila:
- Svaki unos MORA biti potkrijepljen S BAREM JEDNIM citatom: `cahier_quote` (doslovno iz jedinstvenog dokumenta) ILI `wiki_quote` (doslovno iz gornjeg izvatka Wikipedije). Oba se vrlo preporučuju kad oba izvora pokrivaju istu informaciju.
- Daj prednost jedinstvenom dokumentu kao primarnom izvoru: koristi `cahier_quote` kad god se činjenica tamo pojavljuje.
- Dodaj `wiki_quote` kad Wikipedija nudi standardni stručni vokabular za činjenicu koju jedinstveni dokument opisuje svojim riječima.
- Ako vrijednu činjenicu navodi samo Wikipedija, smiješ je zadržati samo s `wiki_quote` — pri prikazu će biti pripisana Wikipediji.
- Citati su DOSLOVNI (kopirani) iz odgovarajućeg izvora. NIKAD ne pripisuj izvoru tekst koji se tamo ne pojavljuje.
- Bez vrijednosnih sudova ("izvanredan", "prestižan" ...).
- Bez vanjskih zaključaka. Bez brojki kojih nema ni u jednom od dva izvora.
- Najviše {max_bullets} unosa; svaki unos je jedna potpuna rečenica od otprilike 120–220 znakova — nikada telegrafski fragment.
- Ako ni jedinstveni dokument ni Wikipedija ne sadrže konkretnu vrijednu činjenicu za ovaj pododjeljak, vrati prazan popis.

Odgovori SAMO u JSON-u, bez teksta prije ili poslije:
{{"facts": [{{"bullet": "...", "cahier_quote": "...", "wiki_quote": "..."}}, ...]}}
Za citat koji nedostaje koristi prazan niz ""."""
...
def _wiki_hint_for_subsection(
    ...
    """Pull the relevant Wikipedia HR section text for a sub-section.
...
def _resolve_lien_and_source(rec: dict) -> tuple[str, dict]:
    """Resolve the terroir-source text for one HR record.

    Preference:
      1. EU Jedinstveni dokument's `link_to_terroir` (Muškat momjanski +
         Ponikve — the only 2 HR wines with a real EU-OJ single document)
      2. MPS specifikacija sidecar's `link_to_terroir` (lettered section
         g) "…povezane sa zemljopisnim uvjetima" — 15 of 16 augmented
         stubs; sidecar at raw/hr/specifikacije-extracted/<slug>.json).

    The Primorska Hrvatska docx has empty `link_to_terroir` (its Word
    auto-numbered outline drops the lettered g) heading), so it is
    skipped by the MIN_LIEN_CHARS guard in collect_targets."""
    ...
    if sidecar_path.exists():
        ...
        if sidecar:
            ...
            if len(spec_lien) >= MIN_LIEN_CHARS:
                ...
                return spec_lien, {"pdf_url": spec_url, "kind": "mps-specifikacija"}
    eu_url = src.get("final_url") or src.get("source_url") or ""
    return lien, {"pdf_url": eu_url, "kind": "eu-oj"}
...
def collect_targets() -> list[dict]:
    """Return HR records with a long-enough link_to_terroir. Croatia has
...
def _ask_line(label: str) -> str:
    return f"Pododjeljak za obradu: {label}"


def _document_block(lien_text: str) -> str:
    return f"Tekst jedinstvenog dokumenta (Opis povezanosti):\n\n{lien_text}"
...
def _process_subsection(
    ...
    system = EXTRACT_SYSTEM.format(
        wiki_hint=wiki_hint or "(izvadak Wikipedije nije dostupan)",
        label=sub["label"],
        topics=sub["topics"],
...
def emit_todo(out_path: Path, *, skip_cached: bool, limit: int = 0) -> int:
    ...
    print(
        f"[02d/hr] wrote {out_path} ({len(items)} items across "
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    for slug, slug_items in by_slug.items():
        ...
        counts[outcome] += 1
    print(
        f"[02d/hr] wrote {counts['wrote']} cache files; "
...
def _run_batch(args) -> int:
    """Extract terroir facts for every HR wine via the provider Batch API."""
    ...
    if not targets:
        print("[02d/hr] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02d/hr] batch: {len(targets)} wines (provider={args.provider}, "
    ...
    def run_loop(prov):
        ...
        if getattr(prov, "kind", "") == "collecting":
            keep = CACHE_DIR
            CACHE_DIR = Path(tempfile.mkdtemp(prefix="batch-02d-hr-"))
            try:
                for rec in targets:
    ...
    batch.run_two_pass(
        ...
        sidecar=ROOT / "raw" / ".batch" / "02d-hr.json",
...
def main() -> int:
    ...
    if not EXTRACTED.exists():
        print(
            f"error: {EXTRACTED} missing — run scripts/hr/02_extract_pliegos.py first",
            file=sys.stderr,
        )
        return 1
    if not WIKI_AOCS.exists():
        print(
            ...
            "Run scripts/02b_fetch_aoc_lexicon.py --lang hr "
            "--source raw/hr/dokumenti-extracted to enable Wikipedia salience.",
            file=sys.stderr,
        )
    ...
    terroir_verbatim.emit_for_country(
        country="hr", extracted_dir=EXTRACTED, cache_dir=CACHE_DIR,
        default_source_lang="hr", cahier_source_kind="eu-oj",
        only=args.only, log_prefix="[02d/hr]",
    )
    ...
    if not targets:
        print("[02d/hr] nothing to do — all caches valid.", file=sys.stderr)
        return 0
    ...
    if provider is None:
        print(
            f"[02d/hr] manual provider: {len(targets)} wines need extraction. "
    ...
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    print(
        f"[02d/hr] {len(targets)} HR wines to extract "
    ...
    t0 = time.time()
    if workers <= 1:
        for rec in tqdm(targets, desc="terroir-hr", leave=False):
        ...
        with ThreadPoolExecutor(max_workers=workers) as ex:
            ...
            for fut in tqdm(as_completed(futures), total=len(targets), desc="terroir-hr", leave=False):
                try:
                    res = fut.result()
                    n_done += 1
                    n_facts += len(res.get("facts", []))
    ...
    print(
        f"[02d/hr] done: {n_done} wines, {n_facts} facts, "
