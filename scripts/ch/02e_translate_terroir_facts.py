"""Translate CH per-AOC terroir-fact bullets into the missing locales.

CH analog of `scripts/at/02e_translate_terroir_facts.py` with one delta:
the source language is **per-record** (fr / de / it depending on canton),
so the target-locale set per record is `{en, fr, es, nl} - {source_lang}`
rather than a single fixed source language.

Reads:  raw/terroir-facts/<slug>.json  (where country == 'ch')
...
SOURCE_LANG_NAME = {"fr": "French", "de": "German", "it": "Italian"}


# Per source-lang system prompts. Each preserves the relevant Swiss
# wine vocabulary in the source script (FR for the Romandie canton
# wines; DE for the Deutschschweiz; IT for Ticino).
SYSTEM_PROMPT_TEMPLATE = """You translate short {source_lang_name} bullets describing a Swiss wine appellation's terroir, history, and wine character into {lang_name}. The bullets come from the AOC's Wikipedia article + cantonal-règlement context, aimed at sommelier students and wine enthusiasts.
...
def _load_ch_source(path: Path) -> dict | None:
...
def enumerate_jobs(
    ...
    """For each CH terroir-fact record, target = languages \\ source_lang."""
    jobs: list[dict] = []
    for f in sorted(TERROIR_FACTS.glob("*.json")):
        if f.name.startswith("_") or f.name.startswith("manifest"):
            continue
        src = _load_ch_source(f)
        if src is None:
            continue
        source_lang = src.get("source_lang") or "fr"
        src_facts = src["facts"]
        sha = facts_sha(src_facts)
        for lang in languages:
            if lang == source_lang:
                continue  # source bullets are already in this locale
...
def _import_one(it: dict, src_index: dict, *,
                translator_id: str, translator_kind: str) -> str:
    slug = it.get("slug") or ""
    lang = it.get("lang") or ""
    ...
    write_cache(
        lang=lang, slug=slug,
        source_lang=it.get("source_lang") or src.get("source_lang") or "fr",
        src_facts=src_facts,
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    for it in payload.get("items") or []:
        outcome = _import_one(
            it, src_index, translator_id=translator_id,
            translator_kind=translator_kind,
        )
        counts[outcome] += 1
    pretty = ", ".join(f"{k}={v}" for k, v in counts.items() if v)
    print(f"[02e/ch] {pretty}", file=sys.stderr)
    return 0
...
def _run_translation_loop(
    ...
    if workers <= 1:
        for job in tqdm(jobs, desc="translate-ch", leave=False):
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in tqdm(as_completed(futures), total=len(jobs), desc="translate-ch", leave=False):
            job = futures[fut]
            try:
                ok, err = fut.result()
            except Exception as e:  # noqa: BLE001
...
def _run_batch(args, languages: tuple[str, ...]) -> int:
    ...
    if not jobs:
        print("[02e/ch] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02e/ch] batch: {len(jobs)} translations (provider={args.provider}, "
          f"model={model_id}, locales={','.join(languages)})", file=sys.stderr)
    batch.run_two_pass(
        provider=args.provider, model=model_id,
        sidecar=ROOT / "raw" / ".batch" / "02e-ch.json",
...
def _dispatch_emit_or_import(args, languages: tuple[str, ...]) -> int | None:
    ...
    if rc is not None:
        return rc
    if args.emit_todo:
        return emit_todo(Path(args.emit_todo), languages=languages,
...
def main() -> int:
    ...
    if not TERROIR_FACTS.exists():
        print("error: raw/terroir-facts is missing — run "
              "scripts/ch/02d_extract_terroir_facts.py first", file=sys.stderr)
        return 1
    ...
    if args.limit:
        jobs = jobs[: args.limit]
    if not jobs:
        print("[02e/ch] nothing to do — all caches up to date.", file=sys.stderr)
        return 0
    ...
    if provider is None:
        ...
        print(f"[02e/ch] manual provider: {len(jobs)} entries need translation.",
              file=sys.stderr)
        return 1

    workers = max(1, args.workers)
    print(f"[02e/ch] {len(jobs)} translations to fetch "
    ...
    print(f"[02e/ch] translated: {done}, skipped: {len(skipped)}", file=sys.stderr)
    if skipped:
