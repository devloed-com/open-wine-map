"""Audit MT pipeline coverage.

Prints extraction status per kind, stub reasons, Figshare geometry
coverage, grape + terroir-fact extraction, and a curator queue for any
wine whose single document could not be fetched. Mirrors
`audit_cz_coverage.py` for the Maltese corpus.

Run: `.venv/bin/python scripts/audit_mt_coverage.py`
"""
...
EAMBROSIA = ROOT / "raw" / "mt" / "eambrosia" / "index.json"
EXTRACTED = ROOT / "raw" / "mt" / "dokumente-extracted"
OJ_MANIFEST = ROOT / "raw" / "mt" / "oj-pages" / "manifest.json"
OVERRIDES = ROOT / "raw" / "mt" / "oj-pages" / "manual_overrides.json"
...
def _load_figshare_file_numbers() -> set[str]:
    ...
    try:
        ...
        return set(
            ...
            if fn and fn.startswith(("PDO-MT", "PGI-MT"))
        )
    except ImportError:
        return set()


def _terroir_fact_counts() -> dict[str, int]:
    out: dict[str, int] = {}
...
def main() -> int:
    ...
    facts = _terroir_fact_counts()

    print("# MT pipeline audit\n")
    ...
    print(f"Figshare MT PDOids:   {len(figshare_ids)}")
    print()
    ...
    print(f"  Missing from Figshare: {no_figshare} "
          f"(the PGI resolves via region-pdo-union)")
    print()
    ...
    print("## Terroir facts")
    if facts:
        for slug, n in sorted(facts.items()):
            print(f"  {n:>3}  {slug}")
    else:
        print("  (none yet — run scripts/mt/02d_extract_terroir_facts.py)")
    print()
