...
for the Czech corpus.

Run: `.venv/bin/python scripts/audit_cz_coverage.py`
"""
...
EAMBROSIA = ROOT / "raw" / "cz" / "eambrosia" / "index.json"
EXTRACTED = ROOT / "raw" / "cz" / "dokumenty-extracted"
OJ_MANIFEST = ROOT / "raw" / "cz" / "oj-pages" / "manifest.json"
OVERRIDES = ROOT / "raw" / "cz" / "oj-pages" / "manual_overrides.json"
...
def _load_figshare_file_numbers() -> set[str]:
    ...
    try:
        ...
        return set(
            ...
            if fn and fn.startswith(("PDO-CZ", "PGI-CZ"))
        )
    except ImportError:
        return set()
...
def main() -> int:
    ...
    print(f"Figshare CZ PDOids:   {len(figshare_ids)}")
    print()
    ...
    if not curator_targets:
        print("  (empty — every Czech wine extracted)")
    print()
    ...
    try:
        ...
        _vb_count, _vb_records = _verbatim.count_verbatim_records(
            Path(__file__).resolve().parent.parent / "raw" / "terroir-facts", "cz",
        )
        if _vb_count:
