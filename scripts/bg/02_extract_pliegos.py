"""Extract structured fields from each cached EUR-Lex Единен документ HTML.

Pipeline stage 02 (bg).

For each Bulgarian wine GI in `raw/bg/eambrosia/index.json`:
  - if a cached HTML exists at `raw/bg/oj-pages/<slug>.html`, parse the
    "ЕДИНЕН ДОКУМЕНТ" block into numbered sections (the EU wine GI
    template) and route them by Bulgarian title keyword
  - else emit a content-stub so the wine remains searchable

In addition to the standard role-routed sections, the BG extractor
harvests the **obshtina list** from the section-6 area body
(`geo_communes`) so stage 04's `BGPolygonIndex.commune_union` can
resolve geometry against the shared GISCO LAU `BG_*` index when
necessary. This is the same shape as the RO commune-list fallback.

Output: one JSON per wine under `raw/bg/dokumenti-extracted/<slug>.json`,
plus a `_index.json` mapping slug → metadata.

v1 models the 54 Bulgarian wine GIs as a flat corpus — the 5 wine
regions (винарски район) are preserved via the `region` facet rather
than as parent/sub-denomination records. The 2 macro PGIs (Дунавска
равнина / Тракийска низина) are first-class records, not parents to
sub-denominations.
"""
...
from _lib.bg.region import derive_region  # noqa: E402
from _lib.bg.commune import parse_commune_list  # noqa: E402
...
OUT_DIR = ROOT / "raw" / "bg" / "dokumenti-extracted"
...
_TOP_LEVEL_ROLE_TITLE_KEYWORDS: tuple[str, ...] = tuple(
    kw for kws in SECTION_ROLE_KEYWORDS.values() for kw in kws
)


def _title_matches_role(title: str) -> bool:
    low = title.casefold()
    return any(kw in low for kw in _TOP_LEVEL_ROLE_TITLE_KEYWORDS)


def extract_sections(html: str) -> tuple[dict[str, str], dict[str, str]]:
    """Walk section headers in document order. Bulgarian publications
    nest sub-section headers inside section 4 (Описание на виното или
    вината) for per-style / per-variety entries — those nested
    `ti-grseq-1` headers are numbered `1.` / `2.` / … and would shadow
    the real sections 5–9 if we deduped naively. Two filters:
      ...
      2. Title match: a top-level header's title must contain at least
         one known section-role keyword (Наименование / Категории /
         Описание / Винопроизводствени практики / Определен географски
         район / Винен сорт / Описание на връзката / Други специфични
         изисквания). Per-variety subsections fail this filter."""
...
# Bulgarian grape-variety sections enumerate varieties separated by
# em-dash bullets, em-dash hyphens, newlines, or numbered lists.
_BULLET_SPLIT_RE = re.compile(r"\s*[—•·]\s*|\n|\s*\d+\.\s+")
# Name — Latin-synonym separator: hyphen-with-spaces. `Мерло - Merlot`
# → Мерло (the canonical name) + Merlot (the synonym, dropped).
...
def _item_candidates(item: str) -> list[str]:
    """Variety-name candidates: canonical Bulgarian name first, then any
...
def parse_grapes(section_text: str) -> dict:
    ...
    observation, details}. Bulgarian DOCUMENT UNIC publications generally
    do not split principal/accessory in the EU single document
    (section 7 lists varieties without role markers); we default to
    `principal` for every match. Unmatched names flow into
    `raw/bg/extraction-unknowns.json`."""
    out: dict[str, list] = {
        "principal": [],
        "accessory": [],
        "observation": [],
    ...
    for raw_item in _grape_items(section_text):
        ...
        low = item.casefold()
        if any(d in low for d in _GRAPE_LINE_DROP) and len(item) < 55:
            continue
...
def parse_styles(sections: dict[str, str], titles: dict[str, str]) -> list[str]:
    ...
    for num, body in sections.items():
        ...
        if (
            "описание на вин" in title_low
            or "категории" in title_low
            or "други специфични" in title_low
            or "други условия" in title_low
        ):
...
def build_record(wine: dict, sections: dict[str, str], titles: dict[str, str],
    ...
    return {
        ...
        "slug": wine["slug"],
        "name": wine["name"],
        "name_latin": _latin_form(wine["name"]),
        "kind": wine["kind"],
...
def _extract_from_html(cache: Path) -> tuple[dict[str, str], dict[str, str], str]:
    ...
    if doc is None:
        return {}, {}, "no-edinen-dokument-anchor"
...
def main() -> int:
    ...
    if not INDEX_IN.exists():
        print(f"error: {INDEX_IN} missing — run scripts/bg/00_fetch_data.py first",
              file=sys.stderr)
        return 1
    ...
    unknowns_path = ROOT / "raw" / "bg" / "extraction-unknowns.json"
