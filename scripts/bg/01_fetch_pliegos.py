"""Fetch the EU-OJ "ЕДИНЕН ДОКУМЕНТ" HTML page per BG wine GI.

Pipeline stage 01 (bg).

For each wine in `raw/bg/eambrosia/index.json`, scan the `publications`
array for the first HTTP URL and request the Bulgarian-language variant
of that EUR-Lex page. The page contains the canonical EU single document
("ЕДИНЕН ДОКУМЕНТ") — the Bulgarian equivalent of the FR cahier des
charges, the ES pliego, the IT documento unico, the AT Einziges Dokument,
the SI Enotni dokument, the HR Jedinstveni dokument, the HU Egységes
dokumentum, and the RO DOCUMENT UNIC.

Coverage:
  Only ~3 of the 54 BG wines carry a fetchable OJ publication URL — the
  other ~51 are Art.107 / Reg.1308/2013 grandfathered names with no
  public single-document URL. They land as `no-publication` and stage
  02 emits content-stubs (the IT/ES/SI/HR/HU/RO curator-queue pattern).
  The other failure mode is the EUR-Lex AWS WAF (HTTP 202 + JavaScript
  challenge); `scripts/bg/01b_solve_waf.py` bootstraps the blocked
  subset via headless Chromium.

Bulgarian variant rules (same as ES/IT/AT/SI/HR/HU/RO, with the language swap):
  /eli/.../oj  →  /eli/.../oj/bul  (3-letter ISO 639-2/T for Bulgarian)
  legal-content/EN/TXT/  →  legal-content/BG/TXT/HTML/
  uri=...01.ENG&...     →  uri=...01.BUL&...

Manual overrides:
  raw/bg/oj-pages/manual_overrides.json (gitignored, optional) keyed by
  giIdentifier or slug → {"url": "<bulgarian-html-url>", "note": "..."}.
  Overrides bypass the eAmbrosia publications list entirely.

Outputs:
  raw/bg/oj-pages/<slug>.html          (the Bulgarian Единен документ HTML)
  raw/bg/oj-pages/manifest.json        (per-slug status + final URL)
...
INDEX_PATH = ROOT / "raw" / "bg" / "eambrosia" / "index.json"
OUT_DIR = ROOT / "raw" / "bg" / "oj-pages"
...
def to_bulgarian(url: str) -> str:
    """Best-effort URL rewrite: any EUR-Lex / EU-OJ URL → its Bulgarian
    HTML variant. Idempotent — already-Bulgarian URLs pass through."""
    if re.search(r"/oj/?(?:[#?]|$)", url) and not re.search(r"/oj/[a-z]{3}", url):
        return re.sub(r"/oj(/?)([#?]|$)", r"/oj/bul\2", url)
    new = re.sub(
        r"/legal-content/[A-Z]{2}/TXT(?:/HTML)?/",
        "/legal-content/BG/TXT/HTML/",
        url,
    )
    new = re.sub(
        r"(\d{2}\.)(?:ENG|FRA|DEU|ITA|POR|SPA|NLD|SLV|HRV|HUN|RON|BUL)(?=&|$)",
        r"\1BUL", new,
    )
    new = re.sub(
        r"(:)(?:ENG|FRA|DEU|ITA|POR|SPA|NLD|SLV|HRV|HUN|RON|BUL)(?=&|$)",
        r"\1BUL", new,
    )
    return new
...
def _is_corrigendum(pub: dict) -> bool:
    text = (pub.get("text") or "").lower()
    return "corrigendum" in text or "поправка" in text
...
def fetch_page(session: requests.Session, url: str) -> requests.Response | None:
    """Fetch with 202-aware retry. EUR-Lex generates `legal-content/BG/TXT/HTML`
...
def looks_like_single_document(html: str) -> bool:
    ...
    publication. The Единен документ HTML is large (>8 KB) and contains
    literal Bulgarian phrases."""
    if len(html) < 8000:
        return False
    low = html.lower()
    return (
        "единен документ" in low
        or "защитено наименование за произход" in low
        or "защитено географско указание" in low
        or "географско указание" in low
        or "географско означение" in low
        or "наименование за произход" in low
        or "продуктова спецификация" in low
    )
...
def main() -> int:
    ...
    if not INDEX_PATH.exists():
        print(f"error: {INDEX_PATH} missing — run scripts/bg/00_fetch_data.py first",
              file=sys.stderr)
        return 1
    ...
    for w in tqdm(wines, desc="oj-pages", leave=False):
        ...
        if override and override.get("url"):
            ...
            source_url = to_bulgarian(base) if base else None
