...
- Максимум {max_bullets} елемента; всеки елемент е пълно изречение от около 120–220 знака — никога телеграфен фрагмент.
...
def _ask_line(label: str) -> str:
    return f"Подсекция за обработка: {label}"


def _document_block(lien_text: str) -> str:
    return f"Текст на единния документ (Описание на връзката):\n\n{lien_text}"
...
def _process_record(provider, model_id: str, record: dict) -> dict:
    ...
    earned = earn_interactions(all_facts, "bg")
...
def main() -> int:
    ...
    terroir_verbatim.emit_for_country(
        country="bg", extracted_dir=EXTRACTED, cache_dir=CACHE_DIR,
        default_source_lang="bg", cahier_source_kind="eu-oj",
        only=args.only, log_prefix="[02d/bg]",
    )
