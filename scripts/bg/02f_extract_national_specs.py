"""Extract the Bulgarian national продуктова спецификация for each BG
content-stub. Sibling of stage 02 — covers the 51 grandfathered BG wines
whose EU-OJ ЕДИНЕН ДОКУМЕНТ was never published.

Pipeline stage 02f (bg). Modelled on `scripts/hr/02f_extract_specifikacije.py`
and the ES MAPA / IT MASAF / DE BLE / SI–HR specifikacije national-spec
pattern: pulls the regulator's canonical per-wine product specification from
ИАЛВ / IAVV (eavw.com) and writes one sidecar JSON that stage 04's
`augment_bg_records_with_national_specs()` merges into each in-memory stub.

Reads:
  raw/bg/national-specs/<slug>.{pdf,doc,docx,html}   (fetched by stage 01c)
  raw/bg/national-specs/manifest.json
  raw/bg/national-specs/manual_overrides.json        (slug → url mapping)

Writes:
  raw/bg/national-specs-extracted/<slug>.json        (sidecar)
  raw/bg/national-specs-extracted/_index.json
  raw/bg/national-specs-extracted/manifest.json
  raw/bg/extraction-unknowns-specifikacije.json       (unknown grape candidates)

The IAVV specs are all PDF (`pdftotext -layout`); the .doc/.docx branches are
kept for robustness (a curator-pinned replacement may be a Word doc). All
branches feed `_lib.bg.specifikacija.parse_specifikacija`.
...
OVERRIDES_PATH = ROOT / "raw" / "bg" / "national-specs" / "manual_overrides.json"
SPECS_DIR = ROOT / "raw" / "bg" / "national-specs"
SPECS_MANIFEST = SPECS_DIR / "manifest.json"
OUT_DIR = ROOT / "raw" / "bg" / "national-specs-extracted"
OUT_INDEX = OUT_DIR / "_index.json"
OUT_MANIFEST = OUT_DIR / "manifest.json"
UNKNOWNS = ROOT / "raw" / "bg" / "extraction-unknowns-specifikacije.json"
DOCKER_IMAGE = "owm-antiword:latest"

_LICENSE = (
    "ИАЛВ / IAVV продуктова спецификация (Bulgaria, eavw.com) — official act of "
    "a state administration body, ЗАПСП Art. 4 copyright exemption; reuse with "
    "attribution to ИАЛВ."
)
...
def main() -> int:
    ...
    for slug in tqdm(targets, desc="bg-specifikacije-extract", leave=False):
        ...
        source = {
            ...
            "source_org": overrides.get(slug, {}).get("source_org", "iavv"),
            "license": _LICENSE,
        }
