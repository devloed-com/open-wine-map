...
SYSTEM_PROMPT = """You translate short Bulgarian bullets describing a Bulgarian wine appellation's terroir, history, and wine character into {lang_name}. The bullets come from the EU "Единен документ" / продуктова спецификация and are aimed at sommelier students and wine enthusiasts.
...
def emit_todo(out_path: Path, *, languages: tuple[str, ...], skip_cached: bool) -> int:
    ...
    print(f"[02e/bg] wrote {out_path} ({len(items)} items: {pretty})", file=sys.stderr)
    return 0
...
def import_todo(in_path: Path, *, translator_id: str, translator_kind: str) -> int:
    ...
    print(f"[02e/bg] {pretty}", file=sys.stderr)
    return 0
...
def _check_inputs() -> int:
    if not TERROIR_FACTS.exists():
        print(
            ...
            "scripts/bg/02d_extract_terroir_facts.py first",
            file=sys.stderr,
        )
        return 1
    return 0
...
def _run_batch(args, languages: tuple[str, ...]) -> int:
    ...
    if not jobs:
        print("[02e/bg] batch: nothing to do.", file=sys.stderr)
        return 0
    print(f"[02e/bg] batch: {len(jobs)} translations (provider={args.provider}, "
          f"model={model_id}, locales={','.join(languages)})", file=sys.stderr)
    batch.run_two_pass(
        provider=args.provider, model=model_id,
        sidecar=ROOT / "raw" / ".batch" / "02e-bg.json",
...
def main() -> int:
    ...
    if not jobs:
        print("[02e/bg] nothing to do — all caches up to date.", file=sys.stderr)
        return 0
    ...
    if provider is None:
        ...
        print(
            f"[02e/bg] manual provider: {len(jobs)} entries need translation. "
    ...
    print(
        f"[02e/bg] {len(jobs)} translations to fetch "
    ...
    print(f"[02e/bg] translated: {done}, skipped: {len(skipped)}", file=sys.stderr)
    if skipped:
        for lang, slug, err in skipped[:20]:
            print(f"  skip {lang}/{slug}: {err[:160]}", file=sys.stderr)
        print("[02e/bg] rerun this stage to retry the skipped entries.", file=sys.stderr)
    return 0
