"""Generate one wiki/<slug>.md per BG wine record + extend
wiki/_index.json with BG entries.

Pipeline stage 03 (bg). Mirrors `scripts/ro/03_generate_wiki.py` for the
BG corpus. Reads `raw/bg/dokumenti-extracted/*.json`, emits per-record
markdown pages with Bulgarian section headings, and merges BG entries
...
SECTION_LABELS = {
    "summary": "Резюме",
    "geo": "Определен географски район",
    "grapes": "Сортове грозде",
    "link": "Описание на връзката",
    "note": "Бележка",
    "sources": "Източници",
}
...
def render_record(record: dict) -> str:
    ...
    if record.get("stub"):
        body += [
            "> ⚠️ **Единният документ не е достъпен.** "
            f"Причина: `{record.get('stub_reason') or 'unknown'}`. "
            "Наименованието остава в индекса; съдържанието ще бъде "
            "добавено, когато бъде налична публична продуктова "
            "спецификация.",
            "",
        ]
    ...
    body += [
        f"## {SECTION_LABELS['sources']}",
        "",
        f"- EUR-Lex (ЕДИНЕН ДОКУМЕНТ): <{src.get('final_url') or src.get('source_url') or ''}>",
        ...
        f"- File number: `{record.get('file_number') or ''}`",
        "",
        "_Текст на единния документ: © Европейски съюз / EUR-Lex. "
        "Преизползване с посочване на източника._",
        "",
    ]
    return "\n".join(fm + body)
...
def main() -> int:
    if not EXTRACTED.exists():
        print(f"error: {EXTRACTED} missing — run scripts/bg/02_extract_pliegos.py first",
              file=sys.stderr)
        return 1
    ...
    written = 0
    bg_index: dict[str, dict] = {}
    for f in tqdm(files, desc="bg-wiki", leave=False):
        rec = json.loads(f.read_text(encoding="utf-8"))
        out_path = WIKI / f"{rec['slug']}.md"
        out_path.write_text(render_record(rec), encoding="utf-8")
        bg_index[rec["slug"]] = index_entry(rec)
        written += 1
    ...
    if WIKI_INDEX.exists():
        try:
            ...
            existing = {}
    other_kept = {k: v for k, v in existing.items() if v.get("country") != "bg"}
    merged = {**other_kept, **bg_index}
    WIKI_INDEX.write_text(json.dumps(merged, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
    print(
        f"[bg/03] wrote {written} BG wiki pages, merged index "
        f"({len(other_kept)} non-BG + {len(bg_index)} BG = {len(merged)} entries) "
