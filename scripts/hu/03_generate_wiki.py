"""Generate one wiki/<slug>.md per HU wine record + extend
wiki/_index.json with HU entries.

Pipeline stage 03 (hu). Mirrors `scripts/hr/03_generate_wiki.py` for the
HU corpus. Reads `raw/hu/dokumentumok-extracted/*.json`, emits per-record
markdown pages with Hungarian section headings, and merges HU entries
into `wiki/_index.json` (preserving any pre-existing FR/ES/PT/IT/AT/SI/HR
entries).

A curated cross-border note is rendered as a `## Megjegyzés` section
when `scripts/_lib/appellation_notes.json` carries an entry for the
slug — see that file for the sourced-annotation contract.
"""
...
def render_record(record: dict) -> str:
    ...
    if record.get("stub"):
        body += [
            "> ⚠️ **Az egységes dokumentum nem elérhető.** "
            f"Ok: `{record.get('stub_reason') or 'unknown'}`. "
            "A megnevezés szerepel a tárgymutatóban; a tartalom akkor "
            "kerül kiegészítésre, ha a termékleírás nyilvános forrása "
            "elérhetővé válik.",
            "",
        ]
    ...
    body += [
        f"## {SECTION_LABELS['sources']}",
        "",
        f"- EUR-Lex (EGYSÉGES DOKUMENTUM): <{src.get('final_url') or src.get('source_url') or ''}>",
        ...
        f"- File number: `{record.get('file_number') or ''}`",
        "",
        "_Az egységes dokumentum szövege: © Európai Unió / EUR-Lex. "
        "Újrafelhasználás a forrás megjelölésével engedélyezett._",
        "",
    ]
    return "\n".join(fm + body)
...
def main() -> int:
    if not EXTRACTED.exists():
        print(f"error: {EXTRACTED} missing — run scripts/hu/02_extract_pliegos.py first",
              file=sys.stderr)
        return 1
    ...
    written = 0
    hu_index: dict[str, dict] = {}
    for f in tqdm(files, desc="hu-wiki", leave=False):
        rec = json.loads(f.read_text(encoding="utf-8"))
        out_path = WIKI / f"{rec['slug']}.md"
        out_path.write_text(render_record(rec), encoding="utf-8")
        hu_index[rec["slug"]] = index_entry(rec)
        written += 1
    ...
    if WIKI_INDEX.exists():
        try:
            ...
            existing = {}
    other_kept = {k: v for k, v in existing.items() if v.get("country") != "hu"}
    merged = {**other_kept, **hu_index}
    WIKI_INDEX.write_text(json.dumps(merged, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
    print(
        f"[hu/03] wrote {written} HU wiki pages, merged index "
        f"({len(other_kept)} non-HU + {len(hu_index)} HU = {len(merged)} entries) "
