"""One-off bootstrap: fetch the EUR-Lex Egységes-dokumentum HTMLs that
`scripts/hu/01_fetch_pliegos.py` couldn't reach because EUR-Lex sits
...
Run: `.venv/bin/python scripts/hu/01b_solve_waf.py`
After: re-run `scripts/hu/02_extract_pliegos.py` to pick up the new HTMLs.
"""
...
def to_hungarian(url: str) -> str:
    ...
    new = re.sub(r"/legal-content/[A-Z]{2}/TXT(?:/HTML)?/", "/legal-content/HU/TXT/HTML/", url)
...
def main() -> int:
    ...
    if not MANIFEST_PATH.exists():
        print(f"error: {MANIFEST_PATH} missing — run scripts/hu/01_fetch_pliegos.py first",
              file=sys.stderr)
        return 1
    ...
    for w in wines:
        ...
        if override and override.get("url"):
            ...
            url = to_hungarian(base) if base else None
        if not url:
            continue
