"""Regenerate the manual_overrides.json template for SK wines whose
single-document URL is not auto-discovered / not fetchable.

Reads:
  raw/sk/eambrosia/index.json — full SK wine list
  raw/sk/oj-pages/manifest.json — stage 01/01b fetch outcomes per wine

Writes:
  raw/sk/oj-pages/manual_overrides.json — one entry per wine that needs
...
6 of the 10 Slovak wines are Art.107/Reg.1308/2013 grandfathered names
with no public single-document URL — they dominate this queue. The
curator fills `url` with a public, licence-clear specification
(EUR-Lex OJ-C page, or the MPRV national špecifikácia výrobku once a
parser branch exists). After filling URLs, re-run
scripts/sk/01_fetch_pliegos.py → 02_extract_pliegos.py → stage 04.
"""
...
EAMBROSIA_INDEX = ROOT / "raw" / "sk" / "eambrosia" / "index.json"
OJ_MANIFEST = ROOT / "raw" / "sk" / "oj-pages" / "manifest.json"
OVERRIDES_PATH = ROOT / "raw" / "sk" / "oj-pages" / "manual_overrides.json"


def main() -> int:
    if not EAMBROSIA_INDEX.exists():
        print(f"error: {EAMBROSIA_INDEX} missing — run scripts/sk/00_fetch_data.py first",
              file=sys.stderr)
        return 1
    ...
    out["__doc__"] = (
        "Curator-filled overrides for SK wines whose single-document "
        "URL is not auto-discovered or not fetchable. Fill the `url` "
        "field for each entry to point at a public, licence-clear "
        "specification (EUR-Lex OJ-C page, or the MPRV / Slov-Lex "
        "national specification). Stage 01 reads this file before "
        "falling back to eAmbrosia publications. Re-run "
        "scripts/sk/01_fetch_pliegos.py after editing. Entries with "
