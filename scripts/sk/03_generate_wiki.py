"""Generate one wiki/<slug>.md per SK wine record + extend
wiki/_index.json with SK entries.

Pipeline stage 03 (sk). Mirrors `scripts/si/03_generate_wiki.py` for
the Slovak corpus. Reads `raw/sk/dokumenty-extracted/*.json`, emits
per-record markdown pages with Slovak section headings, and merges SK
entries into `wiki/_index.json` (preserving any pre-existing entries
...
SECTION_LABELS = {
    "summary": "Zhrnutie",
    "geo": "Vymedzená zemepisná oblasť",
    "grapes": "Muštové odrody",
    "link": "Súvislosť so zemepisnou oblasťou",
    "note": "Poznámka",
    "sources": "Zdroje",
}
...
def render_record(record: dict) -> str:
    ...
    if record.get("stub"):
        body += [
            "> ⚠️ **Jednotný dokument nie je k dispozícii.** "
            f"Dôvod: `{record.get('stub_reason') or 'unknown'}`. "
            "Názov zostáva v zozname; obsah bude doplnený, keď bude "
            "k dispozícii verejný zdroj špecifikácie výrobku.",
            "",
        ]
    ...
    body += [
        f"## {SECTION_LABELS['sources']}",
        "",
        f"- EUR-Lex (JEDNOTNÝ DOKUMENT): <{src.get('final_url') or src.get('source_url') or ''}>",
        ...
        f"- File number: `{record.get('file_number') or ''}`",
        "",
        "_Text jednotného dokumentu: © Európska únia / EUR-Lex. "
        "Opätovné použitie s uvedením zdroja._",
        "",
    ]
    return "\n".join(fm + body)
...
def main() -> int:
    if not EXTRACTED.exists():
        print(f"error: {EXTRACTED} missing — run scripts/sk/02_extract_pliegos.py first",
              file=sys.stderr)
        return 1
    ...
    written = 0
    sk_index: dict[str, dict] = {}
    for f in tqdm(files, desc="sk-wiki", leave=False):
        rec = json.loads(f.read_text(encoding="utf-8"))
        out_path = WIKI / f"{rec['slug']}.md"
        out_path.write_text(render_record(rec), encoding="utf-8")
        sk_index[rec["slug"]] = index_entry(rec)
        written += 1
    ...
    if WIKI_INDEX.exists():
        try:
            ...
            existing = {}
    other_kept = {k: v for k, v in existing.items() if v.get("country") != "sk"}
    merged = {**other_kept, **sk_index}
    WIKI_INDEX.write_text(json.dumps(merged, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
    print(
        f"[sk/03] wrote {written} SK wiki pages, merged index "
        f"({len(other_kept)} non-SK + {len(sk_index)} SK = {len(merged)} entries) "
