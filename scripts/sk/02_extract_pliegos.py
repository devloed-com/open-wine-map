...
Pipeline stage 02 (sk).

For each Slovak wine GI in `raw/sk/eambrosia/index.json`:
  - if a cached HTML exists at `raw/sk/oj-pages/<slug>.html`, parse the
    "JEDNOTNÝ DOKUMENT" block into numbered sections (the EU wine GI
    template) and route them by Slovak title keyword
  - else emit a stub record so the wine remains searchable

Output: one JSON per wine under `raw/sk/dokumenty-extracted/<slug>.json`,
plus a `_index.json` mapping slug → metadata.

v1 models the 10 Slovak wine GIs as a flat corpus — vinohradnícke
rajóny (sub-districts of an oblasť) are deferred to Phase 2 (gated on
the national-specification research), mirroring how Austria / Slovenia
shipped flat.
"""
...
from _lib.sk.region import derive_region  # noqa: E402
from _lib.grape_entity import (  # noqa: E402
    flush_unknowns_queue, match_variety, set_pliego_context,
)

INDEX_IN = ROOT / "raw" / "sk" / "eambrosia" / "index.json"
OJ_DIR = ROOT / "raw" / "sk" / "oj-pages"
OJ_MANIFEST = OJ_DIR / "manifest.json"
OUT_DIR = ROOT / "raw" / "sk" / "dokumenty-extracted"
...
# Slovak grape-variety section: typically one item per line with `Name -
...
_GRAPE_LINE_DROP = (
    "muštové odrody", "hlavné muštové", "odrody viniča", "odporúčané",
    "povolené", "doplnkové",
)
...
def parse_grapes(section_text: str) -> dict:
    ...
    observation, details}. The Slovak Jednotný dokument lists varieties
    without an explicit role split — default to `principal`. Unmatched
    names flow into `raw/sk/extraction-unknowns.json`."""
    out: dict[str, list] = {
        "principal": [],
        "accessory": [],
        "observation": [],
...
def main() -> int:
    ...
    if not INDEX_IN.exists():
        print(f"error: {INDEX_IN} missing — run scripts/sk/00_fetch_data.py first",
              file=sys.stderr)
        return 1
    ...
    unknowns_path = ROOT / "raw" / "sk" / "extraction-unknowns.json"
