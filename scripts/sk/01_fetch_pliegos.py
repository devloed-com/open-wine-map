"""Fetch the EU-OJ "Jednotný dokument" HTML page per SK wine GI.

Pipeline stage 01 (sk).

For each wine in `raw/sk/eambrosia/index.json`, scan the `publications`
array for the first HTTP URL and request the Slovak-language variant of
that EUR-Lex page. The page contains the canonical EU single document
("JEDNOTNÝ DOKUMENT") — the Slovak equivalent of the FR cahier des
charges, the ES pliego, the IT documento unico, the AT/DE Einziges
Dokument, the SI Enotni dokument, etc.

Coverage:
  4 of the 10 SK wines carry an OJ-C publication URL (Vinohradnícka
  oblasť Tokaj, Stredoslovenská, Skalický rubín, TOKAJSKÉ VÍNO zo
  slovenskej oblasti); the other 6 are Art.107/Reg.1308/2013
  grandfathered names with no public single-document URL. They land as
  `no-publication` and stage 02 emits content-stubs. The WAF fallback
  is `scripts/sk/01b_solve_waf.py`.

Slovak variant rules (same as SI, language swap):
  /eli/.../oj      →  /eli/.../oj/slk
  /legal-content/EN/TXT/  →  /legal-content/SK/TXT/HTML/
  uri=...01.ENG    →  uri=...01.SLK

Manual overrides:
  raw/sk/oj-pages/manual_overrides.json (gitignored, optional) keyed by
  giIdentifier or slug → {"url": "<slovak-html-url>", "note": "..."}.
  Overrides bypass the eAmbrosia publications list entirely.

Outputs:
  raw/sk/oj-pages/<slug>.html          (the Slovak Jednotný-dokument HTML)
  raw/sk/oj-pages/manifest.json         (per-slug status + final URL)
...
INDEX_PATH = ROOT / "raw" / "sk" / "eambrosia" / "index.json"
OUT_DIR = ROOT / "raw" / "sk" / "oj-pages"
...
def to_slovak(url: str) -> str:
    """Best-effort URL rewrite: any EUR-Lex / EU-OJ URL → its Slovak
    HTML variant. Idempotent — already-Slovak URLs pass through."""
    if re.search(r"/oj/?(?:[#?]|$)", url) and not re.search(r"/oj/[a-z]{3}", url):
        return re.sub(r"/oj(/?)([#?]|$)", r"/oj/slk\2", url)
    new = re.sub(
        r"/legal-content/[A-Z]{2}/TXT(?:/HTML)?/",
        "/legal-content/SK/TXT/HTML/",
        url,
    )
    new = re.sub(r"(\d{2}\.)(?:ENG|FRA|DEU|ITA|POR|SPA|NLD|SLV|SLK|CES)(?=&|$)", r"\1SLK", new)
    new = re.sub(r"(:)(?:ENG|FRA|DEU|ITA|POR|SPA|NLD|SLV|SLK|CES)(?=&|$)", r"\1SLK", new)
    return new
...
def looks_like_single_document(html: str) -> bool:
    ...
    literal Slovak phrases."""
    if len(html) < 8000:
        return False
    low = html.lower()
    return (
        "jednotný dokument" in low
        or "špecifikácia výrobku" in low
        or "chránené označenie pôvodu" in low
        or "chránené zemepisné označenie" in low
        or "zemepisné označenie" in low
    )
...
def main() -> int:
    ...
    if not INDEX_PATH.exists():
        print(f"error: {INDEX_PATH} missing — run scripts/sk/00_fetch_data.py first",
              file=sys.stderr)
        return 1
    ...
    for w in tqdm(wines, desc="oj-pages", leave=False):
        ...
        if override and override.get("url"):
            ...
            source_url = to_slovak(base) if base else None
